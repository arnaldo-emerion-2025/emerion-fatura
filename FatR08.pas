...
begin
  ...
  sFiltro := ' and PedLib.SitLib = '''+ 'Faturado' +''' and PedLib.AtuEst = '''+ 'Sim' +''' and PedLb2.FlgSep = '''+ 'Sim' +'''';

  if Trim( EdPsqModPfa.Text ) <> '' then sFiltro := sFiltro + ' and PedLib.ModPfa = '''+ EdPsqModPfa.Text +'''';

  if Trim(fLimpaStr( EdDteFat1.Text )) <> '' then sFiltro := sFiltro + ' and PedLib.DteFat >= '''+ fDateToSQL( EdDteFat1.Date ) +'''';
  if Trim(fLimpaStr( EdDteFat2.Text )) <> '' then sFiltro := sFiltro + ' and PedLib.DteFat <= '''+ fDateToSQL( EdDteFat2.Date ) +'''';
  ...
  with PedLb2,SQL do begin
       ...
       Close;
       Text := ' Select PedR20.*,'+
               '        TextoOcor(IntStrZeros(PedR20.CodVen,7),'''+ '-' +''',FinVen.ApeVen,'''+ '' +''','''+ '' +''','''+ '' +''','''+ '' +''') as ApeVen,'+
               '        TextoOcor(IntStrZeros(PedR20.CodCli,7),'''+ '-' +''',FinCli.NomCli,'''+ '' +''','''+ '' +''','''+ '' +''','''+ '' +''') as NomCli,'+
               '        TextoOcor(PedR20.CodGru,'''+ '.' +''',PedR20.CodSub,'''+ '.' +''',PedR20.CodPro,'''+ '-' +''',EstPro.DscPro) as CodIte'+
