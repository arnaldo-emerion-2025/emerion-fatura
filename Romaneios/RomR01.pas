...
type
  ...
  TfmRomR01 = class(TfmPadrao)
    ...
    FatOrcNOMVEN: TStringField;
    grOrcNUMRES: TdxDBGridMaskColumn;
    grOrcDTERES: TdxDBGridDateColumn;
    grOrcTOTGER: TdxDBGridMaskColumn;
    grOrcDTEORC: TdxDBGridDateColumn;
    grOrcNOMCLI: TdxDBGridMaskColumn;
    grOrcNOMVEN: TdxDBGridMaskColumn;
    ...
    FatOrcAPETRA: TStringField;
    grOrcAPETRA: TdxDBGridColumn;
...
procedure TfmRomR01.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
...
procedure TfmRomR01.FormCreate(Sender: TObject);
begin
  inherited;
  ...
  sBase := ' Select FatOrc.NumRes,'+
           ...
           '        FatOrc.TotGer,'+
           '        FatOrc.DteOrc,'+
           '        (Select FinTra.ApeTra From FinTra Where FinTra.CodTra = FatOrc.TraSda) as ApeTra,'+
           '        TextoOcor(IntStrZeros(FatOrc.CodCli,05),'''+ '-' +''',FinCli.ApeCli,'''+ '' +''','''+ '' +''','''+ '' +''','''+ '' +''') as NomCli,'+
           '        TextoOcor(IntStrZeros(FatOrc.CodVen,05),'''+ '-' +''',FinVen.ApeVen,'''+ '' +''','''+ '' +''','''+ '' +''','''+ '' +''') as NomVen'+
           ' From FatOrc LEFT JOIN FinCli ON (FatOrc.CodCli = FinCli.CodCli)'+
           '             LEFT JOIN FinVen ON (FatOrc.CodVen = FinVen.CodVen)';

end;

procedure TfmRomR01.bbPesquisaClick(Sender: TObject);
begin
...
procedure TfmRomR01.EdPsqCodEmpExit(Sender: TObject);
begin
...
procedure TfmRomR01.EdPsqCodCliExit(Sender: TObject);
begin
...
procedure TfmRomR01.bPsqEmpClick(Sender: TObject);
begin
  inherited;
...
procedure TfmRomR01.EdPsqCodEmpKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TfmRomR01.EdPsqCodCliKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if key = 112 then begin {F1 - Iniciais}
...
procedure TfmRomR01.EdPsqCodVenExit(Sender: TObject);
begin
...
procedure TfmRomR01.EdPsqCodVenKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if key = 112 then begin {F1 - Iniciais}
...
procedure TfmRomR01.bPsqCliClick(Sender: TObject);
begin
...
procedure TfmRomR01.bPsqVenClick(Sender: TObject);
begin
...
procedure TfmRomR01.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  if key = 116 then bImprimir.OnClick(Sender);
end;

procedure TfmRomR01.FormShow(Sender: TObject);
begin
  inherited;
  EdPsqUfeRes.SetFocus;
end;

procedure TfmRomR01.PaintBoxPaint(Sender: TObject);
begin
  inherited;
...
procedure TfmRomR01.EdPsqNumResKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  if not ( key in [ '0'..'9' ] ) then key := #0;
end;

procedure TfmRomR01.bImprimirClick(Sender: TObject);
begin
  inherited;
...
procedure TfmRomR01.grOrcReloadGroupList(Sender: TObject);
begin
  inherited;
  (Sender as TdxDBGrid).FullExpand;
end;

procedure TfmRomR01.HTMLClick(Sender: TObject);
begin
  inherited;
  Save('htm', 'HTML File (*.htm; *.html)|*.htm', 'Relação de Romaneios Emitidos no Periodo.htm', grOrc.SaveToHTML);
end;

procedure TfmRomR01.ExcelClick(Sender: TObject);
begin
  inherited;
  Save('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', 'Relação de Romaneios Emitidos no Periodo.xls', grOrc.SaveToXLS);
end;

procedure TfmRomR01.XMLClick(Sender: TObject);
begin
  inherited;
  Save('xml', 'XML File (*.xml)|*.xml', 'Relação de Romaneios Emitidos no Periodo.xml', grOrc.SaveToXML);
end;

procedure TfmRomR01.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  Action := CaFree;
end;

procedure TfmRomR01.FormDestroy(Sender: TObject);
begin
  inherited;
  fmRomR01 := Nil;
end;
