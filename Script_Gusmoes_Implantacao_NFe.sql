--Scrip para a GUSMOES - implantação da NF-e
...
--Operação 8,5
--primeiro criar esses campos
alter table lojfat add flgnfe char(3);
alter table cmpnfs add flgnfe char(3);
--depois Acertar a trigger before update da tabela fatped
-- acertar uns and em alguns if´s



--Operação 9
update geremp set qtdnfs = 1, nronfs = 1 where codemp = 3
update geremp set flgnfe = 'Sim' where codemp = 3



-- Ver o que é isso
--ALTER TABLE
--ADD ID_ESTDIP INTEGER,
--ADD ID_FINCIF INTEGER,
--ADD ID_FINUFF INTEGER;
...
select fp.nronfs, fp.flgnfe, fp.numres from fatped fp
order by fp.nronfs, fp.flgnfe

select fg.nronfs, fg.flgnfe, fg.numger from fatger fg
order by fg.nronfs, fg.flgnfe

select fd.nronfs, fd.flgnfe, fd.numres from fatdev fd
order by fd.nronfs, fd.flgnfe

select lf.nronfs, lf.flgnfe, lf.numped from lojfat lf
order by lf.nronfs, lf.flgnfe

select cn.nronfs, cn.flgnfe from cmpnfs cn
order by cn.nronfs, cn.flgnfe
