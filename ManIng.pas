...
uses dxDemoUtils, Bbgeral, Bbmensag, Bbfuncao, ManGDB, ManNge;

{$R *.DFM}

procedure TfmManIng.bretornarClick(Sender: TObject);
begin

  if fmManNge.FatGerSitGer.Value = 'Faturado' then
    fmManNge.Finalizar := 'S';

  close;

end;

procedure TfmManIng.bcontinuarClick(Sender: TObject);
var
...
begin
  if fmManNge.FatGerNroNfs.Value > 0 then
  begin
    ...
    begin
      ...
      begin
        ...
        fmManNge.FatGerNroNfs.Value := QtdNfs;
    ...
    begin
      ...
      begin
        ...
        if fmManNge.FatGerSitGer.Value = 'Nao Concluido' then
        begin

          if fmManNge.FatGer.State = dsBrowse then
            fmManNge.FatGer.Edit;

          fmManNge.FatGerFlgNfs.Value := '*';
          ...
          fmManNge.FatGe2.Close;
          fmManNge.FatGe2.Open;
        ...
        begin
          ...
          begin
            ...
            Text := ' Select FinCli.NomCli,' +
              ...
              '        FinCli.CecCli,' +
              '        FinCli.Pt1Cli,' +
              '        FinCli.Fo1Cli From FinCli Where FinCli.CodCli = ''' + IntToStr(fmManNge.FatGerCodCli.Value) + '''';
            Open;
          ...
          begin
            ...
            begin
              ...
              if fmManNge.FatGerAtuEst.Value = 'Sim' then
              ...
              begin
                ...
                fmManNge.FatGerSitGer.Value := 'Faturado';
            ...
            begin
              ...
              try
                ...
                if Trim(fmManNge.FatGerNomEnt.Value) <> '' then
                begin

                  EndEnt1 := Trim(fmManNge.FatGerNomEnt.Value);

                  EndEnt1 := EndEnt1 + ' C.N.P.J :' + copy(fmManNge.FatGerCgeCli.Value, 01, 2) + '.' + copy(fmManNge.FatGerCgeCli.Value, 3, 3) + '.' +
                    copy(fmManNge.FatGerCgeCli.Value, 06, 3) + '/' + copy(fmManNge.FatGerCgeCli.Value, 9, 4) + '-' +
                    copy(fmManNge.FatGerCgeCli.Value, 13, 2);

                  if Trim(fmManNge.FatGerTenCli.Value) <> '' then
                    EndEnt2 := Trim(fmManNge.FatGerTenCli.Value) + ' ';

                  if Trim(fmManNge.FatGerNumCli.Value) <> '' then
                    EndEnt2 := EndEnt2 + Trim(fmManNge.FatGerEndCli.Value) + ', ' + Trim(fmManNge.FatGerNumCli.Value) + ' '
                  else
                    EndEnt2 := EndEnt2 + Trim(fmManNge.FatGerEndCli.Value) + ' ';

                  EndEnt2 := EndEnt2 + Trim(fmManNge.FatGerBaiCli.Value) + ' - ' + Trim(fmManNge.FatGerCidCli.Value) + ' ' + Trim(fmManNge.FatGerUfeCli.Value) +
                    ' Cep ' + Trim(fmManNge.FatGerCepCli.Value);
                ...
                begin
                  ...
                  if Trim(fmManNge.FatGerTenCli.Value) <> '' then
                    EndEnt1 := Trim(fmManNge.FatGerTenCli.Value) + ' ';

                  if Trim(fmManNge.FatGerNumCli.Value) <> '' then
                    EndEnt1 := EndEnt1 + Trim(fmManNge.FatGerEndCli.Value) + ', ' + Trim(fmManNge.FatGerNumCli.Value) + ' '
                  else
                    EndEnt1 := EndEnt1 + Trim(fmManNge.FatGerEndCli.Value) + ' ';

                  EndEnt1 := EndEnt1 + Trim(fmManNge.FatGerBaiCli.Value) + ' - ' + Trim(fmManNge.FatGerCidCli.Value) + ' ' + Trim(fmManNge.FatGerUfeCli.Value) +
                    ' Cep ' + Trim(fmManNge.FatGerCepCli.Value);

                  EndEnt2 := 'C.N.P.J.:' + copy(fmManNge.FatGerCgeCli.Value, 01, 2) + '.' + copy(fmManNge.FatGerCgeCli.Value, 3, 3) + '.' +
                    copy(fmManNge.FatGerCgeCli.Value, 06, 3) + '/' + copy(fmManNge.FatGerCgeCli.Value, 9, 4) + '-' +
                    copy(fmManNge.FatGerCgeCli.Value, 13, 2);
                ...
                Nota01[002] := fmManNge.FatGerFlgSai.Value; {Saida}
                Nota02[002] := 'Comp12';

                Nota01[003] := fmManNge.FatGerFlgEnt.Value; {Entrada}
                Nota02[003] := 'Comp12';

                Nota01[004] := Trim(fmManNge.FatGerDesNat.Value); {NatOper}
                Nota02[004] := 'Comp12';

                if Trim(fmManNge.FatGerCodCf2.Value) <> '' then {Cfop}
                  Nota01[005] := Trim(fmManNge.FatGerCodCf1.Value) + '/' + Trim(fmManNge.FatGerCodCf2.Value)
                else
                  Nota01[005] := Trim(fmManNge.FatGerCodCf1.Value);
                ...
                Nota01[007] := fmManNge.FatGerInsSub.Value; {InscEstSubTrib}
                Nota02[007] := 'Comp12';

                Nota01[010] := DateToStr(fmManNge.FatGerDteFat.Value); {DataEmis}
                Nota02[010] := 'Comp12';
                ...
                if Trim(fmManNge.FatGerUfeGer.Value) <> 'EX' then
                begin
                ...
                begin
                  ...
                  Nota01[i] := fNumZeros(IntToStr(fmManNge.FatGerNroNfs.Value), 6) + '/' + FNumZeros(IntToStr(quSQL.FieldbyName('NroGe3').AsInteger), 3);
                    {Número}
                  Nota02[i] := 'Comp12';
                ...
                if fmManNge.FatGerTotIs1.Value > 0 then
                  Nota01[185] := Prestring(FormatFloat('###,###,##0.00', fmManNge.FatGerTotIs1.Value), 14)
                else
                  Nota01[185] := ' ';

                Nota02[185] := 'Comp12';

                if fmManNge.FatGerTotSe1.Value > 0 then
                  Nota01[186] := Prestring(FormatFloat('###,###,##0.00', fmManNge.FatGerTotSe1.Value), 14)
                else
                  Nota01[186] := ' ';
                ...
                Nota01[053] := fmManNge.FatGerTxfIpi.Value; {TextIpi}
                Nota02[053] := 'Comp20';

                Nota01[054] := fmManNge.FatGerTxfIcm.Value; {TextIcm}
                Nota02[054] := 'Comp20';
                ...
                Nota01[067] := fmManNge.FatGerNomTra.Value; {NomeTransp}
                Nota02[067] := 'Comp12';

                Nota01[068] := fmManNge.FatGerTipFrt.Value; {FretePc}
                Nota02[068] := 'Comp12';

                Nota01[069] := fmManNge.FatGerPlcTra.Value; {Placa}
                Nota02[069] := 'Comp12';

                Nota01[070] := fmManNge.FatGerUfePlc.Value; {UfPlaca}
                Nota02[070] := 'Comp12';

                if Trim(fmManNge.FatGerCgcTra.Value) <> '' then

                  Nota01[071] := copy(fmManNge.FatGerCgcTra.Value, 01, 2) + '.' + copy(fmManNge.FatGerCgcTra.Value, 3, 3) + '.' +
                    copy(fmManNge.FatGerCgcTra.Value, 06, 3) + '/' + copy(fmManNge.FatGerCgcTra.Value, 9, 4) + '-' +
                    copy(fmManNge.FatGerCgcTra.Value, 13, 2)
                ...
                Nota01[072] := Trim(fmManNge.FatGerTenTra.Value) + ' ' + Trim(fmManNge.FatGerEndTra.Value) + ', ' + Trim(fmManNge.FatGerNumTra.Value) + ' ' +
                  Trim(fmManNge.FatGerBaiTra.Value);
                Nota02[072] := 'Comp12';

                Nota01[073] := fmManNge.FatGerCidTra.Value; {CidTransp}
                Nota02[073] := 'Comp12';

                Nota01[074] := fmManNge.FatGerUfeTra.Value; {UFTransp}
                Nota02[074] := 'Comp12';

                Nota01[075] := fmManNge.FatGerInsTra.Value; {InscTransp}
                Nota02[075] := 'Comp12';
                ...
                if fmManNge.FatGerAltVol.Value > 0 then
                  Nota01[076] := Prestring(IntToStr(fmManNge.FatGerAltVol.Value), 7) {QtdeVol}
                else
                  Nota01[076] := ' ';

                Nota02[076] := 'Comp12';

                Nota01[077] := fmManNge.FatGerEspGer.Value; {Especie}
                Nota02[077] := 'Comp12';

                Nota01[078] := fmManNge.FatGerMarGer.Value; {Marca}
                Nota02[078] := 'Comp12';

                Nota01[079] := fmManNge.FatGerNroGer.Value; {Numero}
                Nota02[079] := 'Comp12';

                Nota01[080] := Prestring(FormatFloat('###,###,##0.00', fmManNge.FatGerInfBrt.Value), 14); {PesBruto}
                Nota02[080] := 'Comp12';

                Nota01[081] := Prestring(FormatFloat('###,###,##0.00', fmManNge.FatGerInfLiq.Value), 14); {PesLiq}
                Nota02[081] := 'Comp12';

                if (Trim(fmManNge.FatGerOb1Ger.Value) = '') and
                  (Trim(fmManNge.FatGerOb2Ger.Value) = '') and
                  (Trim(fmManNge.FatGerOb3Ger.Value) = '') and
                  (Trim(fmManNge.FatGerOb4Ger.Value) = '') and
                  (Trim(fmManNge.FatGerOb5Ger.Value) = '') and
                  (Trim(fmManNge.FatGerOb6Ger.Value) = '') and
                  (Trim(fmManNge.FatGerOb7Ger.Value) = '') and
                  (Trim(fmManNge.FatGerOb8Ger.Value) = '') then
                begin

                  Nota01[187] := copy(fmManNge.FatGerObsGer.Value, 001, 070); {Obs1}
                  Nota02[187] := 'Comp20';

                  Nota01[188] := copy(fmManNge.FatGerObsGer.Value, 071, 070); {Obs2}
                  Nota02[188] := 'Comp20';

                  Nota01[189] := copy(fmManNge.FatGerObsGer.Value, 141, 070); {Obs3}
                  Nota02[189] := 'Comp20';

                  Nota01[190] := copy(fmManNge.FatGerObsGer.Value, 211, 070); {Obs4}
                  Nota02[190] := 'Comp20';

                  Nota01[191] := copy(fmManNge.FatGerObsGer.Value, 281, 070); {Obs5}
                  Nota02[191] := 'Comp20';

                  Nota01[192] := copy(fmManNge.FatGerObsGer.Value, 351, 070); {Obs6}
                  Nota02[192] := 'Comp20';
                ...
                begin
                  ...
                  Nota01[187] := fmManNge.FatGerOb1Ger.Value; {Obs1}
                  Nota02[187] := 'Comp20';

                  Nota01[188] := fmManNge.FatGerOb2Ger.Value; {Obs2}
                  Nota02[188] := 'Comp20';

                  Nota01[189] := fmManNge.FatGerOb3Ger.Value; {Obs3}
                  Nota02[189] := 'Comp20';

                  Nota01[190] := fmManNge.FatGerOb4Ger.Value; {Obs4}
                  Nota02[190] := 'Comp20';

                  Nota01[191] := fmManNge.FatGerOb5Ger.Value; {Obs5}
                  Nota02[191] := 'Comp20';

                  Nota01[192] := fmManNge.FatGerOb6Ger.Value; {Obs6}
                  Nota02[192] := 'Comp20';

                  Nota01[193] := fmManNge.FatGerOb7Ger.Value; {Obs7}
                  Nota02[193] := 'Comp20';

                  Nota01[194] := fmManNge.FatGerOb8Ger.Value; {Obs8}
                  Nota02[194] := 'Comp20';

                end;

                Nota01[117] := IntToStr(fmManNge.FatGerCodCli.Value); {código do Cliente}
                Nota02[117] := 'Comp12';

                Nota01[118] := fNumZeros(IntToStr(fmManNge.FatGerCodVen.Value), 3); {código do Vendedor}
                Nota02[118] := 'Comp12';

                Nota01[119] := ''; {SeuPed}
                Nota02[119] := 'Comp12';

                Nota01[120] := fNumZeros(IntToStr(fmManNge.FatGerNumGer.Value), 7); {Nosso Pedido}
                Nota02[120] := 'Comp12';
                ...
                ValorExt := fExtenso(fmManNge.FatGerTotGer.Value);
                ...
                Nota01[126] := fmManNge.FatGerDesReg.Value; {Observações Sobre Desconto}
                Nota02[126] := 'Comp20';
                ...
                //            qtdnfis := (fmManNge.FatGerNroNfs.Value - fmManNge.FatGerQtdNfs.Value)+ 1;  {Quantidade de Notas Fiscais Impressas}

                qtdnfis := 1; {Quantidade de Notas Fiscais Impressas}

                while qtdnfis <= fmManNge.FatGerQtdNfs.Value do
                begin
                  ...
                  if qtdnfis = fmManNge.FatGerQtdNfs.Value then
                  begin
                    ...
                    {BaseIcms}
                    if fmManNge.FatGerBasIc1.Value > 0 then
                      Nota01[057] := Prestring(FormatFloat('###,###,##0.00', fmManNge.FatGerBasIc1.Value), 14)
                    else
                      Nota01[057] := ' ';
                    ...
                    {ValIcms}
                    if fmManNge.FatGerTotIc1.Value > 0 then
                      Nota01[058] := Prestring(FormatFloat('###,###,##0.00', fRound(fmManNge.FatGerTotIc1.Value, 2)), 14)
                    else
                      Nota01[058] := ' ';
                    ...
                    {BaseIcmsSub}
                    if fmManNge.FatGerBasSu1.Value > 0 then
                      Nota01[059] := Prestring(FormatFloat('###,###,##0.00', fmManNge.FatGerBasSu1.Value), 14)
                    else
                      Nota01[059] := ' ';
                    ...
                    {ValIcmsSub}
                    if fmManNge.FatGerTotSu1.Value > 0 then
                      Nota01[060] := Prestring(FormatFloat('###,###,##0.00', fRound(fmManNge.FatGerTotSu1.Value, 2)), 14)
                    else
                      Nota01[060] := ' ';

                    Nota02[060] := 'Comp12';

                    Nota01[061] := Prestring(FormatFloat('###,###,##0.00', fmManNge.FatGerTotIt1.Value), 14); {TotalProds}
                    Nota02[061] := 'Comp12';

                    if fmManNge.FatGerTotFrt.Value > 0 then
                      Nota01[062] := Prestring(FormatFloat('###,###,##0.00', fmManNge.FatGerTotFrt.Value), 14) {Frete}
                    else
                      Nota01[062] := ' ';

                    Nota02[062] := 'Comp12';

                    if fmManNge.FatGerTotSeg.Value > 0 then
                      Nota01[063] := Prestring(FormatFloat('###,###,##0.00', fmManNge.FatGerTotSeg.Value), 14) {Seguro}
                    else
                      Nota01[063] := ' ';

                    Nota02[063] := 'Comp12';

                    if fmManNge.FatGerTotDes.Value > 0 then
                      Nota01[064] := Prestring(FormatFloat('###,###,##0.00', fmManNge.FatGerTotDes.Value), 14) {Outras}
                    else
                      Nota01[064] := ' ';

                    Nota02[064] := 'Comp12';

                    if fmManNge.FatGerTotIp1.Value > 0 then
                      Nota01[065] := Prestring(FormatFloat('###,###,##0.00', fRound(fmManNge.FatGerTotIp1.Value, 2)), 14) {TotalIPI}
                    else
                      Nota01[065] := ' ';

                    Nota02[065] := 'Comp12';

                    if fmManNge.FatGerTotGe1.Value > 0 then
                      Nota01[066] := Prestring(FormatFloat('###,###,##0.00', fmManNge.FatGerTotGe1.Value), 14) {TotalNota}
                    else
                      Nota01[066] := ' ';

                    Nota02[066] := 'Comp12';

                    Nota01[126] := fmManNge.FatGerDesReg.Value; {Descontos Gerais na Nota}
                    Nota02[126] := 'Comp20';

                    if fmManNge.FatGerTotDsr.Value > 0 then
                      Nota01[127] := PreString('-' + Trim(FormatFloat('###,##0.00', fmManNge.FatGerTotDsr.Value)), 12) {Descontos Gerais na Nota}
                    else
                      Nota01[127] := ' ';
                  ...
                  ValorExt := fExtenso(fmManNge.FatGerTotGe1.Value);
                        ...
                        begin
                          ...
                          begin
                            ...
                            begin
                              ...
                              rdprint1.impf(Lin, Col, fNumZeros(IntToStr(fmManNge.FatGerNroNfs.Value), 6), [EXPANDIDO, negrito]);
                                            ...
                                            begin
                                              ...
                                              begin
                                                ...
                                                begin
                                                  ...
                                                  if (fmManNge.FatGerTotDsr.Value > 0) and (SeqNfg = QtiNfg) then
                                                  begin
                              ...
                              begin
                                ...
                                begin
                                  ...
                                  begin
                                    ...
                                    if (Reg = 67) and (qtdnfis < fmManNge.FatGerQtdNfs.Value) then
                                    begin
                                    ...
                                    begin
                                      ...
                                      if (Reg >= 68) and (Reg <= 81) and (qtdnfis < fmManNge.FatGerQtdNfs.Value) then
                                      begin
                                      ...
                                      begin
                                        ...
                                        if (reg >= 187) and (reg <= 194) and (qtdnfis < fmManNge.FatGerQtdNfs.Value) then
                                        begin

                                          if reg = 187 then
                                            Linha := 'Continua ' + Trim(IntToStr(qtdnot)) + '/' + Trim(IntToStr(fmManNge.FatGerQtdNfs.Value))
                                          else
                                            Linha := ' ';
                ...
                begin
                  ...
                  fmManNge.FatGerFlgAtu.Value := 'F';

                  fmManNge.FatGerFlgImp.Value := 'Sim';
...
procedure TfmManIng.FormShow(Sender: TObject);
begin
  inherited;
  ...
  begin
    ...
    begin
      ...
      GerEmp.Close;
      GerEmp.Params[0].AsInteger := fmManNge.FatGerCodEmp.Value;
      GerEmp.Open;

      fmManNge.FatGer.Edit;

      if fmManNge.FatGerNroNfs.Value = 0 then
        fmManNge.FatGerNroNfs.Value := GerEmp.FieldbyName('QtdNfs').AsInteger + 1;

      if fmManNge.FatGerNroNfs.Value > 0 then
        EdNroNfs.Text := IntToStr(fmManNge.FatGerNroNfs.Value)
      else
        EdNroNfs.Text := '0';
...
procedure TfmManIng.PaintBoxPaint(Sender: TObject);
begin
