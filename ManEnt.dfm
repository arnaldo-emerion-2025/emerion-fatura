inherited fmManEnt: TfmManEnt
  Left = 157
  Top = 206
  Caption = 
    'Envio de arquivos para a contabilidade - Notas emitidas - Entrad' +
    'as'
  ClientHeight = 394
  ClientWidth = 762
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  object PaintBox: TPaintBox
    Left = 0
    Top = 0
    Width = 762
    Height = 394
    Align = alClient
    OnPaint = PaintBoxPaint
  end
  object Bevel2: TBevel
    Left = 176
    Top = 11
    Width = 584
    Height = 3
    Shape = bsTopLine
    Style = bsRaised
  end
  object Label11: TLabel
    Left = 4
    Top = 108
    Width = 115
    Height = 14
    Caption = 'Notas selecionadas'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Bevel3: TBevel
    Left = 176
    Top = 116
    Width = 584
    Height = 3
    Shape = bsTopLine
    Style = bsRaised
  end
  object Label7: TLabel
    Left = 4
    Top = 4
    Width = 164
    Height = 14
    Caption = 'Informações para pesquisa'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label2: TLabel
    Left = 4
    Top = 77
    Width = 125
    Height = 14
    Caption = 'Apelido para o arquivo:'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object Label26: TLabel
    Left = 4
    Top = 26
    Width = 50
    Height = 14
    Caption = 'Empresa:'
    Color = clBtnFace
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object BbPsqEmp: TSpeedButton
    Left = 278
    Top = 24
    Width = 23
    Height = 21
    Hint = 'Pesquisar Empresas Existentes'
    Flat = True
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = []
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    ParentFont = False
    OnClick = BbPsqEmpClick
  end
  object Label8: TLabel
    Left = 4
    Top = 51
    Width = 129
    Height = 14
    Caption = 'Emitidas no periodo de:'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object Label1: TLabel
    Left = 286
    Top = 52
    Width = 6
    Height = 14
    Caption = 'a'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object grPed1: TdxDBGraphicEdit
    Left = 3
    Top = 121
    Width = 759
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Haettenschweiler'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = clBlack
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 9
    TabStop = False
    DblClickActivate = False
    Stretch = True
    ToolbarLayout.Buttons = []
    ToolbarLayout.IsPopupMenu = False
    ToolbarPosStored = False
    Height = 251
  end
  object grRes: ThGrid
    Left = 5
    Top = 123
    Width = 753
    Height = 245
    Selected.Strings = (
      'NRONFS'#9'13'#9'No. Nota Fiscal'
      'DTEFAT'#9'10'#9'Emitido Em'
      'CODCF1'#9'5'#9'CFOP'
      'NOMCLI'#9'26'#9'Cliente/Fornecedor'
      'TOTIPI'#9'10'#9'   Total IPI'
      'TOTICM'#9'10'#9'     Total ICMS'
      'TOTGER'#9'10'#9'      Total Nota'
      'MODENV'#9'10'#9'Tipo                  ')
    IniAttributes.Delimiter = ';;'
    TitleColor = clBtnFace
    FixedCols = 0
    ShowHorzScrollBar = True
    ShowVertScrollBar = False
    BorderStyle = bsNone
    Color = 16577773
    DataSource = DsRes
    Font.Charset = ANSI_CHARSET
    Font.Color = clBackground
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = []
    KeyOptions = []
    Options = [dgTitles, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit, dgWordWrap]
    ParentFont = False
    TabOrder = 10
    TitleAlignment = taLeftJustify
    TitleFont.Charset = ANSI_CHARSET
    TitleFont.Color = clBlack
    TitleFont.Height = -12
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = [fsBold]
    TitleLines = 3
    TitleButtons = False
    IndicatorColor = icYellow
    CorDeFoco = clInfoBk
  end
  object bbPesquisa: TBitBtn
    Left = 324
    Top = 74
    Width = 149
    Height = 39
    Caption = '&Selecionar'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 7
    OnClick = bbPesquisaClick
    Glyph.Data = {
      
      3232323232323232323232323232323232323232323232323232}
  end
  object bConfirma: TBitBtn
    Left = 475
    Top = 74
    Width = 149
    Height = 39
    Hint = 'Enviar Informações Selecionadas'
    Caption = '&Enviar ...'
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 8
    OnClick = bConfirmaClick
    Glyph.Data = {
      
      C0CFD0C0CFD0C0CFD000}
  end
  object EdPsqApeArq: TdxColorEdit
    Left = 175
    Top = 73
    Width = 101
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 4
    Alignment = taRightJustify
    CharCase = ecUpperCase
    MaxLength = 8
    CorDeFoco = clInfoBk
    StoredValues = 3
  end
  object EdPsqTipArq: TdxColorEdit
    Left = 276
    Top = 73
    Width = 45
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 5
    Text = '.E01'
    OnKeyPress = EdPsqAnoFatKeyPress
    Alignment = taCenter
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
    StoredValues = 1
  end
  object rgStatus: TRadioGroup
    Left = 626
    Top = 18
    Width = 136
    Height = 96
    Caption = ' Status '
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ItemIndex = 1
    Items.Strings = (
      'Enviados'
      'Não Enviados'
      'Todos')
    ParentFont = False
    TabOrder = 6
  end
  object dxDBGraphicEdit1: TdxDBGraphicEdit
    Left = 3
    Top = 371
    Width = 759
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Haettenschweiler'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = clBlack
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 11
    TabStop = False
    DblClickActivate = False
    Stretch = True
    ToolbarLayout.Buttons = []
    ToolbarLayout.IsPopupMenu = False
    ToolbarPosStored = False
    Height = 21
  end
  object Panel1: TPanel
    Left = 5
    Top = 373
    Width = 753
    Height = 15
    BevelOuter = bvNone
    TabOrder = 12
    object ProgressBar: TdxfProgressBar
      Left = 0
      Top = 0
      Width = 753
      Height = 15
      BarBevelOuter = bvNone
      BevelOuter = bvLowered
      Orientation = orHorizontal
      Max = 100
      Min = 0
      Position = 0
      ShowText = True
      ShowTextStyle = stsPercent
      BeginColor = clNavy
      EndColor = clWhite
      Style = sExSolid
      Step = 10
      TransparentGlyph = True
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object EdPsqCodEmp: TdxColorEdit
    Left = 175
    Top = 23
    Width = 101
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 0
    OnExit = EdPsqCodEmpExit
    OnKeyDown = EdPsqCodEmpKeyDown
    OnKeyPress = EdPsqAnoFatKeyPress
    Alignment = taRightJustify
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
    StoredValues = 1
  end
  object EdPsqApeEmp: TdxColorEdit
    Left = 301
    Top = 23
    Width = 325
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 1
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdDteFat1: TdxColorDateEdit
    Left = 175
    Top = 48
    Width = 101
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 2
    Date = -700000
    SaveTime = False
    UseEditMask = True
    CorDeFoco = clInfoBk
    StoredValues = 4
  end
  object EdDteFat2: TdxColorDateEdit
    Left = 301
    Top = 48
    Width = 101
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 3
    Date = -700000
    SaveTime = False
    UseEditMask = True
    CorDeFoco = clInfoBk
    StoredValues = 4
  end
  object FatEnv: TwwQuery
    CachedUpdates = True
    DatabaseName = 'ISade'
    SQL.Strings = (
      'Select * From FatEnv'
      'Where FatEnv.SeqArq  = :SeqArq'
      'Order by FatEnv.DteFat,FatEnv.NroNfs')
    ValidateWithMask = True
    Left = 6
    Top = 311
    ParamData = <
      item
        DataType = ftString
        Name = 'SeqArq'
        ParamType = ptInput
      end>
    object FatEnvNRONFS: TFloatField
      Alignment = taLeftJustify
      DisplayLabel = 'No. Nota Fiscal'
      DisplayWidth = 13
      FieldName = 'NRONFS'
      Origin = 'ISADE.FATENV.NRONFS'
    end
    object FatEnvDTEFAT: TDateTimeField
      DisplayLabel = 'Emitido Em'
      DisplayWidth = 10
      FieldName = 'DTEFAT'
    end
    object FatEnvCODCF1: TStringField
      DisplayLabel = 'CFOP'
      DisplayWidth = 5
      FieldName = 'CODCF1'
      Origin = 'ISADE.FATENV.CODCF1'
      FixedChar = True
      Size = 15
    end
    object FatEnvNOMCLI: TStringField
      DisplayLabel = 'Cliente/Fornecedor'
      DisplayWidth = 26
      FieldName = 'NOMCLI'
      Origin = 'ISADE.FATENV.NOMCLI'
      FixedChar = True
      Size = 70
    end
    object FatEnvTOTIPI: TFloatField
      DisplayLabel = '   Total IPI'
      DisplayWidth = 10
      FieldName = 'TOTIPI'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object FatEnvTOTICM: TFloatField
      DisplayLabel = '     Total ICMS'
      DisplayWidth = 10
      FieldName = 'TOTICM'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object FatEnvTOTGER: TFloatField
      DisplayLabel = '      Total Nota'
      DisplayWidth = 10
      FieldName = 'TOTGER'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object FatEnvMODENV: TStringField
      DisplayLabel = 'Tipo                  '
      DisplayWidth = 10
      FieldName = 'MODENV'
      FixedChar = True
      Size = 45
    end
    object FatEnvNUMRES: TIntegerField
      DisplayLabel = 'No. Pedido'
      DisplayWidth = 10
      FieldName = 'NUMRES'
      Visible = False
    end
    object FatEnvDTERES: TDateTimeField
      DisplayLabel = 'Emissão'
      DisplayWidth = 10
      FieldName = 'DTERES'
      Visible = False
    end
    object FatEnvSEQARQ: TStringField
      FieldName = 'SEQARQ'
      Visible = False
      FixedChar = True
      Size = 22
    end
    object FatEnvCODCLI: TIntegerField
      DisplayLabel = 'Cliente'
      DisplayWidth = 10
      FieldName = 'CODCLI'
      Visible = False
    end
    object FatEnvCODEMP: TIntegerField
      FieldName = 'CODEMP'
      Visible = False
    end
    object FatEnvHREFAT: TStringField
      FieldName = 'HREFAT'
      Visible = False
      FixedChar = True
      Size = 8
    end
    object FatEnvTOTFAT: TFloatField
      FieldName = 'TOTFAT'
      Visible = False
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object FatEnvEMPRES: TIntegerField
      FieldName = 'EMPRES'
      Visible = False
    end
    object FatEnvLIBRES: TIntegerField
      FieldName = 'LIBRES'
      Visible = False
    end
    object FatEnvFATRES: TIntegerField
      FieldName = 'FATRES'
      Visible = False
    end
    object FatEnvEMPGER: TIntegerField
      FieldName = 'EMPGER'
      Visible = False
    end
    object FatEnvNUMGER: TIntegerField
      FieldName = 'NUMGER'
      Visible = False
    end
    object FatEnvSITFAT: TStringField
      FieldName = 'SITFAT'
      Visible = False
      FixedChar = True
      Size = 45
    end
    object FatEnvEMPNFS: TIntegerField
      DisplayWidth = 10
      FieldName = 'EMPNFS'
      Visible = False
    end
    object FatEnvSEQNFS: TIntegerField
      DisplayWidth = 10
      FieldName = 'SEQNFS'
      Visible = False
    end
    object FatEnvDTEGER: TDateTimeField
      FieldName = 'DTEGER'
      Visible = False
    end
    object FatEnvDTENFS: TDateTimeField
      FieldName = 'DTENFS'
      Visible = False
    end
    object FatEnvUFEFAT: TStringField
      FieldName = 'UFEFAT'
      Origin = 'ISADE.FATENV.UFEFAT'
      Visible = False
      FixedChar = True
      Size = 2
    end
    object FatEnvINSCLI: TStringField
      FieldName = 'INSCLI'
      Origin = 'ISADE.FATENV.INSCLI'
      Visible = False
      FixedChar = True
      Size = 18
    end
    object FatEnvCGCCLI: TStringField
      FieldName = 'CGCCLI'
      Origin = 'ISADE.FATENV.CGCCLI'
      Visible = False
      FixedChar = True
      Size = 18
    end
    object FatEnvINTFIN: TStringField
      FieldName = 'INTFIN'
      Origin = 'ISADE.FATENV.INTFIN'
      Visible = False
      FixedChar = True
      Size = 3
    end
    object FatEnvTOTDSR: TFloatField
      FieldName = 'TOTDSR'
      Origin = 'ISADE.FATENV.TOTDSR'
      Visible = False
    end
    object FatEnvBASIPI: TFloatField
      FieldName = 'BASIPI'
      Origin = 'ISADE.FATENV.BASIPI'
      Visible = False
    end
    object FatEnvBASICM: TFloatField
      FieldName = 'BASICM'
      Origin = 'ISADE.FATENV.BASICM'
      Visible = False
    end
    object FatEnvEMPENT: TIntegerField
      FieldName = 'EMPENT'
      Origin = 'ISADE.FATENV.EMPENT'
      Visible = False
    end
    object FatEnvDTEENT: TDateTimeField
      FieldName = 'DTEENT'
      Origin = 'ISADE.FATENV.DTEENT'
      Visible = False
    end
    object FatEnvNUMENT: TIntegerField
      FieldName = 'NUMENT'
      Origin = 'ISADE.FATENV.NUMENT'
      Visible = False
    end
    object FatEnvBASSUB: TFloatField
      DisplayWidth = 10
      FieldName = 'BASSUB'
      Origin = 'ISADE.FATENV.BASSUB'
      Visible = False
    end
    object FatEnvTOTSUB: TFloatField
      DisplayWidth = 10
      FieldName = 'TOTSUB'
      Origin = 'ISADE.FATENV.TOTSUB'
      Visible = False
    end
    object FatEnvFATDEV: TIntegerField
      DisplayWidth = 10
      FieldName = 'FATDEV'
      Origin = 'ISADE.FATENV.FATDEV'
      Visible = False
    end
  end
  object DsRes: TwwDataSource
    DataSet = FatEnv
    Left = 34
    Top = 311
  end
  object quSql: TwwQuery
    DatabaseName = 'ISade'
    UniDirectional = True
    ValidateWithMask = True
    Left = 6
    Top = 339
  end
  object SaveDialog: TSaveDialog
    FileName = 'ExpGrid'
    InitialDir = 'C:\ContFat'
    Options = [ofOverwritePrompt, ofHideReadOnly]
    Left = 34
    Top = 339
  end
end
