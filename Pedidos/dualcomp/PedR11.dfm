inherited fmPedR11: TfmPedR11
  Left = 0
  Caption = 'Fechamento de Comissões (Sintetico)'
  ClientHeight = 482
  ClientWidth = 786
  Position = poDesigned
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object PaintBox: TPaintBox
    Left = 0
    Top = 0
    Width = 786
    Height = 482
    Align = alClient
    OnPaint = PaintBoxPaint
  end
  object Label17: TLabel
    Left = 4
    Top = 111
    Width = 56
    Height = 13
    Caption = 'Emitidos Em'
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object Label18: TLabel
    Left = 258
    Top = 111
    Width = 6
    Height = 13
    Caption = 'a'
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object Label26: TLabel
    Left = 4
    Top = 18
    Width = 41
    Height = 13
    Caption = 'Empresa'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label22: TLabel
    Left = 4
    Top = 88
    Width = 46
    Height = 13
    Caption = 'Vendedor'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object BbPsqEmp: TSpeedButton
    Left = 250
    Top = 16
    Width = 23
    Height = 21
    Hint = 'Pesquisar Empresas Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = BbPsqEmpClick
  end
  object BbPsqVen: TSpeedButton
    Left = 250
    Top = 85
    Width = 23
    Height = 21
    Hint = 'Pesquisar Vendedores Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = BbPsqVenClick
  end
  object Label10: TLabel
    Left = 4
    Top = 0
    Width = 154
    Height = 13
    Caption = 'Informações para Pesquisa'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Bevel1: TBevel
    Left = 164
    Top = 6
    Width = 620
    Height = 3
    Shape = bsTopLine
    Style = bsRaised
  end
  object Label2: TLabel
    Left = 3
    Top = 148
    Width = 144
    Height = 13
    Caption = 'Vendedores Selecionados'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Bevel2: TBevel
    Left = 163
    Top = 155
    Width = 620
    Height = 3
    Shape = bsTopLine
    Style = bsRaised
  end
  object Label5: TLabel
    Left = 151
    Top = 19
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label7: TLabel
    Left = 151
    Top = 88
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label9: TLabel
    Left = 151
    Top = 111
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label20: TLabel
    Left = 4
    Top = 42
    Width = 29
    Height = 13
    Caption = 'Grupo'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label1: TLabel
    Left = 4
    Top = 65
    Width = 47
    Height = 13
    Caption = 'Categoria'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label3: TLabel
    Left = 151
    Top = 65
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label6: TLabel
    Left = 151
    Top = 42
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object bPsqCve: TSpeedButton
    Left = 250
    Top = 62
    Width = 23
    Height = 21
    Hint = 'Pesquisar Categorias de Vendedores Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = bPsqCveClick
  end
  object bPsqGve: TSpeedButton
    Left = 250
    Top = 39
    Width = 23
    Height = 21
    Hint = 'Pesquisar Grupos de Vendedores Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = bPsqGveClick
  end
  object EdPsqDteres1: TdxColorDateEdit
    Left = 162
    Top = 107
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 8
    Date = -700000
    SaveTime = False
    UseEditMask = True
    CorDeFoco = clInfoBk
    StoredValues = 4
  end
  object EdPsqDteres2: TdxColorDateEdit
    Left = 273
    Top = 107
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 9
    Date = -700000
    SaveTime = False
    UseEditMask = True
    CorDeFoco = clInfoBk
    StoredValues = 4
  end
  object EdPsqCodVen: TdxColorEdit
    Left = 162
    Top = 84
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 6
    OnExit = EdPsqCodVenExit
    OnKeyDown = EdPsqCodVenKeyDown
    OnKeyPress = EdPsqCodEmpKeyPress
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdPsqCodEmp: TdxColorEdit
    Left = 162
    Top = 15
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 0
    OnExit = EdPsqCodEmpExit
    OnKeyDown = EdPsqCodEmpKeyDown
    OnKeyPress = EdPsqCodEmpKeyPress
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdPsqNomVen: TdxColorEdit
    Left = 273
    Top = 84
    Width = 512
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 7
    CharCase = ecUpperCase
    CorDeFoco = clBlack
  end
  object EdPsqApeEmp: TdxColorEdit
    Left = 273
    Top = 15
    Width = 512
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 1
    CharCase = ecUpperCase
    CorDeFoco = clBlack
  end
  object bbPesquisa: TBitBtn
    Left = 454
    Top = 108
    Width = 164
    Height = 44
    Caption = '&Selecionar'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 10
    OnClick = bbPesquisaClick
    Glyph.Data = {
      
      3232323232323232323232323232323232323232323232323232}
  end
  object grRes: TdxDBGrid
    Left = 2
    Top = 164
    Width = 782
    Height = 318
    Bands = <
      item
        Width = 889
      end>
    DefaultLayout = False
    HeaderPanelRowCount = 1
    KeyField = 'CODVEN'
    ShowGroupPanel = True
    ShowSummaryFooter = True
    SummaryGroups = <
      item
        DefaultGroup = True
        SummaryItems = <
          item
            ColumnName = 'grResTOTPED'
            SummaryFormat = '###,###,##0.00'
            SummaryType = cstSum
          end
          item
            ColumnName = 'grResQTDPED'
            SummaryFormat = '####0'
            SummaryType = cstSum
          end
          item
            ColumnName = 'grResTOTLIB'
            SummaryFormat = '###,###,##0.00'
            SummaryType = cstSum
          end
          item
            SummaryFormat = '###,###,##0.00'
            SummaryType = cstSum
          end
          item
            SummaryFormat = '####0'
            SummaryType = cstSum
          end
          item
            ColumnName = 'grResCREDITO'
            SummaryFormat = '###,###,##0.00'
            SummaryType = cstSum
          end
          item
            SummaryFormat = '###,###,##0.00'
            SummaryType = cstSum
          end
          item
            ColumnName = 'grResQTDCLI'
            SummaryFormat = '####0'
            SummaryType = cstSum
          end
          item
            ColumnName = 'grResQTDCMP'
            SummaryFormat = '####0'
            SummaryType = cstSum
          end>
        Name = 'grResSummaryGroup1'
      end>
    SummarySeparator = ', '
    BorderStyle = bsNone
    Color = clInfoBk
    Font.Charset = ANSI_CHARSET
    Font.Color = clNavy
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    PopupMenu = PopUpMenu
    TabOrder = 11
    BandColor = clMenu
    BandFont.Charset = ANSI_CHARSET
    BandFont.Color = clWindowText
    BandFont.Height = -11
    BandFont.Name = 'Tahoma'
    BandFont.Style = [fsBold]
    DataSource = DsRes
    Filter.Active = True
    Filter.AutoDataSetFilter = True
    Filter.Criteria = {00000000}
    GroupPanelColor = clAppWorkSpace
    GroupPanelFontColor = clInfoBk
    GroupNodeColor = clInfoBk
    HeaderFont.Charset = ANSI_CHARSET
    HeaderFont.Color = clWindowText
    HeaderFont.Height = -11
    HeaderFont.Name = 'Tahoma'
    HeaderFont.Style = [fsBold]
    LookAndFeel = lfFlat
    OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEnterShowEditor, edgoImmediateEditor, edgoTabThrough, edgoVertThrough]
    OptionsCustomize = [edgoBandPanelSizing, edgoBandSizing, edgoColumnMoving, edgoColumnSizing]
    OptionsDB = [edgoCanNavigation, edgoLoadAllRecords, edgoUseBookmarks]
    OptionsView = [edgoRowSelect, edgoUseBitmap]
    PreviewFont.Charset = ANSI_CHARSET
    PreviewFont.Color = clBlue
    PreviewFont.Height = -11
    PreviewFont.Name = 'Tahoma'
    PreviewFont.Style = []
    PreviewLines = 0
    RowFooterColor = clInfoBk
    RowFooterTextColor = clRed
    ShowGrid = False
    ShowHiddenInCustomizeBox = True
    ShowRowFooter = True
    OnReloadGroupList = grResReloadGroupList
    object grResCODVEN: TdxDBGridColumn
      Width = 63
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CODVEN'
    end
    object grResNOMVEN: TdxDBGridMaskColumn
      Caption = 'Representante'
      Width = 132
      BandIndex = 0
      RowIndex = 0
      FieldName = 'NOMVEN'
    end
    object grResTOTPED: TdxDBGridMaskColumn
      HeaderAlignment = taRightJustify
      Width = 100
      BandIndex = 0
      RowIndex = 0
      FieldName = 'TOTPED'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0.00'
    end
    object grResQTDPED: TdxDBGridMaskColumn
      Caption = 'Qtd Ped'
      HeaderAlignment = taRightJustify
      Width = 66
      BandIndex = 0
      RowIndex = 0
      FieldName = 'QTDPED'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '####0'
    end
    object grResTOTLIB: TdxDBGridMaskColumn
      Caption = 'Faturado'
      HeaderAlignment = taRightJustify
      Width = 73
      BandIndex = 0
      RowIndex = 0
      FieldName = 'TOTLIB'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0.00'
    end
    object grResCREDITO: TdxDBGridMaskColumn
      HeaderAlignment = taRightJustify
      Width = 74
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CREDITO'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0.00'
    end
    object grResQTDCLI: TdxDBGridMaskColumn
      HeaderAlignment = taRightJustify
      Width = 96
      BandIndex = 0
      RowIndex = 0
      FieldName = 'QTDCLI'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '####0'
    end
    object grResQTDCMP: TdxDBGridMaskColumn
      Caption = 'Ativos'
      HeaderAlignment = taRightJustify
      Width = 59
      BandIndex = 0
      RowIndex = 0
      FieldName = 'QTDCMP'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '####0'
    end
  end
  object bImprimir: TBitBtn
    Left = 620
    Top = 108
    Width = 164
    Height = 44
    Caption = '&Imprimir'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 12
    OnClick = bImprimirClick
    Glyph.Data = {
      
      D000}
  end
  object EdPsqCodGve: TdxColorEdit
    Left = 162
    Top = 38
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 2
    OnExit = EdPsqCodGveExit
    OnKeyPress = EdPsqCodEmpKeyPress
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdPsqCodCve: TdxColorEdit
    Left = 162
    Top = 61
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 4
    OnExit = EdPsqCodCveExit
    OnKeyPress = EdPsqCodEmpKeyPress
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdPsqNomGve: TdxColorEdit
    Left = 273
    Top = 38
    Width = 512
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 3
    CharCase = ecUpperCase
    CorDeFoco = clBlack
  end
  object EdPsqNomCve: TdxColorEdit
    Left = 273
    Top = 61
    Width = 512
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 5
    CharCase = ecUpperCase
    MaxLength = 0
    CorDeFoco = clBlack
    StoredValues = 2
  end
  object PedRes: TwwQuery
    Active = True
    CachedUpdates = True
    DatabaseName = 'ISade'
    SQL.Strings = (
      'Select CodVen,'
      '           ApeVen as NomVen,'
      
        '          (Select Round(Sum(TotRes),2) From PedRes Where CodEmp ' +
        '= 1'
      
        '                                                    and DteRes >' +
        '= '#39'07/01/2003'#39
      
        '                                                    and DteRes <' +
        '= '#39'07/31/2003'#39
      
        '                                                    and CodVen =' +
        ' FinVen.CodVen'
      
        '                                                    and ( SitRes' +
        ' = '#39'Faturado'#39' or SitRes = '#39'Faturado com Saldo nao Atendido'#39' )) a' +
        's TotPed,'
      
        '          (Select Round(Sum(TotLib),2) From PedLib Where CodEmp ' +
        '= 1'
      
        '                                                                ' +
        '                   and DteRes >= '#39'07/01/2003'#39
      
        '                                                                ' +
        '                   and DteRes <= '#39'07/31/2003'#39
      
        '                                                                ' +
        '                   and SitLib = '#39'Faturado'#39
      
        '                                                                ' +
        '                   and Exists( Select * From PedRes Where CodEmp' +
        ' = PedLib.CodEmp'
      
        '                                                                ' +
        '                                                                ' +
        '             and DteRes = PedLib.DteRes'
      
        '                                                                ' +
        '                                                                ' +
        '             and NumRes = PedLib.NumRes'
      
        '                                                                ' +
        '                                                                ' +
        '             and CodVen = FinVen.CodVen)) as TotLib,'
      '       (Select Round(Sum(TotCom),2) From PedLib Where CodEmp = 1'
      
        '                                                  and DteRes >= ' +
        #39'07/01/2003'#39
      
        '                                                  and DteRes <= ' +
        #39'07/31/2003'#39
      
        '                                                  and SitLib = '#39 +
        'Faturado'#39
      
        '                                                  and Exists( Se' +
        'lect * From PedRes Where CodEmp = PedLib.CodEmp'
      
        '                                                                ' +
        '                     and DteRes = PedLib.DteRes'
      
        '                                                                ' +
        '                     and NumRes = PedLib.NumRes'
      
        '                                                                ' +
        '                     and CodVen = FinVen.CodVen)) as Credito,'
      
        '       (Select Count(*) From FinCli Where CodVen = FinVen.CodVen' +
        ') as QtdCli,'
      '       (Select Count(*) From FinCli Where CodVen = FinVen.CodVen'
      
        '                                      and Exists( Select * From ' +
        'PedRes Where CodEmp = 1'
      
        '                                                                ' +
        '         and DteRes >= '#39'07/01/2003'#39
      
        '                                                                ' +
        '         and DteRes <= '#39'07/31/2003'#39
      
        '                                                                ' +
        '         and CodVen = FinCli.CodVen'
      
        '                                                                ' +
        '         and CodCli = FinCli.CodCli'
      
        '                                                                ' +
        '         and ( SitRes = '#39'Faturado'#39' or SitRes = '#39'Faturado com Sal' +
        'do nao Atendido'#39' ))) as QtdCmp,'
      '       (Select Count(*) From PedRes Where CodEmp = 1'
      '                                      and DteRes >= '#39'07/01/2003'#39
      '                                      and DteRes <= '#39'07/31/2003'#39
      '                                      and CodVen = FinVen.CodVen'
      
        '                                      and ( SitRes = '#39'Faturado'#39' ' +
        'or SitRes = '#39'Faturado com Saldo nao Atendido'#39' )) as QtdPed'
      'From FinVen'
      'Where CodVen = :CodVen')
    ValidateWithMask = True
    Left = 264
    Top = 261
    ParamData = <
      item
        DataType = ftInteger
        Name = 'CodVen'
        ParamType = ptInput
      end>
    object PedResCODVEN: TIntegerField
      Alignment = taLeftJustify
      DisplayLabel = 'Código'
      FieldName = 'CODVEN'
    end
    object PedResNOMVEN: TStringField
      FieldName = 'NOMVEN'
      FixedChar = True
      Size = 40
    end
    object PedResTOTPED: TFloatField
      DisplayLabel = 'Total Pedidos'
      FieldName = 'TOTPED'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object PedResQTDPED: TIntegerField
      DisplayLabel = 'Qtd Pedidos'
      FieldName = 'QTDPED'
      DisplayFormat = '######0'
    end
    object PedResTOTLIB: TFloatField
      DisplayLabel = 'Liberado'
      FieldName = 'TOTLIB'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object PedResCREDITO: TFloatField
      DisplayLabel = 'Creditos'
      FieldName = 'CREDITO'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object PedResQTDCLI: TIntegerField
      DisplayLabel = 'Qtd. Clientes'
      FieldName = 'QTDCLI'
      DisplayFormat = '######0'
    end
    object PedResQTDCMP: TIntegerField
      DisplayLabel = 'Clientes Ativos'
      FieldName = 'QTDCMP'
      DisplayFormat = '######0'
    end
  end
  object DsRes: TwwDataSource
    DataSet = PedRes
    Left = 292
    Top = 261
  end
  object quSql: TwwQuery
    DatabaseName = 'ISade'
    UniDirectional = True
    ValidateWithMask = True
    Left = 264
    Top = 289
  end
  object SaveDialog: TSaveDialog
    FileName = 'ExpGrid'
    Options = [ofOverwritePrompt, ofHideReadOnly]
    Left = 292
    Top = 289
  end
  object dxComponentPrinter: TdxComponentPrinter
    CurrentLink = dxComponentPrinterLink1
    DateFormat = 1
    PreviewOptions.Caption = 'Vizualizando Relatorio'
    PreviewOptions.RegistryPath = 'Software\Developer Express\PrintingSystem\DesignTime'
    TimeFormat = 1
    Left = 292
    Top = 317
    object dxComponentPrinterLink1: TdxDBGridReportLink
      Active = True
      Component = grRes
      DateTime = 39562.736278206
      DesignerHelpContext = 0
      PrinterPage.DMPaper = 9
      PrinterPage.Footer = 6350
      PrinterPage.Header = 6350
      PrinterPage.Margins.Bottom = 12700
      PrinterPage.Margins.Left = 10600
      PrinterPage.Margins.Right = 12700
      PrinterPage.Margins.Top = 36000
      PrinterPage.PageFooter.Font.Charset = DEFAULT_CHARSET
      PrinterPage.PageFooter.Font.Color = clWindowText
      PrinterPage.PageFooter.Font.Height = -11
      PrinterPage.PageFooter.Font.Name = 'Tahoma'
      PrinterPage.PageFooter.Font.Style = []
      PrinterPage.PageFooter.LeftTitle.Strings = (
        'Emerion Consultoria e Sistemas de Informatica Ltda')
      PrinterPage.PageFooter.RightTitle.Strings = (
        'Pagina No. [Page #]')
      PrinterPage.PageHeader.Background.Brush.Color = clBlack
      PrinterPage.PageHeader.Background.PictureMode = ppmTile
      PrinterPage.PageHeader.CenterTextAlignY = taTop
      PrinterPage.PageHeader.Font.Charset = DEFAULT_CHARSET
      PrinterPage.PageHeader.Font.Color = clWindowText
      PrinterPage.PageHeader.Font.Height = -11
      PrinterPage.PageHeader.Font.Name = 'Tahoma'
      PrinterPage.PageHeader.Font.Style = []
      PrinterPage.PageHeader.LeftTextAlignY = taTop
      PrinterPage.PageHeader.LeftTitle.Strings = (
        'VMP PAPEIS'
        'VMP PAPEIS PARA EMBALAGENS LTDA'
        
          'RUA TEIXEIRA SOARES - JD DAS ACACIAS - GUARULHOS - SP CEP 03701-' +
          '010'
        ''
        'Empresa :'
        'Vendedor :'
        'Emitidos Em :')
      PrinterPage.PageHeader.RightTextAlignY = taTop
      PrinterPage.PageHeader.RightTitle.Strings = (
        '[Date Printed][Time Printed]')
      PrinterPage.PageSize.X = 210000
      PrinterPage.PageSize.Y = 297000
      PrinterPage.PaperSource = 4
      PrinterPage.ScaleFactor = 79
      PrinterPage._dxMeasurementUnits_ = 0
      PrinterPage._dxLastMU_ = 2
      ReportTitle.Font.Charset = ANSI_CHARSET
      ReportTitle.Font.Color = clWindowText
      ReportTitle.Font.Height = -15
      ReportTitle.Font.Name = 'Tahoma'
      ReportTitle.Font.Style = [fsBold]
      ReportTitle.Text = #13#10'Comercial - Fechamento de Comissões (Sintetico)'#13#10#13#10
      BandColor = clWhite
      BandFont.Charset = DEFAULT_CHARSET
      BandFont.Color = clWindowText
      BandFont.Height = -11
      BandFont.Name = 'Tahoma'
      BandFont.Style = [fsBold]
      EvenFont.Charset = DEFAULT_CHARSET
      EvenFont.Color = clWindowText
      EvenFont.Height = -11
      EvenFont.Name = 'Times New Roman'
      EvenFont.Style = []
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Times New Roman'
      Font.Style = []
      FooterFont.Charset = DEFAULT_CHARSET
      FooterFont.Color = clNavy
      FooterFont.Height = -13
      FooterFont.Name = 'Times New Roman'
      FooterFont.Style = [fsBold]
      GridLineColor = clWhite
      GroupNodeFont.Charset = DEFAULT_CHARSET
      GroupNodeFont.Color = clNavy
      GroupNodeFont.Height = -13
      GroupNodeFont.Name = 'Times New Roman'
      GroupNodeFont.Style = [fsBold]
      GroupNodeColor = clWhite
      HeaderFont.Charset = ANSI_CHARSET
      HeaderFont.Color = clNavy
      HeaderFont.Height = -13
      HeaderFont.Name = 'Times New Roman'
      HeaderFont.Style = [fsBold]
      OddFont.Charset = DEFAULT_CHARSET
      OddFont.Color = clWindowText
      OddFont.Height = -11
      OddFont.Name = 'Times New Roman'
      OddFont.Style = []
      Options = [tlpoHeaders, tlpoFooters, tlpoRowFooters, tlpoFlatCheckMarks]
      PreviewFont.Charset = DEFAULT_CHARSET
      PreviewFont.Color = clWindowText
      PreviewFont.Height = -11
      PreviewFont.Name = 'Times New Roman'
      PreviewFont.Style = []
      RowFooterColor = clWhite
      RowFooterFont.Charset = DEFAULT_CHARSET
      RowFooterFont.Color = clRed
      RowFooterFont.Height = -13
      RowFooterFont.Name = 'Times New Roman'
      RowFooterFont.Style = [fsBold]
    end
  end
  object PopUpMenu: TPopupMenu
    Images = ImageList1
    OwnerDraw = True
    Left = 264
    Top = 317
    object Exportarpara1: TMenuItem
      Caption = 'Exportar para ...           '
      ImageIndex = 4
      object HTML: TMenuItem
        Caption = 'HTML ...                         '
        Hint = 'Exportar Relatorio no Formato HTML'
        ImageIndex = 1
        OnClick = HTMLClick
      end
      object Excel: TMenuItem
        Caption = 'EXCEL ...'
        Hint = 'Exportar Relatorio no Formato EXCEL'
        ImageIndex = 3
        OnClick = ExcelClick
      end
      object XML: TMenuItem
        Caption = 'XML && XSL ...'
        Hint = 'Exportar Relatorio no Formato XML '
        ImageIndex = 2
        OnClick = XMLClick
      end
    end
  end
  object ImageList1: TImageList
    Left = 264
    Top = 345
    Bitmap = {
      
      000000000000}
  end
end
