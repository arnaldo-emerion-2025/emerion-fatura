...
begin
  ...
  sBase := ' Select PedLb2.CodEmp,'+
           ...
           '        TextoOcor(IntStrZeros(PedLib.CodCli,5),'+ QuotedStr('-') +',FinCli.ApeCli,'+ QuotedStr('') +','+ QuotedStr('') +','+ QuotedStr('') +','+ QuotedStr('') +') as Cliente,'+
           '        TextoOcor(IntStrZeros(PedLib.CodVen,3),'+ QuotedStr('-') +',FinVen.ApeVen,'+ QuotedStr('') +','+ QuotedStr('') +','+ QuotedStr('') +','+ QuotedStr('') +') as Vendedor,'+
           '        PedLb2.CodClp || _UNICODE_FSS '+ QuotedStr('-') +' || PedLb2.CodGru || _UNICODE_FSS '+ QuotedStr('.') +' || PedLb2.CodSub || _UNICODE_FSS '+ QuotedStr('.') +' || PedLb2.CodPro as CodIte'+
...
begin
  ...
  if Trim( EdCodEmp.Text ) <> '' then sFiltro := sFiltro + ' and PedLib.CodEmp = '+ QuotedStr(EdCodEmp.Text);
