...
begin
  ...
  with quSQL,SQL do begin
       ...
       Close;
       Text := ' Insert Into PedR15(SeqArq,CodEmp,DteRes,NumRes,SeqLib,NroNfs,DteFat,CodCli,ApeCli,CodVen,ApeVen,TotLiq,TotBrt,CstPrt,CstPfi,CstVme,DscPer,DscPrt,DifDsc,MarPre,MarPed,LuCroL,LuCroP,TotNat,TotPer)'+
               ' Select '''+ SeqArq +''','+
               '        PedLb2.CodEmp,'+
               '        PedLb2.DteRes,'+
               '        PedLb2.NumRes,'+
               ...
               '        Round(Sum(PedLb2.SldLb2 * ( PedLb2.VlqLb2 - PedLb2.VdrLb2 )),4) as TotNat,'''+ '0' +''''+
               ' From PedLb2,PedLib,FinVen,FinCli'+
               ...
               '   and PedLib.CodCli = FinCli.CodCli'+ sFiltro +
               ' Group by PedLb2.CodEmp,'+
               '          PedLb2.DteRes,'+
               '          PedLb2.NumRes,'+
               '          PedLb2.SeqLib,'+
               '          PedLib.NroNfs,'+
  ...
  with PedR15,SQL do begin
       ...
       Text := ' Select CodEmp,'+
               ...
               '        CstVme,'+
               '        TotPer,'+
               '        TextoOcor(IntStrZeros(CodCli,5),'''+ '-' +''',ApeCli,'''+ '' +''','''+ '' +''','''+ '' +''','''+ '' +''') as Cliente,'+
               '        TextoOcor(IntStrZeros(CodVen,3),'''+ '-' +''',ApeVen,'''+ '' +''','''+ '' +''','''+ '' +''','''+ '' +''') as Vendedor'+
               ' From PedR15 LEFT JOIN FinVen ON (PedR15.CodVen = FinVen.CodVen)'+
               ' Where PedR15.SeqArq = '''+ SeqArq +'''';
       ...
       Text := Text + ' Order by PedR15.NumRes';
