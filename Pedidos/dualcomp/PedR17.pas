...
procedure TfmPedR17.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
...
procedure TfmPedR17.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR17.bSelecionarClick(Sender: TObject);
var
sFiltro : string;
...
begin
  ...
  if Trim( EdPsqCodCli.Text ) <> '' then sFiltro := sFiltro + ' and PedLib.CodCli = '''+ EdPsqCodCli.Text +'''';
  ...
  if sTotPed > 0 then begin
     ...
     with quSQL,SQL do begin
          ...
          Text := ' Insert Into PedR17'+
                  ...
                  '        Round(Sum(PedLb2.SldLb2 * ( PedLb2.VlqLb2 - PedLb2.VdrLb2 )),4) as TotNat,'''+ '0' +''','''+ fConvertValor(FloatToStr(sTotPed)) +''','''+ fConvertValor(FloatToStr(sTotCst)) +''','''+ '0' +''','''+ '0' +''''+
                  ' From PedLb2,PedLib,EstTip,FinVen'+
                  ' Where PedLb2.CodEmp = PedLib.CodEmp'+
                  '   and PedLb2.DteRes = PedLib.DteRes'+
                  '   and PedLb2.NumRes = PedLib.NumRes'+
                  '   and PedLb2.SeqLib = PedLib.SeqLib'+
                  '   and PedLb2.CodTip = EstTip.CodTip'+
                  '   and PedLib.CodVen = FinVen.CodVen'+ sFiltro +
                  ' Group by PedLb2.CodTip,EstTip.NomTip';
          ExecSQL;
...
procedure TfmPedR17.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  if key = 116 then bImprimir.OnClick(Sender);
end;

procedure TfmPedR17.FormShow(Sender: TObject);
begin
  inherited;
  EdPsqCodEmp.SetFocus;
end;

procedure TfmPedR17.PaintBoxPaint(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR17.bImprimirClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR17.grR17ReloadGroupList(Sender: TObject);
begin
  inherited;
  (Sender as TdxDBGrid).FullExpand;
end;

procedure TfmPedR17.HTMLClick(Sender: TObject);
begin
  inherited;
  Save('htm', 'HTML File (*.htm; *.html)|*.htm', 'Curva ABC por Tipo de Produtos.htm', grR17.SaveToHTML);
end;

procedure TfmPedR17.ExcelClick(Sender: TObject);
begin
  inherited;
  Save('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', 'Curva ABC por Tipo de Produtos.xls', grR17.SaveToXLS);
end;

procedure TfmPedR17.XMLClick(Sender: TObject);
begin
  inherited;
  Save('xml', 'XML File (*.xml)|*.xml', 'Curva ABC por Tipo de Produtos.xml', grR17.SaveToXML);
end;

procedure TfmPedR17.EdPsqCodEmpKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  if not ( key in [ '0'..'9' ] ) then key := #0;
end;

procedure TfmPedR17.BbPsqEmpClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR17.EdPsqCodEmpExit(Sender: TObject);
begin
...
procedure TfmPedR17.bPsqVenClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR17.EdPsqCodVenExit(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR17.bPsqCliClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR17.EdPsqCodCliExit(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR17.EdPsqCodGveExit(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR17.bPsqGveClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR17.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  Action := CaFree;
end;

procedure TfmPedR17.FormDestroy(Sender: TObject);
begin
  inherited;
  fmPedR17 := nil;
end;

procedure TfmPedR17.Colunas1Click(Sender: TObject);
begin
  inherited;
  ...
  try
     ...
     fmManCol.Synchronize(fmPedR17.grR17);
