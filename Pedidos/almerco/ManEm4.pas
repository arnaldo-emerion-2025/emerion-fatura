...
type
  TfmManEm4 = class(TForm)
    ...
    PedLibOB1FAT: TStringField;
    PedLibOB2FAT: TStringField;
    PedLibOB3FAT: TStringField;
    PedLibOB4FAT: TStringField;
    PedLibOB5FAT: TStringField;
    PedLibOB6FAT: TStringField;
    PedLibOB7FAT: TStringField;
    PedLibOB8FAT: TStringField;
...
procedure TfmManEm4.RLPreviewSetup1Send(Sender: TObject);
begin
  TFormSendMailOptions.CreateShowAndFree(TRLPreviewForm(Sender).Preview.Pages);
end;

procedure TfmManEm4.RLBand1BeforePrint(Sender: TObject;  var PrintIt: Boolean);
begin
...
procedure TfmManEm4.RLBand4BeforePrint(Sender: TObject; var PrintIt: Boolean);
begin
  ...
  with quSQL,SQL do begin
       ...
       with Params do begin
            ...
            Params[0].AsInteger  := fmPsqOrc.FatOrcCodEmp.Value;
            Params[1].AsDateTime := fmPsqOrc.FatOrcDteRes.Value;
            Params[2].AsInteger  := fmPsqOrc.FatOrcNumRes.Value;
            Params[3].AsInteger  := fmPsqOrc.FatOrcSeqLib.Value;
            Params[4].AsInteger  := fmPsqOrc.FatOrcSeqOrc.Value;
  ...
  while not quSQL.Eof do begin
        ...
        ppTexto1.Lines.Add(IntToStr(fmPsqOrc.FatOrcNumRes.Value));
        ppTexto2.Lines.Add(FormatDateTime('dd/mm/yyyy',quSql.FieldbyName('DtvOr3').AsDateTime));
        ppTexto3.Lines.Add(FormatFloat('###,###,##0.00',quSql.FieldbyName('VlpOr3').AsFloat));
  ...
  PraMed := PraMed / StrToFloat(IntToStr(fmPsqOrc.FatOrcQtpOrc.Value));

end;

procedure TfmManEm4.RLBand6BeforePrint(Sender: TObject; var PrintIt: Boolean);
var
CodTra : integer;
begin
...
procedure TfmManEm4.RLDetailGrid1BeforePrint(Sender: TObject; var PrintIt: Boolean);
begin
  ppCodPro.Caption := PedLb2CodGru.Value+'.'+PedLb2CodSub.Value+'.'+PedLb2CodPro.Value;
end;

procedure TfmManEm4.RLBand5BeforePrint(Sender: TObject; var PrintIt: Boolean);
begin
