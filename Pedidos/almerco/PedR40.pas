...
type
  ...
  TfmPedR40 = class(TfmPadrao)
    ...
    Colunas1: TMenuItem;
    PedR17CODCFO: TStringField;
    PedR17PRODEP: TStringField;
    grR17CODCFO: TdxDBGridColumn;
    grR17PRODEP: TdxDBGridColumn;
...
procedure TfmPedR40.Save(ADefaultExt, AFilter, AFileName: String; AMethod: TSaveMethod);
begin
...
procedure TfmPedR40.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR40.bSelecionarClick(Sender: TObject);
var
sFiltro : string;
...
begin
  ...
  with quSQL,SQL do begin
       ...
       Text := ' Select Round(Sum(PedLb2.TotLb2 - PedLb2.TotDsr),2) as TotLiq,'+
               ...
               '   and PedLib.Id_PedRes = PedRes.Id_PedRes and PedRes.CodVen = FinVen.CodVen'+ sFiltro;
       Open;
  ...
  if sTotPed > 0 then begin
     ...
     with quSQL,SQL do begin
          ...
          Close;
          Text := ' Insert Into PedR17(SeqArq,CodCfo,ProDep,CodTip,TotLiq,TotBrt,CstPrt,CstPfi,CstVme,DscPer,DscPrt,TotNat,TotFat,CstFat)'+
                  ...
                  ' Group by PedRe2.CodCfo,EstPro.ProDep,EstPro.CodTip';
          ExecSQL;
     ...
     with PedR17,SQL do begin
          ...
          Text := ' Select PedR17.CodCfo,'+
                  ...
                  '        PedR17.PrtFat,'+
                  '        PedR17.PrtCst,'+
                  '        EstTip.NomTip as Tipo'+
                  ' From PedR17 LEFT JOIN EstTip ON (PedR17.CodTip = EstTip.CodTip)'+
                  ' Where PedR17.SeqArq = '+ QuotedStr(SeqArq) +
                  ' Order by EstTip.NomTip';
          Open;
...
procedure TfmPedR40.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  if key = 116 then bImprimir.OnClick(Sender);
end;

procedure TfmPedR40.FormShow(Sender: TObject);
begin
  inherited;
  EdPsqCodEmp.SetFocus;
end;

procedure TfmPedR40.PaintBoxPaint(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR40.bImprimirClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR40.grR17ReloadGroupList(Sender: TObject);
begin
  inherited;
  (Sender as TdxDBGrid).FullExpand;
end;

procedure TfmPedR40.HTMLClick(Sender: TObject);
begin
  inherited;
  Save('htm', 'HTML File (*.htm; *.html)|*.htm', 'Curva ABC por tipo de produtos.htm', grR17.SaveToHTML);
end;

procedure TfmPedR40.ExcelClick(Sender: TObject);
begin
  inherited;
  Save('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', 'Curva ABC por tipo de produtos.xls', grR17.SaveToXLS);
end;

procedure TfmPedR40.XMLClick(Sender: TObject);
begin
  inherited;
  Save('xml', 'XML File (*.xml)|*.xml', 'Curva ABC por tipo de produtos.xml', grR17.SaveToXML);
end;

procedure TfmPedR40.EdPsqCodEmpKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  if not ( key in [ '0'..'9' ] ) then key := #0;
end;

procedure TfmPedR40.BbPsqEmpClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR40.EdPsqCodEmpExit(Sender: TObject);
begin
...
procedure TfmPedR40.bPsqVenClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR40.EdPsqCodVenExit(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR40.bPsqCliClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR40.EdPsqCodCliExit(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR40.EdPsqCodGveExit(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR40.bPsqGveClick(Sender: TObject);
begin
  inherited;
...
procedure TfmPedR40.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  Action := CaFree;
end;

procedure TfmPedR40.FormDestroy(Sender: TObject);
begin
  inherited;
  fmPedR40 := nil;
end;

procedure TfmPedR40.Colunas1Click(Sender: TObject);
begin
  inherited;
  ...
  try
     ...
     fmManCol.Synchronize(fmPedR40.grR17);
