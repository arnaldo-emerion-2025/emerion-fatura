...
begin
  ...
  if Trim( fAcesso(sNome) ) = '' then fmsgErro('Usuario n�o Possui Acesso a Op��o',Nil);
  ...
  if sNome = 'PED030210' then CriarForm(TfmPedR21, fmPedR21);
...
begin
  ...
  if sCancelar = 'F' then begin
     ...
     if sCancelar = 'N' then begin
        ...
        if GEmp_Id > 0 then
           fmManPri.sbMain.Panels[1].Text := ' Usu�rio : ' + GUsu_Nm
        else
           fmManPri.sbMain.Panels[1].Text := ' Usu�rio : ';
