...
type
  ...
  TfmCmpR14 = class(TfmPadrao)
    ...
    grLb2ULTQTS: TdxDBGridMaskColumn;
    grLb2TOTLB2: TdxDBGridMaskColumn;
    grLb2NOMCLI: TdxDBGridMaskColumn;
    PedLb2CODITE: TStringField;
    grLb2CODITE: TdxDBGridColumn;
    PedLb2DTERES: TDateTimeField;
    PedLb2NRONFS: TIntegerField;
    PedLb2DTEFAT: TDateTimeField;
    grLb2DTERES: TdxDBGridColumn;
    grLb2DTEFAT: TdxDBGridColumn;
    grLb2NRONFS: TdxDBGridColumn;
    grLb2ID_PEDRES: TdxDBGridColumn;
    PedLb2APEVEN: TStringField;
    grLb2APEVEN: TdxDBGridColumn;
    PedLb2DTESDA: TDateTimeField;
    PedLb2DTEENT: TDateTimeField;
    grLb2DTESDA: TdxDBGridColumn;
    grLb2DTEENT: TdxDBGridColumn;
    Label14: TLabel;
    EdCodVen: TdxColorEdit;
    bPsqVen: TSpeedButton;
    EdNomVen: TdxColorEdit;
    PedLb2TOTCST: TFloatField;
    grLb2TOTCST: TdxDBGridColumn;
    PedLb2FLGTAB: TStringField;
    PedLb2VCSLB2: TFloatField;
    grLb2VCSLB2: TdxDBGridColumn;
    grLb2FLGTAB: TdxDBGridColumn;
    PedLb2VLQLB2: TFloatField;
    grLb2VLQLB2: TdxDBGridColumn;
    PedLb2TOTREN: TFloatField;
    grLb2TOTREN: TdxDBGridColumn;
    Label16: TLabel;
    ...
    grLb2SITLIB: TdxDBGridColumn;
    grLb2CODCLI: TdxDBGridColumn;
    PedLb2FLGENC: TStringField;
    PedLb2MODPFA: TStringField;
    grLb2MODPFA: TdxDBGridColumn;
    PedLb2ID_PEDRES: TIntegerField;
    Colunas1: TMenuItem;
    grLb2: TdxDBGrid;
...
begin
  ...
  sFiltro := sFiltro + ' and PedRes.AtuEst = '+ QuotedStr('Sim') +' and PedLb2.FlgSep = '+ QuotedStr('Sim');
  ...
  with quSQL,SQL do begin
       ...
       Close;
       Text := ' Insert Into CmpR14(SeqArq,CodCli,CodVen,Id_PedRes,FlgTab,DteRes,NroNfs,DteFat,DteSda,DteEnt,CodClp,CodGru,CodSub,CodPro,UltQts,TotCst,TotRen,VcsLb2,TotLb2,VlqLb2,ModPfa,SitLib)'+
               ...
               '        Round(PedRe2.VlqRe2 - PedRe2.VdrRe2,4) as VlqLb2,'+
               '        PedRes.ModPfa,'+
               '        PedLib.SitLib '+
  ...
  with PedLb2,SQL do begin
       ...
       Close;
       Text := ' Select CmpR14.*,'+
               '        TextoOcor(IntStrZeros(CmpR14.CodVen,3),'+ QuotedStr('-') +',FinVen.ApeVen,'+ QuotedStr('') +','+ QuotedStr('') +','+ QuotedStr('') +','+ QuotedStr('') +') as ApeVen,'+
               '        FinCli.NomCli,'+
               '        TextoOcor(CmpR14.CodGru,'+ QuotedStr('.') +',CmpR14.CodSub,'+ QuotedStr('.') +',CmpR14.CodPro,'+ QuotedStr('-') +',EstPro.DscPro) as CodIte'+
               ' From CmpR14 LEFT JOIN EstPro ON (CmpR14.CodClp = EstPro.CodClp '+
               '                             AND  CmpR14.CodGru = EstPro.CodGru '+
               '                             AND  CmpR14.CodSub = EstPro.CodSub '+
               '                             AND  CmpR14.CodPro = EstPro.CodPro)'+
               '             LEFT JOIN FinVen ON (CmpR14.CodVen = FinVen.CodVen)'+
               '             LEFT JOIN FinCli ON (CmpR14.CodCli = FinCli.CodCli) Where CmpR14.SeqArq = '+ QuotedStr(SeqArq);

       if Trim( EdCodClp.Text ) <> '' then Text := Text + ' and CmpR14.CodClp = '+ QuotedStr(EdCodClp.Text);
...
procedure TfmCmpR14.PaintBoxPaint(Sender: TObject);
begin
  inherited;
...
begin
  inherited;
  Save('htm', 'HTML File (*.htm; *.html)|*.htm', 'Perfil de consumo (Cliente-Item).htm', grLb2.SaveToHTML);
end;

procedure TfmCmpR14.ExcelClick(Sender: TObject);
begin
  inherited;
  Save('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', 'Perfil de consumo (Cliente-Item.xls)', grLb2.SaveToXLS);
end;

procedure TfmCmpR14.XMLClick(Sender: TObject);
begin
  inherited;
  Save('xml', 'XML File (*.xml)|*.xml', 'Perfil de consumo (Cliente-Item).xml', grLb2.SaveToXML);
end;
