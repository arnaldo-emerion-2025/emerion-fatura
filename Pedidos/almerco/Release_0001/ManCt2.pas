...
uses dxDemoUtils, Bbgeral, Bbfuncao, BbMensag, ManGDB, ManCta, PsqEmp, AuxPsq,
     ...
     PsqCmp, PsqUnd, PsqUfe, PsqOcc, PsqEs2, PsqUve, PsqCf4;
...
begin
  ...
  if fmManCta.Id_PedCta > 0 then begin

     sEntr := 'S';

     PedCta.Close;
     PedCta.Params[0].AsInteger := fmManCta.Id_PedCta;
     PedCta.Open;
     ...
     if PedParFlgCot.Value = 'Sim' then begin
        ...
        if PedCt2Id_PedCt2.Value > 0 then begin
           ...
           try
              ...
              fmPsqCf4 := TfmPsqCf4.Create(Self);

              fmPsqCf4.Panel3.Caption := ' '+PedCt2CodClp.Value+ '-' +PedCt2CodGru.Value+ '.' +PedCt2CodSub.Value+ '.' +PedCt2CodPro.Value+ ' - ' +PedCt2DesCt2.Value;

              fmPsqCf4.pnQtdCot.Caption := FormatFloat('###,###,##0.0000',PedCt2QtpCt2.Value);

              fmPsqCf4.PedCt4.Close;
              fmPsqCf4.PedCt4.Params[0].AsInteger := PedCt2Id_PedCt2.Value;
              fmPsqCf4.PedCt4.Open;
...
begin
  ...
  if Finalizar = 'N' then begin
     ...
     if fMsg('Deseja Realmente Abandonar a Cotacao ?','O') then begin
        ...
        if PedCtaId_PedCta.Value = Null then PedCta.Insert;
     ...
     if PedCta.State <> dsBrowse then begin
        ...
        if Status = 'dsInsert' then begin
           ...
           with fmManCta.PedCta,SQL do begin
                ...
                Close;
                Text := fmManCta.sBase +
                        ' Where PedCta.Id_PedCta = '+ QuotedStr(IntToStr(PedCtaId_PedCta.Value));
                Open;
...
procedure TfmManCt2.EdTotMk2Exit(Sender: TObject);
var
  saida : boolean;
begin
  inherited;
...
procedure TfmManCt2.EdPacCt2Exit(Sender: TObject);
var
  saida : boolean;
begin
  inherited;
...
procedure TfmManCt2.EdVcsCt2Exit(Sender: TObject);
var
  saida : boolean;
begin
  inherited;
