...
uses Bbfuncao, ManGDB, dxDemoUtils, Bbgeral, Bbmensag, ManAt2;

{$R *.DFM}

procedure TfmPsqCf2.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TfmPsqCf2.PaintBoxPaint(Sender: TObject);
begin
  with Sender as TPaintBox do FillGrayGradientRect(PaintBox.Canvas, PaintBox.ClientRect, PaintBox.Color);
end;

procedure TfmPsqCf2.FormShow(Sender: TObject);
begin
  inherited;
  grCt4.SetFocus;
end;

procedure TfmPsqCf2.EdVluCt4Exit(Sender: TObject);
begin
  inherited;
  if PedCt4.State <> dsBrowse then begin
...
procedure TfmPsqCf2.Panel2Exit(Sender: TObject);
var
     ...
     if PedCt4.State <> dsBrowse then begin
        ...
        if PedCt4.State = dsInsert then begin
           ...
           if Trim(PedCt4CodPfo.Value) = '' then
              PedCt4CodPfo.Value := fmManAt2.PedCt2CodClp.Value+'-'+
                                    fmManAt2.PedCt2CodGru.Value+'.'+
                                    fmManAt2.PedCt2CodSub.Value+'.'+
                                    fmManAt2.PedCt2CodPro.Value;
           ...
           if PedCt4NroCt4.Value = fmManAt2.PedCt2QtiCt4.Value then
              PedCt4.Append
           else
              begin
...
procedure TfmPsqCf2.EdPraEntExit(Sender: TObject);
begin
  inherited;
  PedCt4DteEnt.Value := fmManAt2.PedCtaDteCta.Value + PedCt4PraEnt.Value;
end;

procedure TfmPsqCf2.grCt4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  Id_PedCt2,Id_PedCt4 : integer;
begin
  ...
  if key = 40 then begin // Tecla - Seta para Baixo //
     ...
     if PedCt4NroCt4.Value = fmManAt2.PedCt2QtiCt4.Value then incluir;
...
procedure TfmPsqCf2.grCt4Enter(Sender: TObject);
begin
  inherited;
...
procedure TfmPsqCf2.DsPedCt4DataChange(Sender: TObject; Field: TField);
begin
  inherited;
...
procedure TfmPsqCf2.EdCodForEnter(Sender: TObject);
begin
  inherited;
  pnTexto.Caption := 'F1-Iniciais F2-Inteligente';
end;

procedure TfmPsqCf2.EdCodForExit(Sender: TObject);
var
  saida : boolean;
begin
  inherited;
  if (Tecla <> 'ESC') and (Tecla <> 'UP') then begin
     ...
     if Tecla = 'UP' then begin
        ...
        if fmManAt2.PedCt2QtiCt4.Value > 0 then begin
...
procedure TfmPsqCf2.PedCt4NewRecord(DataSet: TDataSet);
begin
  inherited;
  ...
  PedCt4Id_PedCt2.Value := fmManAt2.PedCt2Id_PedCt2.Value;
  ...
  PedCt4DteEnt.Value := fmManAt2.PedCtaDteCta.Value;
  PedCt4NroCt4.Value := fmManAt2.PedCt2QtiCt4.Value + 1;
...
procedure TfmPsqCf2.incluir;
begin
  PedCt4.Append;
end;

procedure TfmPsqCf2.excluir;
var
     ...
     if PedCt4FlgInt.Value = 'Nao' then begin
        ...
        with PedCt4 do begin
             ...
             try
                ...
                PedCt4.Locate('Id_PedCt2;NroCt4',VarArrayOf([fmManAt2.PedCt2Id_PedCt2.Value,NroCt4]),[LoPartialKey]);
        ...
        if NroCt4 < fmManAt2.PedCt2QtiCt4.Value then
           PedCt4.Locate('Id_PedCt2;NroCt4',VarArrayOf([fmManAt2.PedCt2Id_PedCt2.Value,NroCt4]),[LoPartialKey])
        else
           begin

           if fmManAt2.PedCt2QtiCt4.Value = 0 then
              PedCt4.Append
           else
              PedCt4.Last;
...
procedure TfmPsqCf2.IncluirNovo1Click(Sender: TObject);
begin
  inherited;
  incluir;
end;

procedure TfmPsqCf2.ExcluirAmbiente1Click(Sender: TObject);
begin
  inherited;
  excluir;
end;

procedure TfmPsqCf2.FormCreate(Sender: TObject);
begin
  inherited;
  pSaida := 'Sim';
end;
