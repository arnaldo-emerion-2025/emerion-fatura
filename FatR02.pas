...
type
  ...
  TfmFatR02 = class(TfmPadrao)
    ...
    grFatTOTPIS: TdxDBGridMaskColumn;
    grFatTOTCOF: TdxDBGridMaskColumn;
    FatPedDTEFAT: TDateTimeField;
    grFatDTEFAT: TdxDBGridColumn;
    FatPedTOTFAT: TFloatField;
    grFatTOTFAT: TdxDBGridColumn;
    Label4: TLabel;
    Label9: TLabel;
    EdPsqModPfa: TdxColorPickEdit;
    FatPedHREFAT: TStringField;
    FatPedAPETRA: TStringField;
    grFatHREFAT: TdxDBGridColumn;
    grFatAPETRA: TdxDBGridColumn;
    Colunas1: TMenuItem;
    FatPedTOTFRT: TFloatField;
    grFatTOTFRT: TdxDBGridColumn;
    FatPedSITFAT: TStringField;
    grFatSITFAT: TdxDBGridColumn;
    Label8: TLabel;
    Label12: TLabel;
...
uses dxDemoUtils, Bbgeral, Bbfuncao, Bbmensag, PsqEmp, PsqCli, PsqVen, PsqPfa,
  AuxPsq, ManGDB, ManPri, ColR02;

{$R *.DFM}

procedure TfmFatR02.Save(ADefaultExt, AFilter, AFileName: string; AMethod: TSaveMethod);
begin
  with SaveDialog do
  begin
...
procedure TfmFatR02.FormCreate(Sender: TObject);
begin
  inherited;

  sBase := ' Select FatPed.NroNfs,' +
    '        FinCli.ApeCli,' +
    '        FatPed.NumRes,' +
    '        FatPed.CodVen,' +
    '        FatPed.TotFat,' +
    ...
    '        FatPed.SitFat,' +
    '        FatPed.TotSub,' +
    '        (Select FinTra.ApeTra From FinTra Where FinTra.CodTra = FatPed.TraSda) as ApeTra' +
    ' From FatPed,FinCli' +
    ' Where FatPed.CodCli = FinCli.CodCli';

end;

procedure TfmFatR02.bbPesquisaClick(Sender: TObject);
var
  imptip: string;
begin
  ...
  sFiltro := ' and ( FatPed.SitFat = ''' + 'Faturado' + ''' or FatPed.SitFat = ''' + 'Devolvido' + ''') and FatPed.TipPfa = ''' + 'Saida' + ''' and FatPed.' +
    imptip + ' = ''' + 'Sim' + '''';
  ...
  if Trim(fLimpaStr(EdPsqDteDev1.Text)) <> '' then
    sFiltro := sFiltro + ' and FatPed.DteDev >= ''' + fDateToSQL(EdPsqDteDev1.Date) + '''';
  if Trim(fLimpaStr(EdPsqDteDev2.Text)) <> '' then
    sFiltro := sFiltro + ' and FatPed.DteDev <= ''' + fDateToSQL(EdPsqDteDev2.Date) + '''';
  ...
  begin
    ...
    Open;
    Clipboard.astext := fatped.SQL.text;
  end;
...
procedure TfmFatR02.EdPsqCodEmpExit(Sender: TObject);
begin
  if Trim(EdPsqCodEmp.Text) <> '' then
  begin
...
procedure TfmFatR02.EdPsqCodCliExit(Sender: TObject);
begin
  if Trim(EdPsqCodCli.Text) <> '' then
  begin
...
procedure TfmFatR02.bPsqEmpClick(Sender: TObject);
begin
  inherited;
...
procedure TfmFatR02.EdPsqCodEmpKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if key = 112 then
  begin {F1 - Iniciais}
...
procedure TfmFatR02.EdPsqCodCliKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if key = 112 then
  begin {F1 - Iniciais}
...
procedure TfmFatR02.EdPsqCodVenExit(Sender: TObject);
begin
  if Trim(EdPsqCodVen.Text) <> '' then
  begin
...
procedure TfmFatR02.EdPsqCodVenKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if key = 112 then
  begin {F1 - Iniciais}
...
procedure TfmFatR02.bPsqCliClick(Sender: TObject);
begin
...
procedure TfmFatR02.bPsqVenClick(Sender: TObject);
begin
...
procedure TfmFatR02.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  if key = 116 then
...
procedure TfmFatR02.FormShow(Sender: TObject);
begin
  inherited;
  EdPsqUfeRes.SetFocus;
end;

procedure TfmFatR02.PaintBoxPaint(Sender: TObject);
begin
  inherited;
...
procedure TfmFatR02.EdPsqNumResKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TfmFatR02.bImprimirClick(Sender: TObject);
begin
  inherited;
  ...
  dxComponentPrinterLink1.PrinterPage.PageHeader.LeftTitle.Add('Devolvidas no Periodo de : ' + EdPsqDteDev1.Text + ' a ' + EdPsqDteDev2.Text);
...
procedure TfmFatR02.grFatReloadGroupList(Sender: TObject);
begin
  inherited;
  (Sender as TdxDBGrid).FullExpand;
end;

procedure TfmFatR02.HTMLClick(Sender: TObject);
begin
  inherited;
  Save('htm', 'HTML File (*.htm; *.html)|*.htm', 'Resumo de Notas de Saidas - Sintetico.htm', grFat.SaveToHTML);
end;

procedure TfmFatR02.ExcelClick(Sender: TObject);
begin
  inherited;
  Save('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', 'Resumo de Notas de Saidas - Sintetico.xls', grFat.SaveToXLS);
end;

procedure TfmFatR02.XMLClick(Sender: TObject);
begin
  inherited;
  Save('xml', 'XML File (*.xml)|*.xml', 'Resumo de Notas de Saidas - Sintetico.xml', grFat.SaveToXML);
end;

procedure TfmFatR02.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  Action := CaFree;
end;

procedure TfmFatR02.Colunas1Click(Sender: TObject);
begin
  inherited;

  try

    fmColR02 := TfmColR02.Create(Self);
...
procedure TfmFatR02.FormDestroy(Sender: TObject);
begin
  inherited;
  fmFatR02 := nil;
end;
