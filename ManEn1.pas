...
uses dxDemoUtils, Bbgeral, Bbfuncao, Bbmensag, ManGDB, PsqEst, ManEno,
  PsqPfa, AuxPsq, AuxIni, PsqGru, PsqSub, ManEn2;

{$R *.DFM}

procedure TfmManEn1.FormActivate(Sender: TObject);
begin
  inherited;
  if sEnc = 'S' then
  begin
    ...
    if Trim(FatDevSitDev.Value) = 'Devolvido' then
      fMsg('Operação não pode ser realizada. Devolução com lançamentos já realizados.', 'E');

    if Trim(FatDevSitDev.Value) = 'Faturando' then
      fMsg('Operação não pode ser realizada. Devolução em processo de faturamento.', 'E');
...
begin
  ...
  begin
    ...
    begin
      ...
      fmManEno.FatDv2.Close;
      fmManEno.FatDv2.Open;
...
procedure TfmManEn1.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfmManEn1.FormDestroy(Sender: TObject);
begin
  inherited;
  fmManEn1 := nil;
end;
  ...
  begin {Barra de Espaço}
    ...
    begin
      ...
      try
        ...
        FatDv2.Locate('CodEmp;DteRes;NumRes;SeqLib;SeqFat;SeqDev;SeqDv2', VarArrayOf([CodEmp, DteRes, NumRes, SeqLib, SeqFat, SeqDev, SeqDv2]),
          [LoPartialKey]);
...
procedure TfmManEn1.EdNumNfsExit(Sender: TObject);
begin
  inherited;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if Trim(quSQL.FieldbyName('SitFat').AsString) = 'Em Aberto' then
              fmsgErro('Nota fiscal informada não confirmada.', EdNumNfs)

            else if Trim(quSQL.FieldbyName('SitFat').AsString) = 'Cancelado' then
              fmsgErro('Nota fiscal informada cancelada.', EdNumNfs)

            else if Trim(quSQL.FieldbyName('SitFat').AsString) = 'Devolvido' then
              fmsgErro('Nota fiscal informada já devolvida.', EdNumNfs);
...
procedure TfmManEn1.EdNumNfsKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TfmManEn1.Panel1Exit(Sender: TObject);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      if FatDevNumNfs.Value = 0 then
        fmsgErro('Campo de preenchimento obrigatório não informado.', EdNumNfs);
...
procedure TfmManEn1.PaintBox1Paint(Sender: TObject);
begin
  inherited;
  with Sender as TPaintBox do
...
begin
  inherited;
  if fmManEno.CodEmp > 0 then
  begin

    FatDev.Close;
    FatDev.Params[0].AsInteger := fmManEno.CodEmp;
    FatDev.Params[1].AsDateTime := fmManEno.DteRes;
    FatDev.Params[2].AsInteger := fmManEno.NumRes;
    FatDev.Params[3].AsInteger := fmManEno.SeqLib;
    FatDev.Params[4].AsInteger := fmManEno.SeqFat;
    FatDev.Params[5].AsInteger := fmManEno.SeqDev;
    FatDev.Open;
...
procedure TfmManEn1.FatDevNewRecord(DataSet: TDataSet);
begin
  inherited;
...
procedure TfmManEn1.EdCodPfaExit(Sender: TObject);
var
  saida: boolean;
begin
  inherited;
...
procedure TfmManEn1.EdCodPfaKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
var
  LocPfa: string;
begin
  inherited;
...
procedure TfmManEn1.bPsqPfaClick(Sender: TObject);
var
  LocPfa: string;
begin
  inherited;
...
procedure TfmManEn1.grFatDv2KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  if key = 13 then
  begin {Tecla - ENTER}
...
procedure TfmManEn1.Panel2Exit(Sender: TObject);
var
  SeqDv2: integer;
begin
  inherited;
  ...
  begin
    ...
    begin
      ...
      FatDv2.Locate('CodEmp;DteRes;NumRes;SeqLib;SeqFat;SeqDev;SeqDv2', VarArrayOf([FatDv2CodEmp.Value, FatDv2DteRes.Value, FatDv2NumRes.Value,
        FatDv2SeqLib.Value, FatDv2SeqFat.Value, FatDv2SeqDev.Value, SeqDv2]), [LoPartialKey]);
...
procedure TfmManEn1.DsFatDv2DataChange(Sender: TObject; Field: TField);
begin
  inherited;
...
procedure TfmManEn1.grFatDv2Enter(Sender: TObject);
begin
  inherited;
  LbText.Caption := 'Enter-Informar Qtde a separar  Barra de Espaço-Sep saldo/Nao sep  F6-Sep tudo/Nao sep';
end;

procedure TfmManEn1.EdNumNfsEnter(Sender: TObject);
begin
  inherited;
  LbText.Caption := '';
end;

procedure TfmManEn1.EdCodPfaEnter(Sender: TObject);
begin
  inherited;
