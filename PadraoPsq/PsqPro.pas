...
type
  ...
  public
    {Public declarations}
    ExibeTodas,ExiCst,CliTab,SimPro,CodClp,CodGru,CodSub,CodPro,DscPro,CbaPro,NomClp,NomGru,NomSub : string;
  end;
...
procedure TfmPsqPro.grProDblClick(Sender: TObject);
begin
...
procedure TfmPsqPro.EdPesquisaChange(Sender: TObject);
begin
  EstPro.Locate('DSCPRO',EdPesquisa.Text,[LoPartialKey]);
end;

procedure TfmPsqPro.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TfmPsqPro.DsProDataChange(Sender: TObject; Field: TField);
begin
  ...
  if EstProFlgPro.Value = 'Sim' then begin
     ...
     if (LbVp1Ite.Visible) or
        (LbVp2Ite.Visible) or
        (LbVp3Ite.Visible) or
        (LbVp4Ite.Visible) or
        (LbVp5Ite.Visible) then begin
