...
procedure TfmPsqPar.grProDblClick(Sender: TObject);
begin
...
procedure TfmPsqPar.EdPesquisaChange(Sender: TObject);
begin
  EstPro.Locate('REFPRO',EdPesquisa.Text,[LoPartialKey]);
end;

procedure TfmPsqPar.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TfmPsqPar.DsProDataChange(Sender: TObject; Field: TField);
begin
  EdNomGru.Text := EstProNomGru.Value;
  EdNomSub.Text := EstProNomSub.Value;
end;

procedure TfmPsqPar.PaintBoxPaint(Sender: TObject);
begin
