...
type
  ...
  public
    {Public declarations}
    ExiCst,CliTab,SimPro,CodClp,CodGru,CodSub,CodPro,CodAnt,DscPro,CbaPro,NomClp,NomGru,NomSub : string;
  end;
...
procedure TfmPsqAnt.grProDblClick(Sender: TObject);
begin
  ...
  with quSQL,SQL do begin
       ...
       if Trim( FieldbyName('CodBar').AsString ) <> '' then
          CbaPro := Trim( FieldbyName('CodBar').AsString );
...
procedure TfmPsqAnt.EdPesquisaChange(Sender: TObject);
begin
  EstPro.Locate('DSCPRO',EdPesquisa.Text,[LoPartialKey]);
end;

procedure TfmPsqAnt.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TfmPsqAnt.DsProDataChange(Sender: TObject; Field: TField);
var
...
begin
  ...
  if (EstIte.Params[1].AsString <> EstProCodClp.Value) or
     (EstIte.Params[2].AsString <> EstProCodGru.Value) or
     (EstIte.Params[3].AsString <> EstProCodSub.Value) or
     (EstIte.Params[4].AsString <> EstProCodPro.Value) then begin
