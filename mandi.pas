...
type
   TFMMANDI = class(TfmShowPadrao)
    ...
    procedure dxDBColorEdit4KeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
   private
...
procedure TFMMANDI.FormClose(Sender: TObject; var Action: TCloseAction);
begin
   Action := CaFree;

end;

procedure TFMMANDI.FormCreate(Sender: TObject);
begin
   inherited;
   sContinuar := False;
   SQLDI.active := false;
...
procedure TFMMANDI.FormShow(Sender: TObject);
begin
   inherited;
   dxDBColorEdit1.SetFocus;
end;

procedure TFMMANDI.bContinuarClick(Sender: TObject);
begin
   inherited;
...
procedure TFMMANDI.SpeedButton4Click(Sender: TObject);
begin
   inherited;
...
procedure TFMMANDI.SpeedButton1Click(Sender: TObject);
begin
...
procedure TFMMANDI.DBGrid2DblClick(Sender: TObject);
begin
   inherited;
   Panel1.bringtofront;
end;

procedure TFMMANDI.DBGrid2KeyPress(Sender: TObject; var Key: Char);
begin
   inherited;
   if key = #13 then
      Panel1.bringtofront;
...
procedure TFMMANDI.SQLDIDETAfterPost(DataSet: TDataSet);
begin
   inherited;
   with SQLDIDET do
      begin
...
procedure TFMMANDI.SQLDIDETNewRecord(DataSet: TDataSet);
begin
   inherited;
   SQLDIDETID_DI.Value := SQLDIID_DI.Value;
   SQLDIDETNSEQADIC.Value := 1;
...
procedure TFMMANDI.dxDBColorEdit4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
