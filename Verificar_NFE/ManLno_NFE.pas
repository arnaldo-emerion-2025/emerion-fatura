...
begin
  ...
  if Trim(UpperCase(GLibAce)) = 'SIM' then begin
     ...
     for i := 0 to Screen.FormCount - 1 do begin
         ...
         if Screen.Forms[i] is TfmManNot_NFE then Found := i;
        ...
        if PedLibCodEmp.Value > 0 then begin
           ...
           if PedLibSitLib.Value = 'Concluido' then begin
              ...
              if PedLibSitLib.Value = 'Faturando' then
                 fmsgErro('Operação não Pode ser Realizada. Liberação do Pedido com Faturamento em Aberto.',Nil)
              else
                 begin
...
begin
  ...
  if Trim(UpperCase(GLibAce)) = 'SIM' then begin
     ...
     if Found >= 0 then begin
        ...
        if fMsg('Confirma Retorno do Pedido para a Liberação ?','S') then begin
