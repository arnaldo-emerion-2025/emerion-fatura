...
procedure TfmManNot_NFE.PaintBoxPaint(Sender: TObject);
begin
  inherited;
...
begin
  ...
  if sEnc = 'S' then begin
     ...
     if FatPedSitFat.Value = 'Faturado' then fMsg('Operação não Pode ser Realizada. Nota Fiscal já Emitida.','E')

     else if FatPedSitFat.Value = 'Cancelado' then fMsg('Operação não Pode ser Realizada. Nota Fiscal Cancelada.','E');
...
begin
  ...
  if finalizar = 'N' then begin
     ...
     if fMsg('Deseja Realmente Abandonar a Emissão da Nota Fiscal ?','O') then begin

        if FatPedNumRes.Value > 0 then begin

           if FatPedSitFat.Value = 'Nao Concluido' then begin
        ...
        if CodEmp > 0 then fmManLno_NFE.PedLib.Locate('CodEmp;DteRes;NumRes;SeqLib',VarArrayOf([CodEmp,DteRes,NumRes,SeqLib]), [loPartialKey]);
     ...
     if FatPedSitFat.Value <> 'Faturado' then begin
...
begin
  ...
  if FatPedDteFat.Value = 0 then fmsgErro('Campo de Preenchimento Obrigatorio não Informado',EdDteFat);

  if FatPed.State <> dsBrowse then begin
     ...
     with FatPed do begin
          ...
          try
             ...
             if FatPed.State = dsBrowse then FatPed.Edit;
     ...
     if GUfeEmp <> FatPedUfeFat.Value then begin
  ...
  if (Tecla <> 'ESC') and (Tecla <> 'UP') then begin
     ...
     if saida then begin
        ...
        if FatPe2.State <> dsBrowse then begin
           ...
           if Trim( FatPe2CodGru.Value ) <> '' then begin
           ...
           if EdCodGru.Enabled then begin
              ...
              if Trim( FatPe2CodGru.Value ) = '' then fmsgErro('Grupo Informado não Encontrado.',EdCodGru);
     ...
     if (Tecla = 'UP') then begin
        ...
        if FatPedQtiFat.Value > 0 then begin
           ...
           if FatPe2.State <> dsBrowse then
              ...
              if not FatPe2.Bof then FatPe2.Prior;
...
begin
  ...
  if key = 112 then begin {F1 - Iniciais}
     ...
     if Trim( FatPe2CodGru.Value ) <> '' then begin
        ...
        if EdCodSub.Enabled then EdCodSub.SetFocus;
...
begin
  ...
  if (Tecla <> 'ESC') and (Tecla <> 'UP') then begin
     ...
     if saida then begin
        ...
        if Trim( FatPe2CodSub.Value ) <> '' then begin
           ...
           if FatPe2.State <> dsBrowse then begin
              ...
              with quSql,SQL do begin
                   ...
                   if FieldbyName('Reg').AsInteger = 0 then fmsgErro('Grupo e Sub-Grupo Informado não Encontrado.',EdCodGru);
              ...
              if EdCodSub.Enabled then begin
                 ...
                 if Trim( FatPe2CodSub.Value ) = '' then fmsgErro('Sub-Grupo Informado não Encontrado.',EdCodSub);
...
begin
  ...
  if key = 112 then begin {F1 - Iniciais}
     ...
     try
        ...
        if Trim( FatPe2CodGru.Value ) <> '' then fmAuxIni.CodGru := FatPe2CodGru.Value;
  ...
  if key = 113 then begin {F2 - Busca Inteligente}
     ...
     try
        ...
        if Trim( FatPe2CodGru.Value ) <> '' then fmAuxPsq.CodGru := FatPe2CodGru.Value;
...
begin
  ...
  if (Tecla <> 'ESC') and (Tecla <> 'UP') then begin
     ...
     if saida then begin
        ...
        if Trim( FatPe2CodPro.Value ) <> '' then begin

           if FatPe2.State <> dsBrowse then begin

              FatPe2CodPro.Value := FNumStrZero(FatPe2CodPro.Value);

              if FatPe2.State = dsInsert then begin
                 ...
                 with quSQL,SQL do begin
                      ...
                      if quSql.FieldbyName('QtdReg').AsInteger > 0 then fmsgErro('Item Duplicado.',EdCodPro);
              ...
              if Trim( quSQL.FieldbyName('CodPro').AsString ) <> '' then begin
                 ...
                 FatPe2DesPe2.Value := Trim( FatPe2DesPe2.Value ) + fReplicate(' ',40-Length(Trim(FatPe2DesPe2.Value)))+ ' ' +quSQL.FieldbyName('RefPro').AsString;
...
begin
  ...
  if key = 112 then begin {f1 - Iniciais}
     ...
     if Trim( FatPe2CodPro.Value ) <> '' then EdQtpPe2.SetFocus;
  ...
  if key = 113 then begin {F5 - Busca Inteligente}
     ...
     try
        ...
        if Trim( FatPe2CodGru.Value ) <> '' then fmAuxPsq.CodGru := FatPe2CodGru.Value;
        if Trim( FatPe2CodSub.Value ) <> '' then fmAuxPsq.CodSub := FatPe2CodSub.Value;
  ...
  if (Tecla <> 'ESC') and (Tecla <> 'UP') then begin
     ...
     if saida then begin
        ...
        if FatPe2.State <> dsBrowse then begin
           ...
           if FatPe2QtpPe2.Value > 0 then begin
              ...
              if FatPe2.State = dsInsert then begin
                 ...
                 if TabPrc > 0 then begin
                    ...
                    if EdVluPe2.Enabled then
                       ...
                       if FatPe2VluPe2.Value = 0 then fmsgErro('Item sem Preço Únitario Informado.',EdQtpPe2);
  ...
  if (Tecla <> 'ESC') and (Tecla <> 'UP') then begin
     ...
     if saida then begin
        ...
        if FatPe2.State <> dsBrowse then begin
           ...
           if FatPe2VluPe2.Value > 0 then begin

              if FatPe2VluPe2.Value <> AntVluPe2 then begin
                 ...
                 if qticms > 1 then begin
                    ...
                    if ret = 'N' then EdVluPe2.SetFocus;
...
begin
  ...
  if FatPe2.State <> dsBrowse then begin
     ...
     if FatPe2QtpPe2.Value = 0 then fmsgErro('Campo de Preenchimento Obrigatorio não Informado.',EdQtpPe2);
...
begin
  inherited;
  if Tecla = 'UP' then fmManNot_NFE.Panel2.OnExit(Sender);
end;
...
begin
  ...
  if (EdCodGru.Focused) or (EdCodSub.Focused) or (EdCodPro.Focused) then begin
     ...
     if key = 114 then begin // F3 - Descrição //
        ...
        try
           ...
           if FatPedLanEst.Value = 'Sim' then begin

              if FatPedCodFil.Value > 0 then fmAuxIni.CodEmp := FatPedCodFil.Value;
        ...
        if Trim( FatPe2CodGru.Value ) <> '' then begin
           ...
           if Trim( FatPe2CodSub.Value ) <> '' then begin
              ...
              if Trim( FatPe2CodPro.Value ) <> '' then
                 fmPsqEst.EstPro.Locate('CodClp;CodGru;CodSub;CodPro',VarArrayOf([FatPe2CodClp.Value,FatPe2CodGru.Value,FatPe2CodSub.Value,FatPe2CodPro.Value]), [loPartialKey])
              else
  ...
  if key = 27 then begin
     ...
     if pnDscNot.Visible then begin
        ...
        if FatPed.State <> dsBrowse then
           ...
           if (EdCodGru.Enabled) or (EdCodSub.Enabled) or (EdCodPro.Enabled) or
              (EdQtpPe2.Enabled) or (EdVluPe2.Enabled) or (EdIcmPe2.Enabled) or (EdIpiPe2.Enabled) then begin
  ...
  if key = 123 then begin // F12-Finalizar Pedido //
     ...
     if FatPedTotFat.Value > 0 then begin
...
begin
  ...
  if grPe2.Focused then begin
     ...
     if FatPe2.State = dsBrowse then begin
        ...
        if key = #22 then begin {Gerenciamento de Contratos Ctrl + 'V'}
...
begin
  ...
  if FatPe2.State = dsBrowse then AntVluPe2 := FatPe2VluPe2.Value;
