...
begin
  ...
  if Finalizar = 'N' then begin
     ...
     if fMsg('Deseja realmente abandonar a operação ?','O') then begin
        ...
        if FatDevSeqDev.Value = 0 then begin
...
begin
  ...
  if key = 32 then begin {Barra de Espaço}
     ...
     if FatDv2SldDv2.Value > 0 then begin
  ...
  if key = 117 then begin {F6 - Separar Tudo/Nao Separar}
     ...
     if EdUltQtd.Enabled then begin
  ...
  if key = 123 then begin {F12-Finalizar Pedido}
     ...
     if FatDevQtiDev.Value > 0 then begin

        if FatDevTotDev.Value > 0 then begin
        
           if grFatDv2.Focused then begin
...
begin
  ...
  if (Tecla <> 'ESC') and (Tecla <> 'UP') then begin
     ...
     if FatDev.State <> dsBrowse then begin

        if FatDevNumNfs.Value > 0 then begin
           ...
           if quSql.FieldbyName('NumRes').AsInteger > 0 then begin

              if Trim(quSQL.FieldbyName('SitFat').AsString) = 'Faturado' then begin
...
begin
  ...
  if FatDev.State <> dsBrowse then begin
     ...
     if FatDev.State = dsInsert then begin
     ...
     if Status = 'dsInsert' then begin
        ...
        with fmManEno_NFE.FatDev,SQL do begin
...
begin
  inherited;
  if fmManEno_NFE.CodEmp > 0 then begin
...
begin
  ...
  if FatDv2.State <> dsBrowse then begin
     ...
     if FatDv2UltQtd.Value <= fRound(FatDv2SldDv2.Value + FatDv2QtdDv2.Value,4) then begin
...
begin
  ...
  if FatDev.State <> dsBrowse then begin
     ...
     if Trim( FatDevCodPfa.Value ) <> '' then begin
