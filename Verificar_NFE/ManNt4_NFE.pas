...
type
  TfmManNt4_NFE = class(TfmShowPadrao)
    FinBan: TwwQuery;
    PaintBox: TPaintBox;
    Label10: TLabel;
...
var
  fmManNt4_NFE: TfmManNt4_NFE;

implementation

uses dxDemoUtils, BbMensag, Bbgeral, Bbfuncao, ManGDB, ManNot_NFE, PsqBan, AuxPsq,
     ManImp_NFE;

{$R *.DFM}

procedure TfmManNt4_NFE.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TfmManNt4_NFE.FormShow(Sender: TObject);
begin
  inherited;
  ...
  FatPe4.Params[2].AsInteger  := fmManNot_NFE.FatPedNumRes.Value;
  FatPe4.Params[3].AsInteger  := fmManNot_NFE.FatPedSeqLib.Value;
  FatPe4.Params[4].AsInteger  := fmManNot_NFE.FatPedSeqFat.Value;
  FatPe4.Open;

  if fmManNot_NFE.FatPedQtpDup.Value > 0 then
     grPe4.SetFocus
  else
     begin
...
procedure TfmManNt4_NFE.bRetornarClick(Sender: TObject);
begin
  close;
end;

procedure TfmManNt4_NFE.bContinuarClick(Sender: TObject);
begin

  if FatPe4.State <> dsBrowse then FatPe4.CancelUpdates;

  if fmManNot_NFE.FatPedLanDup.Value <> fmManNot_NFE.FatPedTotDup.Value then begin
...
procedure TfmManNt4_NFE.PaintBoxPaint(Sender: TObject);
begin
  with Sender as TPaintBox do
       FillGrayGradientRect(PaintBox.Canvas, PaintBox.ClientRect, PaintBox.Color);
end;

procedure TfmManNt4_NFE.EdPraPe4Exit(Sender: TObject);
begin
  if psaida = 'Nao' then begin

     if (Tecla <> 'ESC') and (Tecla <> 'UP') and (Tecla <> 'MouseUP') then begin

        FatPe4DtvPe4.Value := fmManNot_NFE.FatPedDteFat.Value + FatPe4PraPe4.Value;
        ...
        if (Tecla = 'UP') then begin
           ...
           if fmManNot_NFE.FatPedQtpDup.Value > 0 then begin
...
procedure TfmManNt4_NFE.grPe4KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
var
SeqPe4 : integer;
begin
  ...
  if key = 40 then begin {Tecla - Seta para Baixo}
     ...
     if FatPe4NroPe4.Value = fmManNot_NFE.FatPedQtpDup.Value then FatPe4.Append;
...
procedure TfmManNt4_NFE.EdCodBanKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TfmManNt4_NFE.EdPraPe4KeyPress(Sender: TObject; var Key: Char);
begin
  if not ( key in [ '0'..'9' ] ) then key := #0;
end;

procedure TfmManNt4_NFE.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  if FatPe4.State <> dsBrowse then FatPe4.CancelUpdates;
end;

procedure TfmManNt4_NFE.EdCodBanExit(Sender: TObject);
begin
...
procedure TfmManNt4_NFE.EdDtvPe4Exit(Sender: TObject);
begin
  if psaida = 'Nao' then begin

     if FatPe4.State <> dsBrowse then begin

        if FatPe4DtvPe4.Value >= fmManNot_NFE.FatPedDteFat.Value then begin

           if FatPe4DtvPe4.Value > fmManNot_NFE.FatPedDteFat.Value then
              FatPe4PraPe4.Value := StrToInt(FloatToStr(FatPe4DtvPe4.Value - fmManNot_NFE.FatPedDteFat.Value))
           else
...
procedure TfmManNt4_NFE.Panel1Exit(Sender: TObject);
var
SeqPe4 : integer;
begin
  if psaida = 'Nao' then begin
     ...
     if FatPe4.State <> dsBrowse then begin
        ...
        if FatPe4DtvPe4.Value < fmManNot_NFE.FatPedDteFat.Value then fmsgErro('Data de Vencimento Informada Inferior a Data de Faturamento.',EdDtvPe4);
        ...
        if FatPe4.State = dsInsert then begin
           ...
           FatPe4.Locate('CodEmp;DteRes;NumRes;SeqLib;SeqFat;SeqPe4',VarArrayOf([fmManNot_NFE.FatPedCodEmp.Value,fmManNot_NFE.FatPedDteRes.Value,fmManNot_NFE.FatPedNumRes.Value,fmManNot_NFE.FatPedSeqLib.Value,fmManNot_NFE.FatPedSeqFat.Value,SeqPe4]),[LoPartialKey]);
...
procedure TfmManNt4_NFE.FatPe4NewRecord(DataSet: TDataSet);
begin
  ...
  FatPe4DtvPe4.Value := fmManNot_NFE.FatPedDteFat.Value;
  FatPe4CodEmp.Value := fmManNot_NFE.FatPedCodEmp.Value;
  FatPe4DteRes.Value := fmManNot_NFE.FatPedDteRes.Value;
  FatPe4NumRes.Value := fmManNot_NFE.FatPedNumRes.Value;
  FatPe4SeqLib.Value := fmManNot_NFE.FatPedSeqLib.Value;
  FatPe4SeqFat.Value := fmManNot_NFE.FatPedSeqFat.Value;
  FatPe4SeqPe4.Value := fmManNot_NFE.FatPedStpDup.Value + 1;
  FatPe4NroPe4.Value := fmManNot_NFE.FatPedQtpDup.Value + 1;
...
procedure TfmManNt4_NFE.FormCreate(Sender: TObject);
begin
  inherited;
  psaida := 'Nao';
end;
