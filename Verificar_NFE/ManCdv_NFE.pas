...
//não sei o que é isso ROLANDO
//  fmManEno_NFE.FatDevObsCft.Value := EdObsCft.Text;


  if Trim( fmManEno_NFE.FatDevObsCft.Value ) <> '' then begin

     sText := 'Deseja Realmente Cancelar a Nota Fiscal ? ';

     if fmManEno_NFE.FatDevAtuEst.Value = 'Sim' then sEstoque := ' . Saida do Estoque dos Itens Devolvidos;';

     if fmManEno_NFE.FatDevIntFin.Value = 'Sim' then sReceber := ' . Cancelamento dos Titulos a Receber Marcados como Devolvidos.';

     if fmManEno_NFE.FatDevIntFin.Value = 'Sim' then sReceber := ' . Cancelamento de Creditos Gerados ao Cliente.';
     ...
     if fMsg(sText,'O') then begin
        ...
        with fmManEno_NFE.FatDev do begin
             ...
             try
                ...
                if fmManEno_NFE.FatDev.State = dsBrowse then fmManEno_NFE.FatDev.Edit;
...
begin
  inherited;
  if fmManEno_NFE.FatDev.State <> dsBrowse then fmManEno_NFE.FatDev.CancelUpdates;
end;
