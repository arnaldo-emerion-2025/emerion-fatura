...
type
  TfmManGr1_NFE = class(TfmPadrao)
    ...
    FatGerTOTDESONERADO: TFloatField;
    FatGerTOT_FCPUFDEST: TFloatField;
    FatGerTOT_ICMSUFDEST: TFloatField;
    FatGerTOT_ICMSUFREMET: TFloatField;
...
begin
  ...
  sBase := ' Select FatGer.CodEmp,' +
    ...
    ' FatGer.TrbCof,' +
    ' FatGer.PerCof,' +
    ' FatGer.TOT_FCPUFDEST,FatGer.TOT_ICMSUFDEST,FatGer.TOT_ICMSUFREMET,'+
    ' FatGer.TotIt1,' +
    ' FatGer.TotDsr,' +
    ' FatGer.TotFrt,' +
    ' FatGer.TotSeg,' +
    ...
    ' FatGer.ProtDPEC, ' +
    ' FatGer.INDIC_CF,' +
    ' FatGer.INDIC_PRESENCA,' +
    ' FatGer.TOTDESONERADO,' +
    ' FatGer.CODCF1,' +
    ' FatGer.TOTIBPT ' +
    ' From FatGer ' +
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if fCaracIgual(fLimpaStr(FatGe2ClsIpi.Value)) then
          fmsgErro('Conteudo invalido.', EdClsIpi);
...
var
  ...
  msgJust: string;
  dtCont: TDatetime;
  aliqPis, aliqCofins: string; // Al�quota de PIS e COFINS do �tem
  idEstrangeiro: string; //3.1
  INDIC_IE: string; //3.1
  INSC_MUNICIPAL: string; //3.1
  idDest, TOTIBPT, TOTDESONERADO, CodDesoneracao: string; //3.1
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    idEstrangeiro := trim(FieldbyName('INDIC_ESTRANGEIRO').AsString); //3.1
                    INDIC_IE := copy(FieldByName('INDIC_IE').AsString, 1, 1); //3.1
                    ...
                    try
                      ...
                      begin
                        ...
                        begin
                          ...
                          Open;
                          if not (fatger.State in [dsedit]) then
                            fatger.edit;
                        ...
                        if Trim(FatGerModPfa.AsString) = 'Complemento' then
                          ...
                          Finalidade := '3'
                        else if Trim(FatGerModPfa.AsString) = 'Devolucoes' then
                          finalidade := '4'
                        else
                          Finalidade := '3'; //4.0
                        ...
                        //Identifica destino
                        if not (strtoint(FatGerCODCF1.AsString[1]) in [3, 7]) then
                        begin
                          if strtoint(FatGerCODCF1.AsString[1]) in [1, 5] then
                            idDest := '01'
                          else
                            idDest := '02';
                        end
                        ...
                        begin
                          ...
                          Writeln(ArqEnv, 'EM0202' + // Uso interno do sistema
                            ...
                            Finalidade + // Finalidade de emiss�o da NF-e
                        ...
                        quSQL.Active := False;
                        quSQL.Sql.Text := 'select ID_FATGER, NFE_REF, CODUF, CNPJ, MODELO, SERIE, NRONFS, ' +
                          'MODELO_ECF, NUM_ECF, NUM_COO, TIPO_IMPRESSAO_ECF from FATGER_REF WHERE ID_FATGER = ' +
                          FatGerNUMGER.Asstring;
                        quSQL.Active := true;
                        ...
                        begin
                          ...
                          begin
                            ...
                            Writeln(ArqEnv, 'EM1202' + //Uso interno
                              ...
                              quSQL.fieldByname('NFE_REF').AsString + // Chave NFe refer�ncia  44
                              replicate('0', 2 - length(quSQL.fieldByname('CODUF').AsString)) + //
                              quSQL.fieldByname('CODUF').AsString + // C�digo UF               02
                              ...
                              copy(quSQL.fieldByname('NFE_REF').AsString, 3, 4) + // M�s ano   04
                              replicate('0', 3 - length(quSQL.fieldByname('SERIE').AsString)) + //
                              quSQL.fieldByname('SERIE').AsString + // C�digo SERIE            03
                              replicate('0', 14 - length(quSQL.fieldByname('CNPJ').AsString)) + //
                              quSQL.fieldByname('CNPJ').AsString + // C�digo CNPJ              14
                              replicate('0', 9 - length(quSQL.fieldByname('NRONFS').AsString)) + //
                              quSQL.fieldByname('NRONFS').AsString + // NRONFS               09

                              quSQL.fieldByname('MODELO_ECF').AsString + // Modelo Doc Fiscal 02
                              quSQL.fieldByname('NUM_ECF').AsString + // Nro ECF           03
                        ...
                        UfeCli := fLimpaAcentos(FatGerUffCli.Value);
                        ...
                        INSC_MUNICIPAL := copy(Trim(INSC_MUNICIPAL), 1, 15) + fReplicate(' ', 15 - Length(copy(Trim(INSC_MUNICIPAL), 1, 15)));
                        ...
                        begin
                          if Trim(FatGerCgeCli.Value) <> '' then
                            CgcCli := copy(Trim(FatGerCgeCli.AsString), 1, 14) + fReplicate(' ', 14 - Length(copy(Trim(FatGerCgeCli.AsString), 1, 14)));
                          if FatGerTenCli.Value <> '' then
                            EndCli := Trim(FatGerTenCli.AsString) + ' ' + FatGerEndCli.AsString
                          else
                            EndCli := FatGerEndCli.AsString;
                          EndCli := fLimpaAcentos(EndCli);
                          NumCli := fLimpaAcentos(FatGerNumCli.AsString);
                          RefCli := fLimpaAcentos(FatGerRefCli.AsString);
                          BaiCli := fLimpaAcentos(FatGerBaiCli.AsString);
                          UfeCli := fLimpaAcentos(FatGerUfeCli.AsString);
                          EndCli := copy(Trim(EndCli), 1, 60) + fReplicate(' ', 60 - Length(copy(Trim(EndCli), 1, 60)));
                          NumCli := copy(Trim(NumCli), 1, 60) + fReplicate(' ', 60 - Length(copy(Trim(NumCli), 1, 60)));
                          RefCli := copy(Trim(RefCli), 1, 60) + fReplicate(' ', 60 - Length(copy(Trim(RefCli), 1, 60)));
                          BaiCli := copy(Trim(BaiCli), 1, 60) + fReplicate(' ', 60 - Length(copy(Trim(BaiCli), 1, 60)));
                          UfeCli := copy(Trim(UfeCli), 1, 02) + fReplicate(' ', 02 - Length(copy(Trim(UfeCli), 1, 02)));
                          CepCli := FatGerCepCli.AsString;
                          CepCli := fReplicate('0', 08 - Length(copy(Trim(CepCli), 1, 08))) + copy(Trim(CepCli), 1, 08);
                          ...
                          if (trim(FatGerCepCli.AsString) <> '') then
                          begin

                            if FatGerTenCli.AsString <> '' then
                              EndCli := Trim(FatGerTenCli.AsString) + ' ' + FatGerEndCli.AsString
                            else
                        ...
                        begin
                          ...
                          ' FatGe2.CodClp,' +
                            ...
                            ' FatGe2.VLRDESPATU, ' +
                            ' FatGe2.TOTIBPT,' +
                            ' FatGe2.TOTDESONERADO,' + //3.1
                          ' FatGe2.CODDESONERACAO,' + //3.1
                          ' FatGe2.BAS_UFDEST,' +
                            ' FatGe2.ALIQ_FCPUFDEST,' +
                            ' FatGe2.ALIQ_ICMSUFDEST,' +
                            ' FatGe2.ALIQ_ICMSINTER,' +
                            ' FatGe2.ALIQ_ICMSINTERPART,' +
                            ' FatGe2.TOT_FCPUFDEST,' +
                            ' FatGe2.TOT_ICMSUFDEST,' +
                            ' FatGe2.TOT_ICMSUFREMET,' +
                        ...
                        begin
                          ...
                          if Trim(CodPro) = '8888888888888' then
                            CodPro := 'CFOP' + fLimpaStr(quSQL.FieldbyName('CodCfo').AsString);
                          if Trim(CodPro) = '9999999999999' then
                            CodPro := 'CFOP' + fLimpaStr(quSQL.FieldbyName('CodCfo').AsString);
                          ...
                          TrbPis := Trim(quSQL.FieldbyName('TRBPIS').AsString);
                          TrbCof := Trim(quSQL.FieldbyName('TRBCOF').AsString);
                          ...
                          TotDsr := fSubstDecimal(FormatFloat('########0.00', quSQL.FieldbyName('TotDescInc').AsFloat), 15);
                          ...
                          aliqPis := fSubstDecimal(FormatFloat('##0.00', quSQL.FieldbyName('aliqPis').AsFloat), 05);
                          aliqCofins := fSubstDecimal(FormatFloat('##0.00', quSQL.FieldbyName('aliqCof').AsFloat), 05);
                        ...
                        //========================== THIAGO OBS DO ITEM
                          ...
                          begin
                            ...
                            begin
                              strAux := copy(Trim(qusql1.fieldbyname('numdi').asstring), 1, 10) +
                                fReplicate(' ', 10 - Length(copy(Trim(qusql1.fieldbyname('numdi').asstring), 1, 10))) +
                                formatdatetime('dd/mm/yyyy', qusql1.fieldbyname('datadi').asdatetime) +
                                copy(Trim(qusql1.fieldbyname('LOCALDESEMB').asstring), 1, 60) + fReplicate(' ', 60 -
                                Length(copy(Trim(qusql1.fieldbyname('LOCALDESEMB').asstring), 1, 60))) +
                                copy(Trim(qusql1.fieldbyname('UFDESEMB').asstring), 1, 2) + fReplicate(' ', 2 -
                                Length(copy(Trim(qusql1.fieldbyname('UFDESEMB').asstring), 1, 2))) +
                                formatdatetime('dd/mm/yyyy', qusql1.fieldbyname('datadesemb').asdatetime) +
                                copy(Trim(qusql1.fieldbyname('CODEXPORT').asstring), 1, 60) + fReplicate(' ', 60 -
                          ...
                          Writeln(ArqEnv, 'EM0207' + // Uso interno do sistema  1/6
                            ...
                            '3' + // Modalidade de determina��o da BC do ICMS ST 15/1
                            RedIcm + // Percential de redu��o de BC do ICMS 16/15
                          ...
                          if (FlgDifal <> 'N') then
                          begin
                            if fatGerINDIC_CF.AsString = '1' then
                            begin
                        ...
                        Writeln(ArqEnv, 'EM0210' + // Uso interno do sistema
                          fSubstDecimal(FormatFloat('###0.00', FatGerBasIc1.AsFloat), 15) + // Base de Calculo do ICMS
                          ...
                          fSubstDecimal(FormatFloat('###0.00', FatGerTotIt1.AsFloat), 15) + // Valor Total dos produtos e servi�os
                          ...
                          fSubstDecimal(FormatFloat('###0.00', FatGerTotImpII.AsFloat), 15) + // Valor Total do II
                          ...
                          fSubstDecimal(FormatFloat('###0.00', FatGerTotOutDesp.AsFloat), 15) + // Outras Despesas Acess�rias
                          fSubstDecimal(FormatFloat('###0.00', FatGerTotGe1.AsFloat), 15) + // Valor Total da NFe
                          fSubstDecimal(FormatFloat('###0.00', FatGERTOTIBPT.AsFloat), 15) + // Valor Total Tribut�rio Tranparencia
                        ...
                        if Trim(FatGerTenTra.Value) <> '' then
                          EndTra := Trim(FatGerTenTra.Value) + '. ' + FatGerEndTra.Value + ', ' + Trim(FatGerNumTra.Value)
                        else
  ...
  //Chamando o NFeEmerion
                        ...
                        begin
                          ...
                          begin
                            ...
                            repeat
                              ...
                              if pos('<xMotivo>', TLinha) > 0 then
                                FatGerRETNFE.Value := (copy(TLinha, pos('<xMotivo>', TLinha) + Length('<xMotivo>'), pos('</xMotivo>', TLinha) -
                                  (pos('<xMotivo>', TLinha) + Length('<xMotivo>'))));

                              if pos('<dhRecbto>', TLinha) > 0 then
                              begin
                                FatGerDTEPNF.value := strtodate(copy(TLinha, pos('<dhRecbto>', TLinha) + Length('<dhRecbto>') + 8, 2) + '/' + copy(TLinha,
                                  pos('<dhRecbto>', TLinha) + Length('<dhRecbto>') + 5, 2) + '/' + copy(TLinha, pos('<dhRecbto>', TLinha) +
                                  Length('<dhRecbto>'),
                                  4));
                                FatGerDTENFE.Value := strtodate(copy(TLinha, pos('<dhRecbto>', TLinha) + Length('<dhRecbto>') + 8, 2) + '/' + copy(TLinha,
                                  pos('<dhRecbto>', TLinha) + Length('<dhRecbto>') + 5, 2) + '/' + copy(TLinha, pos('<dhRecbto>', TLinha) +
                                  Length('<dhRecbto>'),
                                  4));
                              end;
                            ...
                            CloseFile(TArquivo);
                            movefile(pchar(CaminhoRetorno + '\' + VNumNota + ' - NF-e- ' + Chave + '.xml'), pchar(CaminhoRetorno + '\' + VNumNota + ' - NF-e- '
                              + chaveN + '.xml'));
                            chave := chaven;
                            FatGer.Post;
                            fatGer.ApplyUpdates; //Tenta aplicar as altera��es
                            ...
                            begin
                              ...
                              end; //if fMsg('Confirma impress�o da DANFE ?','O') then
                          ...
                          begin
                            ...
                            MoveFile(pchar(CaminhoRetorno + '\LogErro-CP' + VNumNota + '.txt'), pchar(CaminhoRetorno + '\LogErro-CP' + VNumNota + '.EP' +
                              inttostr(i)));
                          end
...
procedure TfmManGr1_NFE.EnvioServico(TipoEnvio: integer);
var
  Finalizar: string;
...
begin
  ...
  begin
    ...
    if Finalizar = 'S' then
    begin
      fmmangdb.CliSocket.Socket.SendText(GCodEmpCodUsuServ + 'FATURA||FATGER_ENVIO||' + strNumger + '||' + inttostr(TipoEnvio) + '||');
    end;
  end
  else if TipoEnvio = 4 then
  begin
    ...
    dtCont := Now;
    msgJust := Trim(FatGerJustDPEC.AsString);
    ...
    Fatger.Edit;
    FatgerFlgAtu.AsString := 'F';
    FatgerJustDPEC.AsString := msgJust;
  ...
  begin
    fmmangdb.CliSocket.Socket.SendText(GCodEmpCodUsuServ + 'FATURA||FATGER_DPECSEFAZ||' + strNumger + '||' + inttostr(TipoEnvio) + '||');
  end;
