...
uses
  ...
  dxDBColorCurrencyEdit, dxDBColorPickEdit, DBGrids, ComCtrls;
...
type
  TfmManEn1_NFE = class(TfmPadrao)
    ...
    edSeguro: TdxDBColorCurrencyEdit;
    dxDBColorCurrencyEdit34: TdxDBColorCurrencyEdit;
    edOutDesp: TdxDBColorCurrencyEdit;
    dxDBColorCurrencyEdit36: TdxDBColorCurrencyEdit;
    edDescinc: TdxDBColorCurrencyEdit;
...
begin
  ...
  begin {F6 - Separar Tudo/Nao Separar}
    ...
    if not EdUltQtd.ReadOnly then
    begin

      FatDv2.CancelUpdates;

      EdUltQtd.ReadOnly := True;
  ...
  begin {F12-Finalizar Pedido}
    ...
    begin
      ...
      begin
        ...
        if messagebox(handle, 'Deseja descontar valor da substituição no crédidto do cliente?', 'Confrimação para o crédito', MB_YESNO + MB_NOFOCUS) = ID_YES
          then
        begin
          fmmanen2_NFE.Cre_SUB := 'S';
        end
        else
        begin
          fmmanen2_NFE.Cre_SUB := 'N';
        end;
...
begin
  ...
  begin
    if FatDev.State <> dsBrowse then
    begin
      if FatDevNumNfs.AsInteger > 0 then
      begin
        ...
        begin
          ...
          try
            ...
            if bpedido <> '' then
              showmessage('Número de pedido inválido. Tente novamente.');
            sysutils.abort;
          end;
...
begin
  ...
  begin {Tecla - ENTER}
    ...
    EdUltQtd.ReadOnly := False;
    EdQtdRma.ReadOnly := False;
    EdCFOP.ReadOnly := False;
...
begin
  ...
  begin
    ...
    SeqDv2 := FatDv2SeqDv2.AsInteger;

    if FatDv2UltQtd.AsFloat <= fRound(FatDv2SldDv2.AsFloat + FatDv2QtdDv2.AsFloat, 4) then
    begin
      ...
      FatDv2.Locate('CodEmp;DteRes;NumRes;SeqLib;SeqFat;SeqDev;SeqDv2',
      VarArrayOf([FatDevCodEmp.AsInteger,
      FatDevDteRes.AsDateTime,FatDevNumRes.AsInteger, FatDevSeqLib.AsInteger,
      FatDevSeqFat.AsInteger, FatDevSeqDev.AsInteger, SeqDv2]), []);

      FatDev.Close;
      FatDev.Open;

      FatDv2.Locate('SeqDv2', SeqDv2, []);

      FatDv2.Next;

      EdUltQtd.ReadOnly := True;
      EdQtdRma.ReadOnly := True;
      EdCFOP.ReadOnly := True;
...
begin
  ...
  if FatDevFlgDif.AsString = 'Sim' then
...
begin
  ...
  StatusBar1.Panels[0].Text := 'Enter-Informar Qtde a separar  Barra de Espaço-Sep saldo/Nao sep  F6-Sep tudo/Nao sep';
end;
...
begin
  ...
  StatusBar1.Panels[0].Text := 'F1-Iniciais F2-Inteligente';
end;
...
procedure TfmManEn1_NFE.Panel3Exit(Sender: TObject);
begin
  inherited;
    ...
    begin
      ...
      try
        ...
        if edFreteCalc.CanFocus then
          edFreteCalc.SetFocus
        else
          edSeguro.SetFocus;
...
procedure TfmManEn1_NFE.Panel3Enter(Sender: TObject);
begin
  inherited;
  if FatDev.State = dsBrowse then
    FatDev.Edit;
