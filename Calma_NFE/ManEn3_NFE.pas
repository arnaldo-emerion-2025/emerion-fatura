...
begin
  ...
  try
    ...
    Showmessage('Nao foi poss�vel enviar o e-mail para o cliente.');
  end;
  IdSMTP1.Disconnect;
...
begin
  ...
  begin
    ...
    Writeln(ArqEnv, 'EM0202' + // Uso interno do sistema
      ...
      copy(FatdevSeqNFE.Value, 35, 09) + // C�digo n�merico que comp�e a chave de acesso
      ...
      fNumZeros(IntToStr(FatdevNroNfs.Value), 9) + // Numero do documento fiscal
      copy(FormatDateTime('dd/mm/yyyy', FatdevDtedev.Value), 7, 4) + '-' + // Data de emiss�o do documento fiscal
    ...
    Writeln(ArqEnv, 'EM0203' + // Uso interno do sistema
      ...
      FonEmp + // Telefone
      InsEmp + // IE
      copy(Trim(FatdevINSSUB.AsString), 1, 18) + fReplicate(' ', 18 - Length(copy(Trim(FatdevINSSUB.AsString), 1, 18))) + // IE do Substituto tribut�rio
    ...
    Writeln(ArqEnv, 'EM0210' + // Uso interno do sistema
      ...
      fSubstDecimal(FormatFloat('########0.00', FatdevTotDEV.Value), 15) + // Valor Total dos produtos e servi�os
      ...
      fSubstDecimal(FormatFloat('########0.00', FatdevTotDes.Value), 15) + // Outras Despesas Acess�rias
...
//  end;
  ...
  begin
    ...
    Fatdev.Post;
    Fatdev.ApplyUpdates; //Tenta aplicar as alteracoes
