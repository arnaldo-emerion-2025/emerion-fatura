...
begin
  ...
  if (Trim(FMmanlnRMA_NFE.CMPNFSNfePis.Value) = '') or
    (Trim(FMmanlnRMA_NFE.CMPNFSNfeCof.Value) = '') or (FMmanlnRMA_NFE.CMPNFSId_EstSip.Value = 0) then
  begin

    with FMmanlnRMA_NFE.quSQL, SQL do
    begin
    ...
    if FMmanlnRMA_NFE.CMPNFSId_EstSip.Value > 0 then
    begin
  ...
  if (Trim(FMmanlnRMA_NFE.CMPNFSTenTra.Value) <> '') or
    ...
    (Trim(FMmanlnRMA_NFE.CMPNFSCepTra.Value) <> '') then
  begin
  ...
  begin
    ...
    try
      ...
      if FMmanlnRMA_NFE.CMPNFS.State = dsBrowse then
        FMmanlnRMA_NFE.CMPNFS.Edit;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Trim(FMmanlnRMA_NFE.CMPNFSCidTra.Value) <> '' then
      begin
  ...
  if Trim(FMmanlnRMA_NFE.CMPNFSUfffor.Value) <> '' then
  begin
  ...
  if Trim(FMmanlnRMA_NFE.CMPNFSUfeFor.Value) <> '' then
  begin
  ...
  if FMmanlnRMA_NFE.CMPNFSId_FinUff.Value > 0 then
    pnId_FinUff.Caption := IntToStr(FMmanlnRMA_NFE.CMPNFSId_FinUff.Value);
  if FMmanlnRMA_NFE.CMPNFSId_FinUfe.Value > 0 then
    pnId_FinUfe.Caption := IntToStr(FMmanlnRMA_NFE.CMPNFSId_FinUfe.Value);
  ...
  if TrimLeft(TrimRight(FMmanlnRMA_NFE.CMPNFSCgeFor.Value)) <> '' then
  begin
...
begin
  inherited;
  if FMmanlnRMA_NFE.CMPNFS.State <> dsBrowse then
    FMmanlnRMA_NFE.CMPNFS.CancelUpdates;
end;

procedure TfmManNF1R_NFE.EdCgeCliEnter(Sender: TObject);
begin
  inherited;
  if FMmanlnRMA_NFE.CMPNFSCgeFor.EditMask <> '' then
    FMmanlnRMA_NFE.CMPNFSCgeFor.EditMask := '';
end;
...
begin
  inherited;
  if FMmanlnRMA_NFE.CMPNFSCodTra.Value <> CodTra_Ant then
  begin

    CodTra_Ant := FMmanlnRMA_NFE.CMPNFSCodTra.Value;

    if FMmanlnRMA_NFE.CMPNFSCodTra.Value > 0 then
    begin
...
begin
  inherited;
  if FMmanlnRMA_NFE.CMPNFSUfeTra.Value <> UfeTra_Ant then
  begin
...
begin
  inherited;
  if FMmanlnRMA_NFE.CMPNFSId_TraCie.Value <> Id_TraCie_Ant then
  begin
...
begin
  inherited;
  if FMmanlnRMA_NFE.CMPNFSId_FinCif.Value <> Id_FinCif_Ant then
  begin
...
begin
  inherited;
  if FMmanlnRMA_NFE.CMPNFSId_FinCie.Value <> Id_FinCie_Ant then
  begin
...
begin
  inherited;
  if FMmanlnRMA_NFE.CMPNFSUffFor.Value <> UffCli_Ant then
  begin
...
begin
  inherited;
  if FMmanlnRMA_NFE.CMPNFSUfeFor.Value <> UfeCli_Ant then
  begin
