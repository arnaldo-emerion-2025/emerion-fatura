...
var
  ...
  EndEn1: string;
  EndEn2: string;
  fmManLno_NFE: TfmManLno_NFE;

implementation

uses dxDemoUtils, Bbgeral, Bbfuncao, Bbmensag, ManGDB, ManPri, ManNot_NFE,
  ManObp_NFE;

{$R *.DFM}

procedure TfmManLno_NFE.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TfmManLno_NFE.FormShow(Sender: TObject);
begin
  inherited;
  EdPsqNumRes.SetFocus;
end;

procedure TfmManLno_NFE.PaintBoxPaint(Sender: TObject);
begin
  inherited;
...
procedure TfmManLno_NFE.EdPsqNumResKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
procedure TfmManLno_NFE.bEmitirClick(Sender: TObject);
var
  i: Integer;
  Found: Integer;
begin
  ...
  begin
    ...
    begin
      ...
      if Screen.Forms[i] is TfmManNot_NFE then
        Found := i;
    ...
    begin
      ...
      fmManNot_NFE.WindowState := wsNormal;
      fmManNot_NFE.BringToFront;

    end
    else
    begin

      if PedLibCodEmp.AsInteger > 0 then
      begin

        if PedLibSitLib.AsString = 'Concluido' then
        begin

          CodEmp := PedLibCodEmp.AsInteger;
          DteRes := PedLibDteRes.AsDateTime;
          NumRes := PedLibNumRes.AsInteger;
          SeqLib := PedLibSeqLib.AsInteger;
          ...
          begin
            ...
            PedLibPedTip.AsInteger := 1;

            PedLibSitLib.AsString := 'Faturando';
            ...
            begin
              ...
              fmManNot_NFE := TfmManNot_NFE.Create(Self);
              if fmManNot_NFE.FatPedMODFRT.isnull then
                fmManNot_NFE.FatPedMODFRT.Value := '2 - Destinatario';
              fmManNot_NFE.Show;
...
procedure TfmManLno_NFE.bEditarClick(Sender: TObject);
var
  i: Integer;
  Found: Integer;
begin
...
procedure TfmManLno_NFE.bLiberarClick(Sender: TObject);
var
  i: Integer;
  Found: Integer;
begin
...
procedure TfmManLno_NFE.bAtualizarClick(Sender: TObject);
begin
  inherited;
...
procedure TfmManLno_NFE.bImprimirClick(Sender: TObject);
var
  i: Integer;
  Found: Integer;
  QtdCol: integer;
...
procedure TfmManLno_NFE.FormClose(Sender: TObject; var Action: TCloseAction);
var
  i: Integer;
  Found: Integer;
begin
...
procedure TfmManLno_NFE.RDprint1NewPage(Sender: TObject; Pagina: Integer);
begin
  inherited;
...
procedure TfmManLno_NFE.FormDestroy(Sender: TObject);
begin
  inherited;
  fmManLno_NFE := nil;
end;

procedure TfmManLno_NFE.bObservarClick(Sender: TObject);
begin
  inherited;
  if PedLibCodEmp.Value > 0 then
  begin
    ...
    try
      ...
      fmManObp_NFE := TfmManObp_NFE.Create(Self);

      fmManObp_NFE.PedRes.Close;
      fmManObp_NFE.PedRes.Params[0].AsInteger := PedLibCodEmp.Value;
      fmManObp_NFE.PedRes.Params[1].AsDateTime := PedLibDteRes.Value;
      fmManObp_NFE.PedRes.Params[2].AsInteger := PedLibNumRes.Value;
      fmManObp_NFE.PedRes.Open;

      fmManObp_NFE.ShowModal;

    finally

      FreeAndNil(fmManObp_NFE);

    end;
  end;
end;

procedure TfmManLno_NFE.DsLibDataChange(Sender: TObject; Field: TField);
begin
  inherited;
  if PedLibCodCli.Value > 0 then
    pnCodCli.Caption := ' Código do Cliente : ' + FNumZeros(IntToStr(PedLibCodCli.AsInteger), 7)
  else
    pnCodCli.Caption := ' Código do Cliente : ';
end;

procedure TfmManLno_NFE.DsPedDataChange(Sender: TObject; Field: TField);
begin
  inherited;
