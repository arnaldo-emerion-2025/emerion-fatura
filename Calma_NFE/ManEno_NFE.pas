...
uses
  ...
  dxDBColorDateEdit, dxDBColorCurrencyEdit, clipbrd, dxColorPickEdit;
...
type
  TfmManEno_NFE = class(TfmPadrao)
    ...
    FatDv2ID_FATDEV: TIntegerField;
    EdPsqSitDev: TdxColorPickEdit;
    Label2: TLabel;
    Label3: TLabel;
...
uses dxDemoUtils, Bbgeral, Bbfuncao, Bbacesso, Bbmensag, ManGDB, PsqEmp,
  ManPri, AuxIni, AuxPsq, ManEn1_NFE, FPreview, ManCdv_NFE, ManE03;
...
begin
  ...
  sBase := ' Select cast(nfscli as integer)as NFSCLI, FatDev.*,' +
    {           '        FatDev.DteRes,'+
      '        FatDev.NumRes,'+
      '        FatDev.SeqLib,'+
      '        FatDev.SeqFat,'+
...
begin
  ...
  if Trim(EdPsqNumRes.Text) <> '' then
    sFiltro := sFiltro + ' Where FatDev.NumRes = ' + QuotedStr(EdPsqNumRes.Text);
  ...
  begin
    ...
    if pos('Where', sFiltro) > 0 then
      ...
      sFiltro := ' Where FATDEV.CodCli = ' + QuotedStr(EdPsqCodCli.Text);
  ...
  begin
    ...
    if pos('Where', sFiltro) > 0 then
      ...
      sFiltro := ' Where fatdev.CodVen = ' + QuotedStr(EdPsqCodVen.Text);
  ...
  if Trim(EdPsqSitDev.Text) <> 'Todos' then
  begin

    if pos('Where', sFiltro) > 0 then
      sFiltro := sFiltro + ' and FatDev.Sitdev = ' + QuotedStr(EdPsqSitDev.Text)
    else
      sFiltro := ' Where FatDev.SitDev = ' + QuotedStr(EdPsqSitDev.Text);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Screen.Forms[i] is TfmManEn1_NFE then
          Found := i;
      ...
      begin
        ...
        begin
          ...
          if Trim(FatDevSitDev.Value) = 'Concluido' then
          begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if FatDevCodEmp.Value > 0 then
          begin
            if (Trim(FatDevSitDev.Value) <> 'Devolvido') and (Trim(FatDevSitDev.Value) <> 'Cancelado') then
            begin
            ...
            begin
              if Trim(FatDevSitDev.Value) <> 'Cancelado' then
              begin
                try
...
begin
  ...
  begin
    ...
    case MessageBox(handle,'Deseja imprimir a Versão Simplificada da Devolução?','Qual o Tipo de Relatório?',MB_YESNO) of
       idYes:
       begin
        if FatDevCodEmp.Value > 0 then
        begin
    ...
    idNo:
       begin
          try
             ...
             //fmManE03.FatDev.Params[0].AsInteger := StrToInt(inputBox('ATENÇÃO','Informe o ID_FATDEV','327'));

             //fmManE03.FatDev.Params[0].AsInteger := FatDv2ID_FATDEV.AsInteger;
             fmManE03.FatDev.Params[0].AsInteger := FatDv2CodEmp.AsInteger;
             fmManE03.FatDev.Params[1].AsDateTime:= FatDv2DteRes.AsDateTime;
             fmManE03.FatDev.Params[2].AsInteger := FatDv2NumRes.AsInteger;
             fmManE03.FatDev.Params[3].AsInteger := FatDv2SeqLib.AsInteger;
             fmManE03.FatDev.Params[4].AsInteger := FatDv2SeqFat.AsInteger;
             fmManE03.FatDev.Params[5].AsInteger := FatDv2SeqDev.AsInteger;

             fmManE03.FatDev.Open;
...
begin
  ...
  if FatDevNfsCli.Value > 0 then
    ppNroNfs.Caption := IntToStr(FatDevNfsCli.Value)
  else
    ppNroNfs.Caption := '';
