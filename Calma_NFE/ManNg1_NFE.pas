...
begin
  ...
  begin
    ...
    if Trim(fmManNge_NFE2.FatGerUfeTra.Value) = '' then
      fmsgErro('UF de entrega � d transportadora obrigat�rio. Verifique e tente novamente.', EdUfeTra);

    if fmManNge_NFE2.FatGerId_TraUfe.Value = 0 then
      fmsgErro('C�digo da UF da transportadora � de preenchimento obrigat�rio. Verifique e tente novamente.', EdUfeTra);

    if fmManNge_NFE2.FatGerId_TraCie.Value = 0 then
      fmsgErro('Cidade da transportadora � de preenchimento obrigat�rio. Verifique e tente novamente.', EdId_TraCie);

  end;

  if fmManNge_NFE2.FatGerId_FinUff.Value = 0 then
    fmsgErro('UF de faturamente � de preenchimento. Verifique e tente novamente.', EdUffCli);

  if fmManNge_NFE2.FatGerId_FinCif.Value = 0 then
    fmsgErro('C�digo da cidade de faturamente � de preenchimento. Verifique e tente novamente.', EdId_FinCif);

  if fmManNge_NFE2.FatGerId_FinUfe.Value = 0 then
    fmsgErro('C�digo da UF de entrega � de preenchimento. Verifique e tente novamente.', EdId_FinUfe);

  if fmManNge_NFE2.FatGerId_FinCie.Value = 0 then
    fmsgErro('C�digo da cidade de entrega � de preenchimento. Verifique e tente novamente.', EdId_FinCie);
...
begin
  ...
  if fmManNge_NFE2.FatGerId_FinUff.Value > 0 then
    pnId_FinUff.Caption := IntToStr(fmManNge_NFE2.FatGerId_FinUff.Value);
  if fmManNge_NFE2.FatGerId_FinUfe.Value > 0 then
    pnId_FinUfe.Caption := IntToStr(fmManNge_NFE2.FatGerId_FinUfe.Value);
  ...
  if fmManNge_NFE2.FatGerInfBrt.Value = 0 then
    fmManNge_NFE2.FatGerInfBrt.Value := fmManNge_NFE2.FatGerTotBrt.Value;
  if fmManNge_NFE2.FatGerInfLiq.Value = 0 then
    fmManNge_NFE2.FatGerInfLiq.Value := fmManNge_NFE2.FatGerTotLiq.Value;
...
begin
  inherited;
  if fmManNge_NFE2.FatGerCgeCli.EditMask <> '' then
    fmManNge_NFE2.FatGerCgeCli.EditMask := '';
end;
