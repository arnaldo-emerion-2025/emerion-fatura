...
uses dxDemoUtils, BbMensag, Bbgeral, Bbfuncao, ManGDB, PsqBan, AuxPsq,
  ManIng_NFE, ManNge_NFE2;
...
begin
  ...
  EdTotGer.Text := FormatFloat('###,###,##0.00', fmManNge_NFE2.FatGerTotGe1.Value);
  EdLanGer.Text := FormatFloat('###,###,##0.00', fmManNge_NFE2.FatGerLanGer.Value);
  EdSldGer.Text := FormatFloat('###,###,##0.00', fmManNge_NFE2.FatGerSldGer.Value);

  FatGe3.Close;
  FatGe3.Params[0].AsInteger := fmManNge_NFE2.FatGerCodEmp.Value;
  FatGe3.Params[1].AsDateTime := fmManNge_NFE2.FatGerDteGer.Value;
  FatGe3.Params[2].AsInteger := fmManNge_NFE2.FatGerNumGer.Value;
  FatGe3.Open;

  if fmManNge_NFE2.FatGerQtpGer.Value > 0 then
    grGe3.SetFocus
  else
  begin
...
begin
  ...
  begin
    ...
    if Trim(fmManNge_NFE2.FatGerCodPfa.Value) <> '6.125' then
    begin

      if fmManNge_NFE2.FatGerLanGer.Value <> fmManNge_NFE2.FatGerTotGe1.Value then
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if FatGe3.State <> dsBrowse then
        FatGe3DtvGe3.Value := fmManNge_NFE2.FatGerDteGer.Value + FatGe3PraGe3.Value;
...
begin
  ...
  begin {Tecla - Seta para Baixo}
    ...
    if FatGe3NroGe3.Value = fmManNge_NFE2.FatGerQtpGer.Value then
      FatGe3.Append;
  ...
  begin {Tecla - DEL}
    ...
    begin
      ...
      if SeqGe3 < fmManNge_NFE2.FatGerQtpGer.Value then
...
begin
  ...
  begin
    ...
    if FatGe3DtvGe3.Value < fmManNge_NFE2.FatGerDteGer.Value then
    ...
    begin
      ...
      FatGe3.Locate('CodEmp;DteGer;NumGer;SeqGe3', VarArrayOf([fmManNge_NFE2.FatGerCodEmp.Value, fmManNge_NFE2.FatGerDteGer.Value,
        fmManNge_NFE2.FatGerNumGer.Value, SeqGe3]), [LoPartialKey]);
...
begin
  ...
  begin
    ...
    begin
      ...
      if FatGe3DtvGe3.Value > fmManNge_NFE2.FatGerDteGer.Value then
        FatGe3PraGe3.Value := StrToInt(FloatToStr(FatGe3DtvGe3.Value - fmManNge_NFE2.FatGerDteGer.Value))
      else
...
begin
  ...
  FatGe3DtvGe3.Value := fmManNge_NFE2.FatGerDteGer.Value;
  FatGe3CodEmp.Value := fmManNge_NFE2.FatGerCodEmp.Value;
  FatGe3DteGer.Value := fmManNge_NFE2.FatGerDteGer.Value;
  FatGe3NumGer.Value := fmManNge_NFE2.FatGerNumGer.Value;
  FatGe3SeqGe3.Value := fmManNge_NFE2.FatGerQtpGer.Value + 1;
  FatGe3NroGe3.Value := fmManNge_NFE2.FatGerStpGer.Value + 1;
