inherited fmEstR06: TfmEstR06
  Left = 0
  Caption = 'Relação de informações cadastrais dos itens'
  ClientHeight = 482
  ClientWidth = 786
  Position = poDesigned
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object PaintBox: TPaintBox
    Left = 0
    Top = 0
    Width = 786
    Height = 482
    Align = alClient
    OnPaint = PaintBoxPaint
  end
  object Label3: TLabel
    Left = 4
    Top = 167
    Width = 31
    Height = 13
    Caption = 'Item:'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object bPsqPro: TSpeedButton
    Left = 250
    Top = 164
    Width = 23
    Height = 20
    Hint = 'Pesquisar Grupos Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = bPsqProClick
  end
  object Label2: TLabel
    Left = 4
    Top = 225
    Width = 107
    Height = 13
    Caption = 'Itens Selecionados'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Bevel2: TBevel
    Left = 164
    Top = 232
    Width = 621
    Height = 3
    Shape = bsTopLine
    Style = bsRaised
  end
  object Label110: TLabel
    Left = 4
    Top = 29
    Width = 51
    Height = 13
    Caption = 'Categoria:'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label111: TLabel
    Left = 4
    Top = 52
    Width = 24
    Height = 13
    Caption = 'Tipo:'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label112: TLabel
    Left = 4
    Top = 75
    Width = 33
    Height = 13
    Caption = 'Marca:'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label47: TLabel
    Left = 4
    Top = 98
    Width = 75
    Height = 13
    Caption = 'Classificação:'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label48: TLabel
    Left = 4
    Top = 121
    Width = 37
    Height = 13
    Caption = 'Grupo:'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label49: TLabel
    Left = 4
    Top = 144
    Width = 57
    Height = 13
    Caption = 'Subgrupo:'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object bPsqCat: TSpeedButton
    Left = 250
    Top = 26
    Width = 23
    Height = 21
    Hint = 'Pesquisar Categorias Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = bPsqCatClick
  end
  object bPsqTip: TSpeedButton
    Left = 250
    Top = 49
    Width = 23
    Height = 21
    Hint = 'Pesquisar Tipos Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = bPsqTipClick
  end
  object PsqMrc: TSpeedButton
    Left = 250
    Top = 72
    Width = 23
    Height = 21
    Hint = 'Pesquisar Marcas Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = PsqMrcClick
  end
  object bPsqClp: TSpeedButton
    Left = 250
    Top = 95
    Width = 23
    Height = 21
    Hint = 'Pesquisar Classificações de Itens Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = bPsqClpClick
  end
  object bPsqGru: TSpeedButton
    Left = 250
    Top = 118
    Width = 23
    Height = 21
    Hint = 'Pesquisar Grupos Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = bPsqGruClick
  end
  object bPsqSub: TSpeedButton
    Left = 250
    Top = 141
    Width = 23
    Height = 21
    Hint = 'Pesquisar Sub-Grupos Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = bPsqSubClick
  end
  object Label11: TLabel
    Left = 4
    Top = 7
    Width = 45
    Height = 13
    Caption = 'Empresa:'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object BbPsqEmp: TSpeedButton
    Left = 250
    Top = 3
    Width = 23
    Height = 20
    Hint = 'Pesquisar Empresas Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = BbPsqEmpClick
  end
  object EdCodPro: TdxColorEdit
    Left = 162
    Top = 163
    Width = 87
    Hint = 'Item'
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 14
    OnExit = EdCodProExit
    CharCase = ecUpperCase
    MaxLength = 5
    CorDeFoco = clInfoBk
    StoredValues = 2
  end
  object EdNomPro: TdxColorEdit
    Left = 274
    Top = 163
    Width = 356
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 15
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object bPesquisa: TBitBtn
    Left = 474
    Top = 188
    Width = 155
    Height = 42
    Caption = '&Selecionar'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 19
    OnClick = bPesquisaClick
    Glyph.Data = {
      
      3232323232323232323232323232323232323232323232323232}
  end
  object bImprimir: TBitBtn
    Left = 631
    Top = 188
    Width = 155
    Height = 42
    Caption = '&Imprimir'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 20
    OnClick = bImprimirClick
    Glyph.Data = {
      
      D000}
  end
  object grR06: TdxDBGrid
    Left = 2
    Top = 237
    Width = 783
    Height = 245
    Bands = <
      item
        Width = 1192
      end>
    DefaultLayout = False
    HeaderPanelRowCount = 5
    KeyField = 'NOMCAT'
    ShowGroupPanel = True
    SummaryGroups = <>
    SummarySeparator = ', '
    BorderStyle = bsNone
    Color = clInfoBk
    Font.Charset = ANSI_CHARSET
    Font.Color = clNavy
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    PopupMenu = PopUpMenu
    TabOrder = 21
    BandColor = clMenu
    BandFont.Charset = ANSI_CHARSET
    BandFont.Color = clWindowText
    BandFont.Height = -11
    BandFont.Name = 'Tahoma'
    BandFont.Style = [fsBold]
    DataSource = DsR06
    Filter.Active = True
    Filter.AutoDataSetFilter = True
    Filter.Criteria = {00000000}
    GroupPanelColor = clAppWorkSpace
    GroupPanelFontColor = clInfoBk
    GroupNodeColor = clInfoBk
    HeaderFont.Charset = ANSI_CHARSET
    HeaderFont.Color = clWindowText
    HeaderFont.Height = -11
    HeaderFont.Name = 'Tahoma'
    HeaderFont.Style = [fsBold]
    HideFocusRect = True
    LookAndFeel = lfFlat
    OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEnterShowEditor, edgoImmediateEditor, edgoTabThrough, edgoVertThrough]
    OptionsCustomize = [edgoBandPanelSizing, edgoBandSizing, edgoColumnMoving, edgoColumnSizing, edgoFullSizing]
    OptionsDB = [edgoCanNavigation, edgoUseBookmarks]
    OptionsView = [edgoPreview, edgoUseBitmap]
    PreviewFont.Charset = ANSI_CHARSET
    PreviewFont.Color = clBlue
    PreviewFont.Height = -11
    PreviewFont.Name = 'Tahoma'
    PreviewFont.Style = []
    PreviewLines = 0
    RowFooterColor = clInfoBk
    RowFooterTextColor = clRed
    ShowGrid = False
    ShowHiddenInCustomizeBox = True
    ShowRowFooter = True
    OnReloadGroupList = grR06ReloadGroupList
    object grR06NOMCAT: TdxDBGridMaskColumn
      Sorted = csUp
      Visible = False
      Width = 1105
      BandIndex = 0
      RowIndex = 0
      FieldName = 'NOMCAT'
      GroupIndex = 0
    end
    object grR06CODPRO: TdxDBGridMaskColumn
      Width = 41
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CODPRO'
    end
    object grR06DSCPRO: TdxDBGridMaskColumn
      Width = 135
      BandIndex = 0
      RowIndex = 0
      FieldName = 'DSCPRO'
    end
    object grR06NOMMRC: TdxDBGridMaskColumn
      Width = 65
      BandIndex = 0
      RowIndex = 0
      FieldName = 'NOMMRC'
    end
    object grR06NOMGRU: TdxDBGridMaskColumn
      Sorted = csUp
      Visible = False
      Width = 1105
      BandIndex = 0
      RowIndex = 0
      FieldName = 'NOMGRU'
      GroupIndex = 1
    end
    object grR06NOMSUB: TdxDBGridMaskColumn
      Sorted = csUp
      Visible = False
      Width = 1105
      BandIndex = 0
      RowIndex = 0
      FieldName = 'NOMSUB'
      GroupIndex = 2
    end
    object grR06NOMTIP: TdxDBGridColumn
      Caption = 'Tipo'
      Width = 63
      BandIndex = 0
      RowIndex = 0
      FieldName = 'NOMTIP'
    end
    object grR06SIMPRO: TdxDBGridMaskColumn
      Caption = 'Similar'
      Width = 63
      BandIndex = 0
      RowIndex = 0
      FieldName = 'SIMPRO'
    end
    object grR06CODANT: TdxDBGridMaskColumn
      Caption = 'Cod Ant'
      Width = 80
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CODANT'
    end
    object grR06NUMPRO: TdxDBGridMaskColumn
      Width = 76
      BandIndex = 0
      RowIndex = 0
      FieldName = 'NUMPRO'
    end
    object grR06CODUNS: TdxDBGridMaskColumn
      Width = 41
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CODUNS'
    end
    object grR06PERICM: TdxDBGridColumn
      Caption = '% ICMS'
      HeaderAlignment = taRightJustify
      Width = 20
      BandIndex = 0
      RowIndex = 1
      FieldName = 'PERICM'
      StoredRowIndex = 1
    end
    object grR06PERIPI: TdxDBGridColumn
      Caption = '% IPI'
      HeaderAlignment = taRightJustify
      Width = 53
      BandIndex = 0
      RowIndex = 1
      FieldName = 'PERIPI'
      StoredRowIndex = 1
    end
    object grR06CLSIPI: TdxDBGridColumn
      Caption = 'Cls. Fiscal'
      Width = 41
      BandIndex = 0
      RowIndex = 1
      FieldName = 'CLSIPI'
      StoredRowIndex = 1
    end
    object grR06PERIMP: TdxDBGridColumn
      Alignment = taRightJustify
      Caption = '% II'
      HeaderAlignment = taRightJustify
      Width = 20
      BandIndex = 0
      RowIndex = 2
      FieldName = 'PERIMP'
      StoredRowIndex = 2
    end
    object grR06VB1ITE: TdxDBGridColumn
      Caption = 'Unit 1'
      HeaderAlignment = taRightJustify
      Width = 72
      BandIndex = 0
      RowIndex = 2
      FieldName = 'VB1ITE'
      StoredRowIndex = 2
    end
    object grR06DS1ITE: TdxDBGridColumn
      Caption = '% Dsc 1'
      HeaderAlignment = taRightJustify
      Width = 71
      BandIndex = 0
      RowIndex = 2
      FieldName = 'DS1ITE'
      StoredRowIndex = 2
    end
    object grR06MS1ITE: TdxDBGridColumn
      Caption = '% Mk - S 1'
      HeaderAlignment = taRightJustify
      Width = 82
      BandIndex = 0
      RowIndex = 2
      FieldName = 'MS1ITE'
      StoredRowIndex = 2
    end
    object grR06VB2ITE: TdxDBGridColumn
      Caption = 'Unit 2'
      HeaderAlignment = taRightJustify
      Width = 66
      BandIndex = 0
      RowIndex = 2
      FieldName = 'VB2ITE'
      StoredRowIndex = 2
    end
    object grR06DS2ITE: TdxDBGridColumn
      Caption = '% Dsc 2'
      HeaderAlignment = taRightJustify
      Width = 74
      BandIndex = 0
      RowIndex = 2
      FieldName = 'DS2ITE'
      StoredRowIndex = 2
    end
    object grR06MS2ITE: TdxDBGridColumn
      Alignment = taRightJustify
      Caption = '% Mk - S 2'
      HeaderAlignment = taRightJustify
      Width = 80
      BandIndex = 0
      RowIndex = 2
      FieldName = 'MS2ITE'
      StoredRowIndex = 2
    end
    object grR06VB3ITE: TdxDBGridColumn
      Caption = 'Unit 3'
      HeaderAlignment = taRightJustify
      Width = 54
      BandIndex = 0
      RowIndex = 2
      FieldName = 'VB3ITE'
      StoredRowIndex = 2
    end
    object grR06DS3ITE: TdxDBGridColumn
      Caption = '% Dsc 3'
      HeaderAlignment = taRightJustify
      Width = 66
      BandIndex = 0
      RowIndex = 2
      FieldName = 'DS3ITE'
      StoredRowIndex = 2
    end
    object grR06MS3ITE: TdxDBGridColumn
      Alignment = taRightJustify
      Caption = '% Mk - S 3'
      HeaderAlignment = taRightJustify
      Width = 80
      BandIndex = 0
      RowIndex = 2
      FieldName = 'MS3ITE'
      StoredRowIndex = 2
    end
    object grR06VB4ITE: TdxDBGridColumn
      Caption = 'Unit 4'
      HeaderAlignment = taRightJustify
      Width = 55
      BandIndex = 0
      RowIndex = 2
      FieldName = 'VB4ITE'
      StoredRowIndex = 2
    end
    object grR06DS4ITE: TdxDBGridColumn
      Caption = '% Dsc 4'
      HeaderAlignment = taRightJustify
      Width = 66
      BandIndex = 0
      RowIndex = 2
      FieldName = 'DS4ITE'
      StoredRowIndex = 2
    end
    object grR06MS4ITE: TdxDBGridColumn
      Alignment = taRightJustify
      Caption = '% Mk - S 4'
      HeaderAlignment = taRightJustify
      Width = 69
      BandIndex = 0
      RowIndex = 2
      FieldName = 'MS4ITE'
      StoredRowIndex = 2
    end
    object grR06VB5ITE: TdxDBGridColumn
      Caption = 'Unit 5'
      HeaderAlignment = taRightJustify
      Width = 56
      BandIndex = 0
      RowIndex = 2
      FieldName = 'VB5ITE'
      StoredRowIndex = 2
    end
    object grR06DS5ITE: TdxDBGridColumn
      Caption = '% Dsc 5'
      HeaderAlignment = taRightJustify
      Width = 69
      BandIndex = 0
      RowIndex = 2
      FieldName = 'DS5ITE'
      StoredRowIndex = 2
    end
    object grR06MS5ITE: TdxDBGridColumn
      Alignment = taRightJustify
      Caption = '% Mk - S 5'
      HeaderAlignment = taRightJustify
      Width = 83
      BandIndex = 0
      RowIndex = 2
      FieldName = 'MS5ITE'
      StoredRowIndex = 2
    end
    object grR06CODSIT: TdxDBGridColumn
      Caption = 'Sit Trib.'
      Width = 69
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CODSIT'
    end
    object grR06PERCOM: TdxDBGridMaskColumn
      Caption = '% Comissao'
      HeaderAlignment = taRightJustify
      Width = 94
      BandIndex = 0
      RowIndex = 2
      FieldName = 'PERCOM'
      StoredRowIndex = 2
    end
    object grR06CODITE: TdxDBGridColumn
      Caption = 'Cod Novo'
      Width = 73
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CODITE'
    end
    object grR06IDEPRO: TdxDBGridMaskColumn
      Width = 33
      BandIndex = 0
      RowIndex = 2
      FieldName = 'IDEPRO'
      StoredRowIndex = 2
    end
    object grR06QTAITE: TdxDBGridColumn
      Alignment = taRightJustify
      HeaderAlignment = taRightJustify
      Width = 80
      BandIndex = 0
      RowIndex = 1
      FieldName = 'QTAITE'
      StoredRowIndex = 1
    end
    object grR06DULCMP: TdxDBGridColumn
      Width = 100
      BandIndex = 0
      RowIndex = 1
      FieldName = 'DULCMP'
      StoredRowIndex = 1
    end
    object grR06QTDCMP: TdxDBGridColumn
      Width = 119
      BandIndex = 0
      RowIndex = 1
      FieldName = 'QTDCMP'
      StoredRowIndex = 1
    end
    object grR06VPFITE: TdxDBGridColumn
      Caption = 'Ult Custo'
      HeaderAlignment = taRightJustify
      Width = 79
      BandIndex = 0
      RowIndex = 1
      FieldName = 'VPFITE'
      StoredRowIndex = 1
    end
    object grR06VMEITE: TdxDBGridColumn
      Caption = 'Média Compra'
      Width = 100
      BandIndex = 0
      RowIndex = 1
      FieldName = 'VMEITE'
      StoredRowIndex = 1
    end
    object grR06QTDITE: TdxDBGridColumn
      Caption = 'Est Atual'
      Width = 76
      BandIndex = 0
      RowIndex = 1
      FieldName = 'QTDITE'
      StoredRowIndex = 1
    end
    object grR06VCRITE: TdxDBGridColumn
      Caption = 'Custo Referencial'
      HeaderAlignment = taRightJustify
      Width = 117
      BandIndex = 0
      RowIndex = 1
      FieldName = 'VCRITE'
      StoredRowIndex = 1
    end
    object grR06LIQEMB: TdxDBGridColumn
      HeaderAlignment = taRightJustify
      Width = 84
      BandIndex = 0
      RowIndex = 1
      FieldName = 'LIQEMB'
      StoredRowIndex = 1
    end
    object grR06BRTEMB: TdxDBGridColumn
      HeaderAlignment = taRightJustify
      Width = 80
      BandIndex = 0
      RowIndex = 1
      FieldName = 'BRTEMB'
      StoredRowIndex = 1
    end
    object grR06QTDEMB: TdxDBGridColumn
      Alignment = taRightJustify
      Caption = 'Qtde Emb'
      HeaderAlignment = taRightJustify
      Width = 77
      BandIndex = 0
      RowIndex = 1
      FieldName = 'QTDEMB'
      StoredRowIndex = 1
    end
    object grR06CXAPRO: TdxDBGridColumn
      HeaderAlignment = taRightJustify
      Width = 86
      BandIndex = 0
      RowIndex = 1
      FieldName = 'CXAPRO'
      StoredRowIndex = 1
    end
    object grR06CUBPRO: TdxDBGridColumn
      HeaderAlignment = taRightJustify
      Width = 78
      BandIndex = 0
      RowIndex = 1
      FieldName = 'CUBPRO'
      StoredRowIndex = 1
    end
    object grR06QTMITE: TdxDBGridColumn
      Alignment = taRightJustify
      Caption = 'Estoque Minimo'
      HeaderAlignment = taRightJustify
      Width = 84
      BandIndex = 0
      RowIndex = 0
      FieldName = 'QTMITE'
    end
    object grR06QMAITE: TdxDBGridColumn
      Caption = 'Estoque Máximo'
      HeaderAlignment = taRightJustify
      Width = 86
      BandIndex = 0
      RowIndex = 0
      FieldName = 'QMAITE'
    end
    object grR06MK1ITE: TdxDBGridColumn
      Caption = '% Mk 1'
      Width = 66
      BandIndex = 0
      RowIndex = 0
      FieldName = 'MK1ITE'
    end
    object grR06MK2ITE: TdxDBGridColumn
      Caption = '% Mk 2'
      Width = 66
      BandIndex = 0
      RowIndex = 0
      FieldName = 'MK2ITE'
    end
    object grR06MK3ITE: TdxDBGridColumn
      Caption = '% Mk 3'
      Width = 43
      BandIndex = 0
      RowIndex = 0
      FieldName = 'MK3ITE'
    end
    object grR06MK4ITE: TdxDBGridColumn
      Caption = '% Mk 4'
      Width = 43
      BandIndex = 0
      RowIndex = 0
    end
    object grR06MK5ITE: TdxDBGridColumn
      Caption = '% Mk 5'
      Width = 44
      BandIndex = 0
      RowIndex = 0
    end
    object grR06CODBAR1: TdxDBGridColumn
      Caption = 'Cod. Barras 1'
      Width = 46
      BandIndex = 0
      RowIndex = 3
      FieldName = 'CODBAR1'
      StoredRowIndex = 3
    end
    object grR06TIPBAR1: TdxDBGridColumn
      Caption = 'Tipo Barras 1'
      Width = 67
      BandIndex = 0
      RowIndex = 3
      FieldName = 'TIPBAR1'
      StoredRowIndex = 3
    end
    object grR06QTDBAR1: TdxDBGridColumn
      Caption = 'Qtde Barras 1'
      Width = 79
      BandIndex = 0
      RowIndex = 3
      FieldName = 'QTDBAR1'
      StoredRowIndex = 3
    end
    object grR06CUBBAR1: TdxDBGridColumn
      Caption = 'Cub Barras 1'
      Width = 106
      BandIndex = 0
      RowIndex = 3
      FieldName = 'CUBBAR1'
      StoredRowIndex = 3
    end
    object grR06PESBAR1: TdxDBGridColumn
      Caption = 'Peso Barras 1'
      Width = 103
      BandIndex = 0
      RowIndex = 3
      FieldName = 'PESBAR1'
      StoredRowIndex = 3
    end
    object grR06CODBAR2: TdxDBGridColumn
      Caption = 'Cod. Barras 2'
      Width = 78
      BandIndex = 0
      RowIndex = 3
      FieldName = 'CODBAR2'
      StoredRowIndex = 3
    end
    object grR06TIPBAR2: TdxDBGridColumn
      Caption = 'Tipo Barras 2'
      Width = 82
      BandIndex = 0
      RowIndex = 3
      FieldName = 'TIPBAR2'
      StoredRowIndex = 3
    end
    object grR06QTDBAR2: TdxDBGridColumn
      Caption = 'Qtde Barras 2'
      Width = 85
      BandIndex = 0
      RowIndex = 3
      FieldName = 'QTDBAR2'
      StoredRowIndex = 3
    end
    object grR06CUBBAR2: TdxDBGridColumn
      Caption = 'Cub Barras 2'
      Width = 96
      BandIndex = 0
      RowIndex = 3
      FieldName = 'CUBBAR2'
      StoredRowIndex = 3
    end
    object grR06PESBAR2: TdxDBGridColumn
      Caption = 'Peso Barras 2'
      Width = 109
      BandIndex = 0
      RowIndex = 3
      FieldName = 'PESBAR2'
      StoredRowIndex = 3
    end
    object grR06CODBAR3: TdxDBGridColumn
      Caption = 'Cod. Barras 3'
      Width = 100
      BandIndex = 0
      RowIndex = 3
      FieldName = 'CODBAR3'
      StoredRowIndex = 3
    end
    object grR06TIPBAR3: TdxDBGridColumn
      Caption = 'Tipo Barras 3'
      Width = 121
      BandIndex = 0
      RowIndex = 3
      FieldName = 'TIPBAR3'
      StoredRowIndex = 3
    end
    object grR06QTDBAR3: TdxDBGridColumn
      Caption = 'Qtde Barras 3'
      Width = 31
      BandIndex = 0
      RowIndex = 3
      FieldName = 'QTDBAR3'
      StoredRowIndex = 3
    end
    object grR06CUBBAR3: TdxDBGridColumn
      Caption = 'Cub Barras 3'
      Width = 54
      BandIndex = 0
      RowIndex = 3
      FieldName = 'CUBBAR3'
      StoredRowIndex = 3
    end
    object grR06PESBAR3: TdxDBGridColumn
      Caption = 'Peso Barras 3'
      Width = 35
      BandIndex = 0
      RowIndex = 3
      FieldName = 'PESBAR3'
      StoredRowIndex = 3
    end
    object grR06REFPRO: TdxDBGridColumn
      Caption = 'Referencia'
      BandIndex = 0
      RowIndex = 0
      FieldName = 'REFPRO'
    end
    object grR06OBSSTR: TdxDBGridMemoColumn
      Caption = 'Estados com cobrança de substituição tributária'
      BandIndex = 0
      RowIndex = 4
      FieldName = 'OBSSTR'
      MaxDisplayLength = 270
      StoredRowIndex = 4
    end
    object grR06QTDRMA: TdxDBGridColumn
      Caption = 'RMA'
      BandIndex = 0
      RowIndex = 0
      FieldName = 'QTDRMA'
    end
  end
  object rgStatus: TRadioGroup
    Left = 631
    Top = -1
    Width = 155
    Height = 68
    Caption = ' Status'
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ItemIndex = 0
    Items.Strings = (
      'Ativos'
      'Descontinuados'
      'Todos')
    ParentFont = False
    TabOrder = 16
  end
  object EdCodSub: TdxColorEdit
    Left = 162
    Top = 140
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 12
    OnExit = EdCodSubExit
    OnKeyPress = EdCodEmpKeyPress
    CharCase = ecUpperCase
    MaxLength = 4
    CorDeFoco = clInfoBk
    StoredValues = 2
  end
  object EdCodGru: TdxColorEdit
    Left = 162
    Top = 117
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 10
    OnExit = EdCodGruExit
    OnKeyPress = EdCodEmpKeyPress
    CharCase = ecUpperCase
    MaxLength = 3
    CorDeFoco = clInfoBk
    StoredValues = 2
  end
  object EdCodClp: TdxColorEdit
    Left = 162
    Top = 94
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 8
    OnExit = EdCodClpExit
    OnKeyPress = EdCodEmpKeyPress
    CharCase = ecUpperCase
    MaxLength = 1
    CorDeFoco = clInfoBk
    StoredValues = 2
  end
  object EdCodMrc: TdxColorEdit
    Left = 162
    Top = 71
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 6
    OnExit = EdCodMrcExit
    OnKeyPress = EdCodEmpKeyPress
    Alignment = taRightJustify
    CharCase = ecUpperCase
    MaxLength = 0
    CorDeFoco = clInfoBk
    StoredValues = 3
  end
  object EdCodTip: TdxColorEdit
    Left = 162
    Top = 48
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 4
    OnExit = EdCodTipExit
    OnKeyPress = EdCodEmpKeyPress
    Alignment = taRightJustify
    CharCase = ecUpperCase
    MaxLength = 0
    CorDeFoco = clInfoBk
    StoredValues = 3
  end
  object EdCodCat: TdxColorEdit
    Left = 162
    Top = 25
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 2
    OnExit = EdCodCatExit
    OnKeyPress = EdCodEmpKeyPress
    Alignment = taRightJustify
    CharCase = ecUpperCase
    MaxLength = 0
    CorDeFoco = clInfoBk
    StoredValues = 3
  end
  object EdNomSub: TdxColorEdit
    Left = 274
    Top = 140
    Width = 356
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 13
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdNomGru: TdxColorEdit
    Left = 274
    Top = 117
    Width = 356
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 11
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdNomClp: TdxColorEdit
    Left = 274
    Top = 94
    Width = 356
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 9
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdNomMrc: TdxColorEdit
    Left = 274
    Top = 71
    Width = 356
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 7
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdNomTip: TdxColorEdit
    Left = 274
    Top = 48
    Width = 356
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 5
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdNomCat: TdxColorEdit
    Left = 274
    Top = 25
    Width = 356
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 3
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdCodEmp: TdxColorEdit
    Left = 162
    Top = 2
    Width = 87
    Hint = 'Empresa'
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 0
    OnExit = EdCodEmpExit
    OnKeyPress = EdCodEmpKeyPress
    Alignment = taRightJustify
    CharCase = ecUpperCase
    MaxLength = 0
    CorDeFoco = clInfoBk
    StoredValues = 3
  end
  object EdApeEmp: TdxColorEdit
    Left = 274
    Top = 2
    Width = 356
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 1
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object rgImagem: TRadioGroup
    Left = 631
    Top = 67
    Width = 155
    Height = 63
    Caption = ' Imagem '
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ItemIndex = 2
    Items.Strings = (
      'Com Imagem'
      'Sem Imagem'
      'Todos')
    ParentFont = False
    TabOrder = 17
  end
  object rgSubst: TRadioGroup
    Left = 631
    Top = 130
    Width = 155
    Height = 56
    Caption = ' Substituição Tributária '
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ItemIndex = 2
    Items.Strings = (
      'Sim'
      'Não'
      'Todos')
    ParentFont = False
    TabOrder = 18
  end
  object pnSequenc: TPanel
    Left = 113
    Top = 399
    Width = 561
    Height = 60
    BevelOuter = bvNone
    Color = clTeal
    TabOrder = 22
    Visible = False
    object lbSequenc: TLabel
      Left = 92
      Top = 9
      Width = 378
      Height = 14
      Caption = 'Aguarde ... Verificando informações de substituição tributária'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object ProgressBar: TdxfProgressBar
      Left = 8
      Top = 35
      Width = 544
      Height = 19
      BarBevelOuter = bvNone
      BevelOuter = bvNone
      Orientation = orHorizontal
      Max = 100
      Min = 0
      Position = 0
      ShowText = True
      ShowTextStyle = stsPercent
      BeginColor = clNavy
      EndColor = clWhite
      Style = sExSolid
      Step = 10
      TransparentGlyph = True
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object EstR06: TwwQuery
    Tag = 1
    CachedUpdates = True
    DatabaseName = 'ISade'
    SQL.Strings = (
      
        'Select TextoOcor(IntStrZeros(EstPro.CodCat,3),'#39'-'#39',EstCat.NomCat,' +
        #39#39','#39#39','#39#39','#39#39') as NomCat,'
      
        '           TextoOcor(IntStrZeros(EstPro.CodMrc,3),'#39'-'#39',EstMrc.Nom' +
        'Mrc,'#39#39','#39#39','#39#39','#39#39') as NomMrc,'
      
        '           TextoOcor(IntStrZeros(EstPro.CodTip,3),'#39'-'#39',EstTip.Nom' +
        'Tip,'#39#39','#39#39','#39#39','#39#39') as NomTip,'
      
        '           TextoOcor(EstR06.CodGru,'#39'-'#39',EstGru.NomGru,'#39#39','#39#39','#39#39','#39#39 +
        ') as NomGru,'
      
        '           TextoOcor(EstR06.CodSub,'#39'-'#39',EstSub.NomSub,'#39#39','#39#39','#39#39','#39#39 +
        ') as NomSub,'
      
        '           TextoOcor(EstIcm.CodSt1,EstIcm.CodSt2,'#39#39','#39#39','#39#39','#39#39','#39#39')' +
        ' as CodSit,'
      
        '           (Select CodBar From EstBar Where EstBar.CodClp = EstR' +
        '06.CodClp'
      
        '                                                            and ' +
        'EstBar.CodGru = EstR06.CodGru'
      
        '                                                            and ' +
        'EstBar.CodSub = EstR06.CodSub'
      
        '                                                            and ' +
        'EstBar.CodPro = EstR06.CodPro'
      
        '                                                            and ' +
        'EstBar.NroBar = 1) as CodBar1,'
      
        '           (Select QtdEmb From EstBar Where EstBar.CodClp = EstR' +
        '06.CodClp'
      
        '                                                             and' +
        ' EstBar.CodGru = EstR06.CodGru'
      
        '                                                             and' +
        ' EstBar.CodSub = EstR06.CodSub'
      
        '                                                             and' +
        ' EstBar.CodPro = EstR06.CodPro'
      
        '                                                             and' +
        ' EstBar.NroBar = 1) as QtdBar1,'
      
        '           (Select CubEmb From EstBar Where EstBar.CodClp = EstR' +
        '06.CodClp'
      
        '                                                             and' +
        ' EstBar.CodGru = EstR06.CodGru'
      
        '                                                             and' +
        ' EstBar.CodSub = EstR06.CodSub'
      
        '                                                             and' +
        ' EstBar.CodPro = EstR06.CodPro'
      
        '                                                             and' +
        ' EstBar.NroBar = 1) as CubBar1,'
      
        '           (Select PesEmb From EstBar Where EstBar.CodClp = EstR' +
        '06.CodClp'
      
        '                                                             and' +
        ' EstBar.CodGru = EstR06.CodGru'
      
        '                                                             and' +
        ' EstBar.CodSub = EstR06.CodSub'
      
        '                                                             and' +
        ' EstBar.CodPro = EstR06.CodPro'
      
        '                                                             and' +
        ' EstBar.NroBar = 1) as PesBar1,'
      
        '           (Select EstEmb.NomEmb From EstBar,EstEmb Where EstBar' +
        '.CodEmb = EstEmb.CodEmb'
      
        '                                                                ' +
        '                         and EstBar.CodClp = EstR06.CodClp'
      
        '                                                                ' +
        '                         and EstBar.CodGru = EstR06.CodGru'
      
        '                                                                ' +
        '                         and EstBar.CodSub = EstR06.CodSub'
      
        '                                                                ' +
        '                         and EstBar.CodPro = EstR06.CodPro'
      
        '                                                                ' +
        '                         and EstBar.NroBar = 1) as TipBar1,'
      
        '           (Select CodBar From EstBar Where EstBar.CodClp = EstR' +
        '06.CodClp'
      
        '                                                            and ' +
        'EstBar.CodGru = EstR06.CodGru'
      
        '                                                            and ' +
        'EstBar.CodSub = EstR06.CodSub'
      
        '                                                            and ' +
        'EstBar.CodPro = EstR06.CodPro'
      
        '                                                            and ' +
        'EstBar.NroBar = 2) as CodBar2,'
      
        '           (Select QtdEmb From EstBar Where EstBar.CodClp = EstR' +
        '06.CodClp'
      
        '                                                             and' +
        ' EstBar.CodGru = EstR06.CodGru'
      
        '                                                             and' +
        ' EstBar.CodSub = EstR06.CodSub'
      
        '                                                             and' +
        ' EstBar.CodPro = EstR06.CodPro'
      
        '                                                             and' +
        ' EstBar.NroBar = 1) as QtdBar2,'
      
        '           (Select CubEmb From EstBar Where EstBar.CodClp = EstR' +
        '06.CodClp'
      
        '                                                             and' +
        ' EstBar.CodGru = EstR06.CodGru'
      
        '                                                             and' +
        ' EstBar.CodSub = EstR06.CodSub'
      
        '                                                             and' +
        ' EstBar.CodPro = EstR06.CodPro'
      
        '                                                             and' +
        ' EstBar.NroBar = 1) as CubBar2,'
      
        '           (Select PesEmb From EstBar Where EstBar.CodClp = EstR' +
        '06.CodClp'
      
        '                                                             and' +
        ' EstBar.CodGru = EstR06.CodGru'
      
        '                                                             and' +
        ' EstBar.CodSub = EstR06.CodSub'
      
        '                                                             and' +
        ' EstBar.CodPro = EstR06.CodPro'
      
        '                                                             and' +
        ' EstBar.NroBar = 1) as PesBar2,'
      
        '           (Select EstEmb.NomEmb From EstBar,EstEmb Where EstBar' +
        '.CodEmb = EstEmb.CodEmb'
      
        '                                                                ' +
        '                         and EstBar.CodClp = EstR06.CodClp'
      
        '                                                                ' +
        '                         and EstBar.CodGru = EstR06.CodGru'
      
        '                                                                ' +
        '                         and EstBar.CodSub = EstR06.CodSub'
      
        '                                                                ' +
        '                         and EstBar.CodPro = EstR06.CodPro'
      
        '                                                                ' +
        '                         and EstBar.NroBar = 1) as TipBar2,'
      
        '           (Select CodBar From EstBar Where EstBar.CodClp = EstR' +
        '06.CodClp'
      
        '                                                            and ' +
        'EstBar.CodGru = EstR06.CodGru'
      
        '                                                            and ' +
        'EstBar.CodSub = EstR06.CodSub'
      
        '                                                            and ' +
        'EstBar.CodPro = EstR06.CodPro'
      
        '                                                            and ' +
        'EstBar.NroBar = 3) as CodBar3,'
      
        '           (Select QtdEmb From EstBar Where EstBar.CodClp = EstR' +
        '06.CodClp'
      
        '                                                             and' +
        ' EstBar.CodGru = EstR06.CodGru'
      
        '                                                             and' +
        ' EstBar.CodSub = EstR06.CodSub'
      
        '                                                             and' +
        ' EstBar.CodPro = EstR06.CodPro'
      
        '                                                             and' +
        ' EstBar.NroBar = 1) as QtdBar3,'
      
        '           (Select CubEmb From EstBar Where EstBar.CodClp = EstR' +
        '06.CodClp'
      
        '                                                             and' +
        ' EstBar.CodGru = EstR06.CodGru'
      
        '                                                             and' +
        ' EstBar.CodSub = EstR06.CodSub'
      
        '                                                             and' +
        ' EstBar.CodPro = EstR06.CodPro'
      
        '                                                             and' +
        ' EstBar.NroBar = 1) as CubBar3,'
      
        '           (Select PesEmb From EstBar Where EstBar.CodClp = EstR' +
        '06.CodClp'
      
        '                                                             and' +
        ' EstBar.CodGru = EstR06.CodGru'
      
        '                                                             and' +
        ' EstBar.CodSub = EstR06.CodSub'
      
        '                                                             and' +
        ' EstBar.CodPro = EstR06.CodPro'
      
        '                                                             and' +
        ' EstBar.NroBar = 1) as PesBar3,'
      
        '           (Select EstEmb.NomEmb From EstBar,EstEmb Where EstBar' +
        '.CodEmb = EstEmb.CodEmb'
      
        '                                                                ' +
        '                         and EstBar.CodClp = EstR06.CodClp'
      
        '                                                                ' +
        '                         and EstBar.CodGru = EstR06.CodGru'
      
        '                                                                ' +
        '                         and EstBar.CodSub = EstR06.CodSub'
      
        '                                                                ' +
        '                         and EstBar.CodPro = EstR06.CodPro'
      
        '                                                                ' +
        '                         and EstBar.NroBar = 1) as TipBar3,'
      '           EstIpi.PerIpi,'
      '           EstIcm.PerIcm,'
      '           EstIpi.ClsIpi,'
      '           EstR06.CodPro,'
      '           EstR06.ObsStr,'
      '           EstPro.RefPro,'
      '           EstPro.QtdEmb,'
      '           EstPro.DscPro,'
      '           EstPro.IdePro,'
      '           EstPro.SimPro,'
      '           EstPro.CodAnt,'
      '           EstPro.CodUns,'
      '           EstPro.CodCom,'
      '           EstPro.NumPro,'
      '           EstPro.LiqEmb,'
      '           EstPro.BrtEmb,'
      '           EstPro.CubPro,'
      '           EstPro.CxaPro,'
      
        '           (Select EstIpi.PerImp From EstIpi Where EstIpi.CodIpi' +
        ' = EstPro.IpiEnt and EstIpi.TipIpi = EstPro.IpiTen) as PerImp,'
      '           FinCom.PerCom,'
      '           EstIte.VmeIte,'
      '           EstIte.VpfIte,'
      '           EstIte.Vb1Ite,'
      '           EstIte.Vb2Ite,'
      '           EstIte.Vb3Ite,'
      '           EstIte.Vb4Ite,'
      '           EstIte.Vb5Ite,'
      '           EstIte.Ms1Ite,'
      '           EstIte.Ms2Ite,'
      '           EstIte.Ms3Ite,'
      '           EstIte.Ms4Ite,'
      '           EstIte.Ms5Ite,'
      '           EstIte.Mk1Ite,'
      '           EstIte.Mk2Ite,'
      '           EstIte.Mk3Ite,'
      '           EstIte.Mk4Ite,'
      '           EstIte.Mk5Ite,'
      '           EstIte.QtmIte,'
      '           EstIte.QmaIte,'
      '           EstIte.QtdRma,'
      '           EstIte.VcrIte,'
      '           EstIte.Ds1Ite,'
      '           EstIte.Ds2Ite,'
      '           EstIte.Ds3Ite,'
      '           EstIte.Ds4Ite,'
      '           EstIte.Ds5Ite,'
      '           EstIte.QtdIte,'
      '           EstIte.QtaIte,'
      '           EstIte.DulCmp,'
      '           EstIte.QtdCmp,'
      
        '           EstPro.CodGru || _UNICODE_FSS '#39'.'#39' || EstPro.CodSub ||' +
        ' _UNICODE_FSS '#39'.'#39' || EstPro.CodPro as CodIte'
      'From EstR06 LEFT JOIN EstIte ON (EstR06.CodEmp = EstIte.CodEmp'
      
        '                                                 AND EstR06.CodC' +
        'lp = EstIte.CodClp'
      
        '                                                 AND EstR06.CodG' +
        'ru = EstIte.CodGru'
      
        '                                                 AND EstR06.CodS' +
        'ub = EstIte.CodSub'
      
        '                                                 AND EstR06.CodP' +
        'ro = EstIte.CodPro)'
      
        '                    LEFT JOIN EstPro ON (EstIte.CodClp = EstPro.' +
        'CodClp'
      
        '                                                 AND EstIte.CodG' +
        'ru = EstPro.CodGru'
      
        '                                                 AND EstIte.CodS' +
        'ub = EstPro.CodSub'
      
        '                                                 AND EstIte.CodP' +
        'ro = EstPro.CodPro)'
      
        '                    LEFT JOIN EstCat ON (EstPro.CodCat = EstCat.' +
        'CodCat)'
      
        '                    LEFT JOIN EstMrc ON (EstPro.CodMrc = EstMrc.' +
        'CodMrc)'
      
        '                    LEFT JOIN EstTip ON (EstPro.CodTip = EstTip.' +
        'CodTip)'
      
        '                    LEFT JOIN EstSub ON (EstPro.CodGru = EstSub.' +
        'CodGru'
      
        '                                                  AND EstPro.Cod' +
        'Sub = EstSub.CodSub)'
      
        '                    LEFT JOIN EstGru ON (EstSub.CodGru = EstGru.' +
        'CodGru)'
      
        '                    LEFT JOIN FinCom ON (EstPro.CodCom = FinCom.' +
        'CodCom)'
      
        '                    LEFT JOIN EstIcm ON (EstPro.IcmSai = EstIcm.' +
        'CodIcm'
      
        '                                                 AND EstPro.IcmT' +
        'sd = EstIcm.TipIcm)'
      
        '                      LEFT JOIN EstIpi ON (EstPro.IpiSai = EstIp' +
        'i.CodIpi'
      
        '                                                 AND EstPro.IpiT' +
        'sd = EstIpi.TipIpi)'
      'Where EstPro.CodClp = :CodClp'
      'Order by EstPro.CodCat,EstPro.CodGru,EstPro.CodSub,EstPro.DscPro')
    ValidateWithMask = True
    Left = 3
    Top = 353
    ParamData = <
      item
        DataType = ftString
        Name = 'CodClp'
        ParamType = ptInput
      end>
    object EstR06NOMCAT: TStringField
      DisplayLabel = 'Categoria'
      FieldName = 'NOMCAT'
      FixedChar = True
      Size = 100
    end
    object EstR06NOMMRC: TStringField
      DisplayLabel = 'Marca'
      FieldName = 'NOMMRC'
      FixedChar = True
      Size = 100
    end
    object EstR06NOMGRU: TStringField
      DisplayLabel = 'Grupo'
      FieldName = 'NOMGRU'
      FixedChar = True
      Size = 100
    end
    object EstR06NOMSUB: TStringField
      DisplayLabel = 'SubGrupo'
      FieldName = 'NOMSUB'
      FixedChar = True
      Size = 100
    end
    object EstR06CODPRO: TStringField
      DisplayLabel = 'Item'
      FieldName = 'CODPRO'
      FixedChar = True
      Size = 5
    end
    object EstR06DSCPRO: TStringField
      DisplayLabel = 'Descrição Completa'
      FieldName = 'DSCPRO'
      FixedChar = True
      Size = 70
    end
    object EstR06IDEPRO: TStringField
      DisplayLabel = 'ID'
      FieldName = 'IDEPRO'
      FixedChar = True
      Size = 30
    end
    object EstR06SIMPRO: TStringField
      DisplayLabel = 'Similiar'
      FieldName = 'SIMPRO'
      FixedChar = True
      Size = 10
    end
    object EstR06CODANT: TStringField
      DisplayLabel = 'Codigo Antigo'
      FieldName = 'CODANT'
      FixedChar = True
      Size = 30
    end
    object EstR06CODUNS: TStringField
      DisplayLabel = 'UN'
      FieldName = 'CODUNS'
      FixedChar = True
      Size = 3
    end
    object EstR06CODCOM: TStringField
      DisplayLabel = 'Comissão'
      FieldName = 'CODCOM'
      FixedChar = True
      Size = 3
    end
    object EstR06NUMPRO: TStringField
      DisplayLabel = 'Part Number'
      FieldName = 'NUMPRO'
      FixedChar = True
      Size = 30
    end
    object EstR06NOMTIP: TStringField
      FieldName = 'NOMTIP'
      FixedChar = True
      Size = 100
    end
    object EstR06PERCOM: TFloatField
      FieldName = 'PERCOM'
      DisplayFormat = '##0.00'
      Precision = 2
    end
    object EstR06QTDEMB: TFloatField
      DisplayLabel = 'Embalagem'
      FieldName = 'QTDEMB'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object EstR06CODSIT: TStringField
      FieldName = 'CODSIT'
      FixedChar = True
      Size = 100
    end
    object EstR06PERIPI: TFloatField
      FieldName = 'PERIPI'
      DisplayFormat = '##0.00'
    end
    object EstR06PERICM: TFloatField
      FieldName = 'PERICM'
      DisplayFormat = '##0.00'
    end
    object EstR06CLSIPI: TStringField
      FieldName = 'CLSIPI'
      FixedChar = True
      Size = 30
    end
    object EstR06VB1ITE: TFloatField
      FieldName = 'VB1ITE'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object EstR06VB2ITE: TFloatField
      FieldName = 'VB2ITE'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object EstR06VB3ITE: TFloatField
      FieldName = 'VB3ITE'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object EstR06VB4ITE: TFloatField
      FieldName = 'VB4ITE'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object EstR06VB5ITE: TFloatField
      FieldName = 'VB5ITE'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object EstR06DS1ITE: TFloatField
      FieldName = 'DS1ITE'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object EstR06DS2ITE: TFloatField
      FieldName = 'DS2ITE'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object EstR06DS3ITE: TFloatField
      FieldName = 'DS3ITE'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object EstR06DS4ITE: TFloatField
      FieldName = 'DS4ITE'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object EstR06DS5ITE: TFloatField
      FieldName = 'DS5ITE'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object EstR06QTDITE: TFloatField
      FieldName = 'QTDITE'
      DisplayFormat = '###,###,##0.0000'
    end
    object EstR06VPFITE: TFloatField
      FieldName = 'VPFITE'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object EstR06CODITE: TStringField
      FieldName = 'CODITE'
      Size = 14
    end
    object EstR06QTAITE: TFloatField
      DisplayLabel = 'Adquirido'
      FieldName = 'QTAITE'
      DisplayFormat = '###,###,##0.00'
      Precision = 4
    end
    object EstR06DULCMP: TDateTimeField
      DisplayLabel = 'Dt.Ult.Compra'
      FieldName = 'DULCMP'
    end
    object EstR06QTDCMP: TFloatField
      DisplayLabel = 'Qtde.Ult.Compra'
      FieldName = 'QTDCMP'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object EstR06VMEITE: TFloatField
      FieldName = 'VMEITE'
      DisplayFormat = '###,###,##0.0000'
    end
    object EstR06LIQEMB: TFloatField
      DisplayLabel = 'Peso Liquido'
      FieldName = 'LIQEMB'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object EstR06BRTEMB: TFloatField
      DisplayLabel = 'Peso Bruto'
      FieldName = 'BRTEMB'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object EstR06CUBPRO: TFloatField
      DisplayLabel = 'Cubagem'
      FieldName = 'CUBPRO'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object EstR06CXAPRO: TFloatField
      DisplayLabel = 'Qtde Caixa'
      FieldName = 'CXAPRO'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object EstR06VCRITE: TFloatField
      FieldName = 'VCRITE'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object EstR06QTMITE: TFloatField
      FieldName = 'QTMITE'
      DisplayFormat = '###,###,##0.0000'
    end
    object EstR06QMAITE: TFloatField
      FieldName = 'QMAITE'
      DisplayFormat = '###,###,##0.0000'
    end
    object EstR06PERIMP: TFloatField
      FieldName = 'PERIMP'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object EstR06MS1ITE: TFloatField
      FieldName = 'MS1ITE'
      DisplayFormat = '###,###,##0.00'
    end
    object EstR06MS2ITE: TFloatField
      FieldName = 'MS2ITE'
      DisplayFormat = '###,###,##0.00'
    end
    object EstR06MS3ITE: TFloatField
      FieldName = 'MS3ITE'
      DisplayFormat = '###,###,##0.00'
    end
    object EstR06MS4ITE: TFloatField
      FieldName = 'MS4ITE'
      DisplayFormat = '###,###,##0.00'
    end
    object EstR06MS5ITE: TFloatField
      FieldName = 'MS5ITE'
      DisplayFormat = '###,###,##0.00'
    end
    object EstR06MK1ITE: TFloatField
      FieldName = 'MK1ITE'
    end
    object EstR06MK2ITE: TFloatField
      FieldName = 'MK2ITE'
    end
    object EstR06MK3ITE: TFloatField
      FieldName = 'MK3ITE'
    end
    object EstR06MK4ITE: TFloatField
      FieldName = 'MK4ITE'
    end
    object EstR06MK5ITE: TFloatField
      FieldName = 'MK5ITE'
    end
    object EstR06CODBAR1: TStringField
      FieldName = 'CODBAR1'
      FixedChar = True
    end
    object EstR06CODBAR2: TStringField
      FieldName = 'CODBAR2'
      FixedChar = True
    end
    object EstR06CODBAR3: TStringField
      FieldName = 'CODBAR3'
      FixedChar = True
    end
    object EstR06QTDBAR1: TFloatField
      FieldName = 'QTDBAR1'
      DisplayFormat = '###,###,##0'
    end
    object EstR06CUBBAR1: TFloatField
      FieldName = 'CUBBAR1'
      DisplayFormat = '###,###,##0.0000'
    end
    object EstR06PESBAR1: TFloatField
      FieldName = 'PESBAR1'
      DisplayFormat = '###,###,##0.0000'
    end
    object EstR06TIPBAR1: TStringField
      FieldName = 'TIPBAR1'
      Size = 40
    end
    object EstR06QTDBAR2: TFloatField
      FieldName = 'QTDBAR2'
      DisplayFormat = '###,###,##0'
    end
    object EstR06CUBBAR2: TFloatField
      FieldName = 'CUBBAR2'
      DisplayFormat = '###,###,##0.0000'
    end
    object EstR06PESBAR2: TFloatField
      FieldName = 'PESBAR2'
      DisplayFormat = '###,###,##0.0000'
    end
    object EstR06TIPBAR2: TStringField
      FieldName = 'TIPBAR2'
      Size = 40
    end
    object EstR06QTDBAR3: TFloatField
      FieldName = 'QTDBAR3'
      DisplayFormat = '###,###,##0'
    end
    object EstR06CUBBAR3: TFloatField
      FieldName = 'CUBBAR3'
      DisplayFormat = '###,###,##0.0000'
    end
    object EstR06PESBAR3: TFloatField
      FieldName = 'PESBAR3'
      DisplayFormat = '###,###,##0.0000'
    end
    object EstR06TIPBAR3: TStringField
      FieldName = 'TIPBAR3'
      Size = 40
    end
    object EstR06REFPRO: TStringField
      FieldName = 'REFPRO'
      FixedChar = True
    end
    object EstR06OBSSTR: TMemoField
      FieldName = 'OBSSTR'
      BlobType = ftFmtMemo
      Size = 270
    end
    object EstR06QTDRMA: TFloatField
      FieldName = 'QTDRMA'
      DisplayFormat = '###,###,##0.0000'
    end
  end
  object quSql: TwwQuery
    Tag = 1
    AutoCalcFields = False
    DatabaseName = 'ISade'
    UniDirectional = True
    ValidateWithMask = True
    Left = 3
    Top = 381
  end
  object dxComponentPrinter: TdxComponentPrinter
    CurrentLink = dxComponentPrinterLink1
    DateFormat = 1
    PreviewOptions.Caption = 'Visualizando Relatorio'
    PreviewOptions.RegistryPath = 'Software\Developer Express\PrintingSystem\DesignTime'
    TimeFormat = 1
    Left = 31
    Top = 437
    object dxComponentPrinterLink1: TdxDBGridReportLink
      Active = True
      Component = grR06
      DateTime = 40212.755602581
      DesignerHelpContext = 0
      PrinterPage.DMPaper = 9
      PrinterPage.Footer = 6350
      PrinterPage.Header = 6350
      PrinterPage.Margins.Bottom = 12700
      PrinterPage.Margins.Left = 12700
      PrinterPage.Margins.Right = 12700
      PrinterPage.Margins.Top = 48200
      PrinterPage.Orientation = poLandscape
      PrinterPage.PageFooter.Font.Charset = DEFAULT_CHARSET
      PrinterPage.PageFooter.Font.Color = clWindowText
      PrinterPage.PageFooter.Font.Height = -11
      PrinterPage.PageFooter.Font.Name = 'Tahoma'
      PrinterPage.PageFooter.Font.Style = []
      PrinterPage.PageFooter.LeftTitle.Strings = (
        'Emerion Consultoria e Sistemas de Informatica Ltda')
      PrinterPage.PageFooter.RightTitle.Strings = (
        'Pagina No. [Page #]')
      PrinterPage.PageHeader.Background.Brush.Color = clBlack
      PrinterPage.PageHeader.Background.PictureMode = ppmTile
      PrinterPage.PageHeader.CenterTextAlignY = taTop
      PrinterPage.PageHeader.Font.Charset = DEFAULT_CHARSET
      PrinterPage.PageHeader.Font.Color = clWindowText
      PrinterPage.PageHeader.Font.Height = -11
      PrinterPage.PageHeader.Font.Name = 'Tahoma'
      PrinterPage.PageHeader.Font.Style = []
      PrinterPage.PageHeader.LeftTextAlignY = taTop
      PrinterPage.PageHeader.LeftTitle.Strings = (
        'VMP PAPEIS'
        'VMP PAPEIS PARA EMBALAGENS LTDA'
        
          'RUA TEIXEIRA SOARES - JD DAS ACACIAS - GUARULHOS - SP CEP 03701-' +
          '010'
        ''
        'Categoria :'
        'Tipo :'
        'Marca :'
        'Classificação :'
        'Grupo :'
        'Sub-Grupo :'
        'Item :')
      PrinterPage.PageHeader.RightTextAlignY = taTop
      PrinterPage.PageHeader.RightTitle.Strings = (
        '[Date Printed][Time Printed]')
      PrinterPage.PageSize.X = 210000
      PrinterPage.PageSize.Y = 297000
      PrinterPage.PaperSource = 15
      PrinterPage.ScaleFactor = 82
      PrinterPage._dxMeasurementUnits_ = 0
      PrinterPage._dxLastMU_ = 2
      ReportTitle.Font.Charset = ANSI_CHARSET
      ReportTitle.Font.Color = clWindowText
      ReportTitle.Font.Height = -15
      ReportTitle.Font.Name = 'Tahoma'
      ReportTitle.Font.Style = [fsBold]
      ReportTitle.Mode = tmOnFirstPage
      ReportTitle.Text = #13#10'Relação de Informações Cadastrais dos Itens'#13#10
      BandColor = clWhite
      BandFont.Charset = DEFAULT_CHARSET
      BandFont.Color = clWindowText
      BandFont.Height = -11
      BandFont.Name = 'Tahoma'
      BandFont.Style = [fsBold]
      EvenFont.Charset = DEFAULT_CHARSET
      EvenFont.Color = clWindowText
      EvenFont.Height = -11
      EvenFont.Name = 'Times New Roman'
      EvenFont.Style = []
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Times New Roman'
      Font.Style = []
      FooterFont.Charset = DEFAULT_CHARSET
      FooterFont.Color = clNavy
      FooterFont.Height = -13
      FooterFont.Name = 'Times New Roman'
      FooterFont.Style = [fsBold]
      GridLineColor = clWhite
      GroupNodeFont.Charset = DEFAULT_CHARSET
      GroupNodeFont.Color = clNavy
      GroupNodeFont.Height = -13
      GroupNodeFont.Name = 'Times New Roman'
      GroupNodeFont.Style = [fsBold]
      GroupNodeColor = clWhite
      HeaderFont.Charset = ANSI_CHARSET
      HeaderFont.Color = clNavy
      HeaderFont.Height = -13
      HeaderFont.Name = 'Times New Roman'
      HeaderFont.Style = [fsBold]
      OddFont.Charset = DEFAULT_CHARSET
      OddFont.Color = clWindowText
      OddFont.Height = -11
      OddFont.Name = 'Times New Roman'
      OddFont.Style = []
      Options = [tlpoBands, tlpoHeaders, tlpoFlatCheckMarks]
      PreviewFont.Charset = DEFAULT_CHARSET
      PreviewFont.Color = clWindowText
      PreviewFont.Height = -11
      PreviewFont.Name = 'Times New Roman'
      PreviewFont.Style = []
      RowFooterColor = clWhite
      RowFooterFont.Charset = DEFAULT_CHARSET
      RowFooterFont.Color = clRed
      RowFooterFont.Height = -13
      RowFooterFont.Name = 'Times New Roman'
      RowFooterFont.Style = [fsBold]
    end
  end
  object SaveDialog: TSaveDialog
    FileName = 'ExpGrid'
    Options = [ofOverwritePrompt, ofHideReadOnly]
    Left = 31
    Top = 409
  end
  object DsR06: TwwDataSource
    DataSet = EstR06
    Left = 31
    Top = 353
  end
  object ImageList1: TImageList
    Left = 3
    Top = 437
    Bitmap = {
      
      000000000000}
  end
  object PopUpMenu: TPopupMenu
    Images = ImageList1
    OwnerDraw = True
    Left = 3
    Top = 409
    object Colunas1: TMenuItem
      Caption = 'Colunas ...'
      OnClick = Colunas1Click
    end
    object Exportarpara1: TMenuItem
      Caption = 'Exportar para ...'
      ImageIndex = 0
      object HTML: TMenuItem
        Caption = 'HTML ...'
        Hint = 'Exportar Relatorio no Formato HTML'
        ImageIndex = 1
        OnClick = HTMLClick
      end
      object Excel: TMenuItem
        Caption = 'EXCEL ...'
        Hint = 'Exportar Relatorio no Formato EXCEL'
        ImageIndex = 3
        OnClick = ExcelClick
      end
      object XML: TMenuItem
        Caption = 'XML && XSL ...'
        Hint = 'Exportar Relatorio no Formato XML '
        ImageIndex = 2
        OnClick = XMLClick
      end
    end
  end
  object quSQL1: TwwQuery
    Tag = 1
    AutoCalcFields = False
    DatabaseName = 'ISade'
    UniDirectional = True
    ValidateWithMask = True
    Left = 31
    Top = 381
  end
end
