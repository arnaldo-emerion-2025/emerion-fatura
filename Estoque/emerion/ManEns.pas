     ...
     if Trim( quSQL.FieldbyName('DscPro').AsString ) <> '' then begin
        ...
        with quSQL,SQL do begin
             ...
             Text := ' Select EstIte.CstIte,'+
                     ...
                     '        EstIte.VpfIte '+
                     ' From EstIte'+
                     ' Where EstIte.CodEmp = '+ QuotedStr(EdCodEmp.Text)+
                     '   and EstIte.CodClp = '+ QuotedStr(EdCodClp.Text)+
                     '   and EstIte.CodGru = '+ QuotedStr(EdCodGru.Text)+
                     '   and EstIte.CodSub = '+ QuotedStr(EdCodSub.Text)+
                     '   and EstIte.CodPro = '+ QuotedStr(EdCodPro.Text);
             Open;

             if sFlgTab = 'Ultimo Preco'           then EdValOpe.Value := FieldbyName('CstIte').AsFloat;
             if sFlgTab = 'Custo Historico'        then EdValOpe.Value := FieldbyName('VchIte').AsFloat;
             if sFlgTab = 'Custo Ponderado'        then EdValOpe.Value := FieldbyName('VcpIte').AsFloat;
             if sFlgTab = 'Custo Referencial'      then EdValOpe.Value := FieldbyName('VcrIte').AsFloat;
             if sFlgTab = 'Custo Historico (G)'    then EdValOpe.Value := FieldbyName('VreIte').AsFloat;
             if sFlgTab = 'Custo Ponderado (G)'    then EdValOpe.Value := FieldbyName('VprIte').AsFloat;
             if sFlgTab = 'Media Ponderada (E)'    then EdValOpe.Value := FieldbyName('VmeIte').AsFloat;
             if sFlgTab = 'Custo da Ultima Compra' then EdValOpe.Value := FieldbyName('VpfIte').AsFloat;
  ...
  if Trim( EdCodTam.Text ) <> '' then begin
     ...
     with quSql,SQL do begin
          ...
          Text := ' Select Count(*) as QtdReg'+
                  ' From EstQte '+
                  ' Where EstQte.CodEmp = '+ QuotedStr(EdCodEmp.Text)+
...
begin
  ...
  if Trim( EdCodCor.Text ) <> '' then begin
     ...
     with quSql,SQL do begin
          ...
          Close;
          Text := ' Select EstCor.NomCor'+
                  ' From EstCor '+
                  ' Where EstCor.CodCor = '+ QuotedStr(EdCodCor.Text);
          Open;
        ...
        if Trim(EdObsEnf.Text) <> '' then begin
           ...
           if GGus_Id <> 1 then begin
              ...
              with quSQL,SQL do begin
                   ...
                   with Params do begin
                        ...
                        Params[0].AsInteger := StrToInt(EdCodEmp.Text);
           ...
           with quSQL,SQL do begin
                ...
                Text := ' Select Count(*) as QtdReg'+
                        ...
                        '   and EstQte.CodSub = '+ QuotedStr(EdCodSub.Text)+
                        '   and EstQte.CodPro = '+ QuotedStr(EdCodPro.Text)+
                        '   and EstQte.CodTam = '+ QuotedStr(EdCodTam.Text)+
                        '   and EstQte.CodCor = '+ QuotedStr(EdCodCor.Text);
                Open;
              ...
              if fMsg('Confirma Operação ?','O') then begin
                 ...
                 if ( GEmp_Id ) > 0 then begin
                    ...
                    with quSQL,SQL do begin
                         ...
                         Close;
                         Text := ' Select GerEmp.NomEmp From GerEmp Where GerEmp.CodEmp = '+ QuotedStr(EdCodEmp.Text);
                         Open;
...
begin
  ...
  with quSQL,SQL do begin
       ...
       sFlgTab := Trim(FieldbyName('FlgTab').AsString);

       if FieldbyName('BloCst').AsString = 'Sim' then begin

          if (GGus_Id <> 1) and (GUsu_Id <> 1) and (GUsu_Id <> 999) then begin

             EdValOpe.Enabled := False;
             EdCstOpe.Enabled := False;

             EdValOpe.Font.Style := [fsBold];
             EdCstOpe.Font.Style := [fsBold];
