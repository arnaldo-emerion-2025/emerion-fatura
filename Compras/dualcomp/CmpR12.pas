...
begin
  ...
  if Trim(EdCodEmp.Text) <> '' then
    sFiltro := sFiltro + ' and CmpNf2.CodEmp = ''' + EdCodEmp.Text + '''';

  if Trim(fLimpaStr(EdPsqDtePed1.Text)) <> '' then
    sFiltro := sFiltro + ' and CmpNfs.DteFat >= ''' + fDateToSQL(EdPsqDtePed1.Date) + '''';
  if Trim(fLimpaStr(EdPsqDtePed2.Text)) <> '' then
    sFiltro := sFiltro + ' and CmpNfs.DteFat <= ''' + fDateToSQL(EdPsqDtePed2.Date) + '''';

  if Trim(EdCodClp.Text) <> '' then
    sFiltro := sFiltro + ' and CmpNf2.CodClp = ''' + EdCodClp.Text + '''';
  if Trim(EdCodGru.Text) <> '' then
    sFiltro := sFiltro + ' and CmpNf2.CodGru = ''' + EdCodGru.Text + '''';
  if Trim(EdCodSub.Text) <> '' then
    sFiltro := sFiltro + ' and CmpNf2.CodSub = ''' + EdCodSub.Text + '''';
  if Trim(EdCodPro.Text) <> '' then
    sFiltro := sFiltro + ' and CmpNf2.CodPro = ''' + EdCodPro.Text + '''';
  if Trim(EdCodFor.Text) <> '' then
    sFiltro := sFiltro + ' and CmpNfs.CodFor = ''' + EdCodFor.Text + '''';

  sFiltro := sFiltro + ' and CmpNfs.SitNfs <> ''' + 'Recebido' + ''' and CmpNfs.SitNfs <> ''' + 'Cancelado' + ''' and CmpNfs.SitNfs <> ''' +
    'Saldo Nao Atendido' + ''' and CmpNfs.SitNfs <> ''' + 'Nao Concluido' + '''';

  with quSQL, SQL do
  begin

    Close;
    Text := ' Delete From CmpR12 Where SeqArq = ''' + SeqArq + '''';
    ExecSQL;
  ...
  begin
    ...
    Close;
    Text := ' Insert Into CmpR12(SeqArq,CodEmp,DtePed,DpePed,NumPed,CodFil,CodFor,LanEst,CodClp,CodGru,CodSub,CodPro,CodUnd,DesPe2,QtpPe2,QtrPe2,SldPe2,VluPe2,IpiPe2,IcmPe2,DscPe2,PacPe2,CodUsu,NroNfs)'
      +
      ' Select ''' + SeqArq + ''',' +
      '        CmpNf2.CodEmp,' +
      '        CmpNfs.DteFat,' +
  ...
  begin
    ...
    Text := ' Select CmpR12.NumPed,' +
      ...
      '        TextoOcor(IntStrZeros(CmpR12.CodFor,5),''' + '-' + ''',FinFor.ApeFor,''' + '' + ''',''' + '' + ''',''' + '' + ''',''' + '' + ''') as ApeFor,' +
      '        CmpR12.CodClp || _UNICODE_FSS ''' + '-' + ''' || CmpR12.CodGru || _UNICODE_FSS ''' + '.' + ''' || CmpR12.CodSub || _UNICODE_FSS ''' + '.' +
        ''' || CmpR12.CodPro as CodIte' +
    ...
    Text := Text + ' Order by CmpR12.CodGru,CmpR12.CodSub,CmpR12.CodPro,CmpR12.NumPed';
