...
begin
  ...
  sFiltro := ' Where CmpPed.SitPed = '''+ 'Concluido' +'''';

  if Trim( EdPsqCodEmp.Text ) <> '' then sFiltro := sFiltro + ' and CmpPed.CodEmp  = '''+ EdPsqCodEmp.Text +'''';
  if Trim( EdPsqCodTmo.Text ) <> '' then sFiltro := sFiltro + ' and CmpPed.CodTmo  = '''+ EdPsqCodTmo.Text +'''';
  if Trim( EdPsqUfeEnt.Text ) <> '' then sFiltro := sFiltro + ' and CmpPed.UfePed  = '''+ EdPsqUfeEnt.Text +'''';
  if Trim( EdPsqCodGFo.Text ) <> '' then sFiltro := sFiltro + ' and FinFor.CodGFo  = '''+ EdPsqCodGFo.Text +'''';

  if Trim(fLimpaStr(EdPsqDteOpe1.Text)) <> '' then sFiltro := sFiltro + ' and CmpPed.DtePed >= '''+ fDateToSQL(EdPsqDteOpe1.Date) +'''';
  if Trim(fLimpaStr(EdPsqDteOpe2.Text)) <> '' then sFiltro := sFiltro + ' and CmpPed.DtePed <= '''+ fDateToSQL(EdPsqDteOpe2.Date) +'''';
  ...
  with quSQL,SQL do begin
       ...
       Text := ' Insert Into CmpR01'+
               ...
               '             LEFT JOIN GerUsu ON (FinFor.CodSup = GerUsu.CodUsu)'+ sFiltro + ' Group by CmpPed.CodFor,FinFor.ApeFor,GerUsu.LogUsu';
       ExecSQL;
