...
begin
  ...
  CmpNfs.Close;
  CmpNfs.Params[0].AsInteger := fmManLro.CodEmp;
  CmpNfs.Params[1].AsDateTime := fmManLro.DteNfs;
  CmpNfs.Params[2].AsInteger := fmManLro.SeqNfs;
  CmpNfs.Open;
...
begin
  ...
  begin
    ...
    if fMsg('Deseja Realmente Abandonar a Emissao da Nota ?', 'O') then
    begin
...
begin
  ...
  begin
    ...
    else if Trim(CmpNfsSitNfs.Value) = 'Nao Concluido' then
    ...
    else if Trim(CmpNfsSitNfs.Value) = 'Processo de Alteracao' then
