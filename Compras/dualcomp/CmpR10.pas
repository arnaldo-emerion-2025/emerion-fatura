...
type
  ...
  TfmCmpR10 = class(TfmPadrao)
    ...
    grEntNUMDOC: TdxDBGridMaskColumn;
    grEntAPEFOR: TdxDBGridMaskColumn;
    grEntNUMENT: TdxDBGridMaskColumn;
    grEntTOTENC: TdxDBGridMaskColumn;
    grEntTOTGEC: TdxDBGridMaskColumn;
    grEntTOTICC: TdxDBGridMaskColumn;
    grEntTOTIPC: TdxDBGridMaskColumn;
    grEntDTEOPE: TdxDBGridDateColumn;
    Label14: TLabel;
...
begin
  ...
  if Trim( EdPsqCodFor.Text ) <> '' then sFiltro := sFiltro + ' and CmpEnt.CodFor = '''+ EdPsqCodFor.Text +'''';
  if Trim( EdPsqUfeEnt.Text ) <> '' then sFiltro := sFiltro + ' and CmpEnt.UfeEnt = '''+ EdPsqUfeEnt.Text +'''';

  if Trim( EdPsqCodPfa.Text ) <> '' then sFiltro := sFiltro + ' and CmpEnt.TipPfa = '''+ 'Entrada' +''' and CmpEnt.CodPfa = '''+ EdPsqCodPfa.Text +'''';
  ...
  if Trim(fLimpaStr(EdPsqDteDoc1.Text)) <> '' then sFiltro := sFiltro + ' and CmpEnt.DteDoc >= '''+ fDateToSQL(EdPsqDteDoc1.Date) +'''';
  if Trim(fLimpaStr(EdPsqDteDoc2.Text)) <> '' then sFiltro := sFiltro + ' and CmpEnt.DteDoc <= '''+ fDateToSQL(EdPsqDteDoc2.Date) +'''';
