...
uses dxDemoUtils, Bbmensag, ManGDB, ManLr3,
  ManCn1_NFE, FileCtrl, Bbfuncao, Bbgeral, ManPri, ConCn1_NFE;

{$R *.DFM}

procedure TfmManCnf.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TfmManCnf.PaintBoxPaint(Sender: TObject);
begin
  with Sender as TPaintBox do
    FillGrayGradientRect(PaintBox.Canvas, PaintBox.ClientRect, PaintBox.Color);
end;

procedure TfmManCnf.FormShow(Sender: TObject);
begin
  inherited;
...
procedure TfmManCnf.bCancelarClick(Sender: TObject);
begin
  Close;
end;
            ...
            begin
              ...
              begin
                ...
                //if (Trim(GNFeEnvia) = '') or (Trim(GNFeEnvia) = 'EXE') or (Trim(GNFeEnvia) = 'DEFAUT') then
                  Executa(ExtractFilePath(Application.exename) + '\NFeEmerion2.exe CANCELA ' + (vnumnota), SW_NORMAL);
                //else
                ...
                begin
                  ...
                  MoveFile(pchar(CaminhoRetorno + '\LogErro-CN' + (VNumNota) + '.txt'), pchar(CaminhoRetorno + '\LogErro-CN' + (VNumNota) + '.EP' +
                    inttostr(i)));
                  ...
                  begin
                    ...
                    if fmmanlr3.CmpNfs.State = dsBrowse then
                      fmmanlr3.CmpNfs.Edit;
                    //
                    ...
                    repeat
                      ...
                      if pos('<dhRecbto>', TLinha) > 0 then
                        fmmanlr3.CmpNfsDtcNFE.Value := strtodate(copy(TLinha, pos('<dhRecbto>', TLinha) + Length('<dhRecbto>') + 8, 2) + '/' + copy(TLinha,
              ...
              begin
                ...
                //if (Trim(GNFeEnvia) = '') or (Trim(GNFeEnvia) = 'EXE') or (Trim(GNFeEnvia) = 'DEFAUT') then
                  Executa(ExtractFilePath(Application.exename) + '\NFeEmerion2.exe INUTIL ' + vnumnota, SW_NORMAL);
                //else
                ...
                else if FileExists(CaminhoRetorno + '\LogErro-IN' + VNumNota + '.txt') then
                begin
...
begin
  inherited;
  if fmManLr3.CmpNfs.State <> dsBrowse then
    fmManLr3.CmpNfs.CancelUpdates;
end;

procedure TfmManCnf.bt_erroClick(Sender: TObject);
begin
  inherited;
