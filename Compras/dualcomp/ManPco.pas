...
type
  TfmManPco = class(TfmPadrao)
    ...
    procedure EdCodClpKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure EdCodClpMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure EdCodGruKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure EdCodGruMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure EdCodPfoKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure EdCodPfoMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
...
begin
  ...
  if Trim( EdPsqCodFor.Text ) <> '' then sFiltro := ' Where FinFor.CodFor = '''+ EdPsqCodFor.Text +'''';
  ...
  if Trim( EdPsqCodClp.Text ) <> '' then sFiltro1 := sFiltro1 + ' and CmpPfo.CodClp = '''+ EdPsqCodClp.Text +'''';
  if Trim( EdPsqCodGru.Text ) <> '' then sFiltro1 := sFiltro1 + ' and CmpPfo.CodGru = '''+ EdPsqCodGru.Text +'''';
  if Trim( EdPsqCodSub.Text ) <> '' then sFiltro1 := sFiltro1 + ' and CmpPfo.CodSub = '''+ EdPsqCodSub.Text +'''';
  if Trim( EdPsqCodPro.Text ) <> '' then sFiltro1 := sFiltro1 + ' and CmpPfo.CodPro = '''+ EdPsqCodPro.Text +'''';    
...
begin
  ...
  if (EstPro.Params[0].AsString <> CmpPfoCodClp.Value) or (EstPro.Params[1].AsString  <> CmpPfoCodGru.Value) or
...
begin
  ...
  CmpPfoSeqPfo.Value := FinForSeqPfo.Value + 1;
...
procedure TfmManPco.EdCodClpKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  pnText.Caption := 'F1-Iniciais';
end;

procedure TfmManPco.EdCodClpMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  inherited;
  pnText.Caption := 'F1-Iniciais';
end;

procedure TfmManPco.EdCodGruKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  pnText.Caption := 'F1-Iniciais F2-Inteligente';
end;

procedure TfmManPco.EdCodGruMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  inherited;
  pnText.Caption := 'F1-Iniciais F2-Inteligente';
end;

procedure TfmManPco.EdCodPfoKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  pnText.Caption := '';
end;

procedure TfmManPco.EdCodPfoMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  inherited;
  pnText.Caption := '';
end;
...
var
sCodFor,sSeqPfo : integer;
begin
  inherited;

  sCodFor := FinForCodFor.Value;
  sSeqPfo := CmpPfoSeqPfo.Value;

  FinFor.Close;
  FinFor.Open;

  if sCodFor > 0 then begin

     FinFor.Locate('CodFor',sCodFor,[LoPartialKey]);

     if sSeqPfo > 0 then CmpPfo.Locate('CodFor;SeqPfo',VarArrayOf([sCodFor,sSeqPfo]), [loPartialKey]);
...
begin
  ...
  try
     ...
     if Trim( EdPsqCodClp.Text ) <> '' then fmAuxIni.CodClp := EdPsqCodClp.Text;
     if Trim( EdPsqCodGru.Text ) <> '' then fmAuxIni.CodGru := EdPsqCodGru.Text;
     if Trim( EdPsqCodSub.Text ) <> '' then fmAuxIni.CodGru := EdPsqCodSub.Text;
...
begin
  inherited;
  if Trim( CmpPfoCodClp.Value ) <> '' then
     Text := CmpPfoCodGru.Value+'.'+CmpPfoCodSub.Value+'.'+CmpPfoCodPro.Value
  else
     Text := CmpPfoCodGru.Value+'.'+CmpPfoCodSub.Value+'.'+CmpPfoCodPro.Value;
end;
