inherited fmCmpR07: TfmCmpR07
  Left = 0
  Caption = 'Relação de Movimentações de Entradas'
  ClientHeight = 482
  ClientWidth = 786
  Position = poDesigned
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  object PaintBox: TPaintBox
    Left = 0
    Top = 0
    Width = 786
    Height = 482
    Align = alClient
    OnPaint = PaintBoxPaint
  end
  object Label28: TLabel
    Left = 4
    Top = 269
    Width = 123
    Height = 13
    Caption = 'Realizadas no Periodo de '
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object Label29: TLabel
    Left = 239
    Top = 269
    Width = 6
    Height = 13
    Caption = 'a'
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object Label26: TLabel
    Left = 4
    Top = 39
    Width = 41
    Height = 13
    Caption = 'Empresa'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label20: TLabel
    Left = 4
    Top = 63
    Width = 55
    Height = 13
    Caption = 'Fornecedor'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object bPsqEmp: TSpeedButton
    Left = 231
    Top = 36
    Width = 23
    Height = 21
    Hint = 'Pesquisar Empresas Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = bPsqEmpClick
  end
  object bPsqFor: TSpeedButton
    Left = 231
    Top = 59
    Width = 23
    Height = 21
    Hint = 'Pesquisar Fornecedores Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = bPsqForClick
  end
  object Label10: TLabel
    Left = 4
    Top = 0
    Width = 154
    Height = 13
    Caption = 'Informações para Pesquisa'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Bevel1: TBevel
    Left = 164
    Top = 6
    Width = 620
    Height = 3
    Shape = bsTopLine
    Style = bsRaised
  end
  object Label2: TLabel
    Left = 4
    Top = 305
    Width = 107
    Height = 13
    Caption = 'Itens Selecionados'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Bevel2: TBevel
    Left = 164
    Top = 312
    Width = 620
    Height = 3
    Shape = bsTopLine
    Style = bsRaised
  end
  object Label5: TLabel
    Left = 132
    Top = 39
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label6: TLabel
    Left = 132
    Top = 62
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label11: TLabel
    Left = 132
    Top = 269
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label4: TLabel
    Left = 4
    Top = 16
    Width = 117
    Height = 13
    Caption = 'Tipo de Nota de Entrada'
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Transparent = True
  end
  object Label9: TLabel
    Left = 132
    Top = 16
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label14: TLabel
    Left = 4
    Top = 87
    Width = 32
    Height = 13
    Caption = 'Moeda'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object bPsqTmo: TSpeedButton
    Left = 231
    Top = 82
    Width = 23
    Height = 20
    Hint = 'Pesquisar Tipos de Moedas Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    ParentShowHint = False
    ShowHint = True
    OnClick = bPsqTmoClick
  end
  object Label7: TLabel
    Left = 132
    Top = 85
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label110: TLabel
    Left = 4
    Top = 108
    Width = 47
    Height = 13
    Caption = 'Categoria'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label111: TLabel
    Left = 4
    Top = 132
    Width = 20
    Height = 13
    Caption = 'Tipo'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label112: TLabel
    Left = 4
    Top = 155
    Width = 29
    Height = 13
    Caption = 'Marca'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label47: TLabel
    Left = 4
    Top = 178
    Width = 72
    Height = 13
    Caption = 'Classificação'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label48: TLabel
    Left = 4
    Top = 201
    Width = 34
    Height = 13
    Caption = 'Grupo'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label49: TLabel
    Left = 4
    Top = 223
    Width = 60
    Height = 13
    Caption = 'Sub-Grupo'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label3: TLabel
    Left = 4
    Top = 246
    Width = 28
    Height = 13
    Caption = 'Item'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label1: TLabel
    Left = 132
    Top = 246
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label8: TLabel
    Left = 132
    Top = 223
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label12: TLabel
    Left = 132
    Top = 200
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label13: TLabel
    Left = 132
    Top = 177
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label15: TLabel
    Left = 132
    Top = 154
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label16: TLabel
    Left = 132
    Top = 131
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object Label17: TLabel
    Left = 132
    Top = 108
    Width = 4
    Height = 13
    Caption = ':'
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object bPsqPro: TSpeedButton
    Left = 231
    Top = 244
    Width = 23
    Height = 20
    Hint = 'Pesquisar Grupos Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = bPsqProClick
  end
  object bPsqSub: TSpeedButton
    Left = 231
    Top = 220
    Width = 23
    Height = 21
    Hint = 'Pesquisar Sub-Grupos Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = bPsqSubClick
  end
  object bPsqGru: TSpeedButton
    Left = 231
    Top = 197
    Width = 23
    Height = 21
    Hint = 'Pesquisar Grupos Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = bPsqGruClick
  end
  object bPsqClp: TSpeedButton
    Left = 231
    Top = 174
    Width = 23
    Height = 21
    Hint = 'Pesquisar Classificações de Itens Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = bPsqClpClick
  end
  object PsqMrc: TSpeedButton
    Left = 231
    Top = 151
    Width = 23
    Height = 21
    Hint = 'Pesquisar Marcas Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = PsqMrcClick
  end
  object PsqTip: TSpeedButton
    Left = 231
    Top = 128
    Width = 23
    Height = 21
    Hint = 'Pesquisar Tipos Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = PsqTipClick
  end
  object PsqCat: TSpeedButton
    Left = 231
    Top = 105
    Width = 23
    Height = 21
    Hint = 'Pesquisar Categorias Existentes'
    Flat = True
    Glyph.Data = {
      
      CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0C0CFD0}
    OnClick = PsqCatClick
  end
  object EdDteDoc1: TdxColorDateEdit
    Left = 143
    Top = 265
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 21
    Date = -700000
    SaveTime = False
    UseEditMask = True
    CorDeFoco = clInfoBk
    StoredValues = 4
  end
  object EdDteDoc2: TdxColorDateEdit
    Left = 254
    Top = 265
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 22
    Date = -700000
    SaveTime = False
    UseEditMask = True
    CorDeFoco = clInfoBk
    StoredValues = 4
  end
  object EdCodFor: TdxColorEdit
    Left = 143
    Top = 58
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 3
    OnExit = EdCodForExit
    OnKeyDown = EdCodForKeyDown
    OnKeyPress = EdPsqNumResKeyPress
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdCodEmp: TdxColorEdit
    Left = 143
    Top = 35
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 1
    OnExit = EdCodEmpExit
    OnKeyDown = EdCodEmpKeyDown
    OnKeyPress = EdPsqNumResKeyPress
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdNomFor: TdxColorEdit
    Left = 254
    Top = 58
    Width = 383
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 4
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdNomEmp: TdxColorEdit
    Left = 254
    Top = 35
    Width = 383
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 2
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object bbPesquisa: TBitBtn
    Left = 392
    Top = 266
    Width = 195
    Height = 44
    Caption = '&Selecionar'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 24
    OnClick = bbPesquisaClick
    Glyph.Data = {
      
      3232323232323232323232323232323232323232323232323232}
  end
  object grEnt: TdxDBGrid
    Left = 2
    Top = 319
    Width = 782
    Height = 162
    Bands = <
      item
        Width = 2415
      end>
    DefaultLayout = False
    HeaderPanelRowCount = 1
    KeyField = 'NUMENT'
    ShowGroupPanel = True
    ShowSummaryFooter = True
    SummaryGroups = <
      item
        DefaultGroup = False
        SummaryItems = <
          item
            ColumnName = 'grEntQTPEN2'
            SummaryFormat = '###,###,##0'
            SummaryType = cstSum
          end>
        Name = 'grEntSummaryGroup1'
      end>
    SummarySeparator = ', '
    BorderStyle = bsNone
    Color = clInfoBk
    Font.Charset = ANSI_CHARSET
    Font.Color = clNavy
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    PopupMenu = PopUpMenu
    TabOrder = 26
    BandColor = clMenu
    BandFont.Charset = ANSI_CHARSET
    BandFont.Color = clWindowText
    BandFont.Height = -11
    BandFont.Name = 'Tahoma'
    BandFont.Style = [fsBold]
    DataSource = DsEnt
    Filter.Active = True
    Filter.AutoDataSetFilter = True
    Filter.Criteria = {00000000}
    GroupPanelColor = clAppWorkSpace
    GroupPanelFontColor = clInfoBk
    GroupNodeColor = clInfoBk
    HeaderFont.Charset = ANSI_CHARSET
    HeaderFont.Color = clWindowText
    HeaderFont.Height = -11
    HeaderFont.Name = 'Tahoma'
    HeaderFont.Style = [fsBold]
    LookAndFeel = lfFlat
    OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEnterShowEditor, edgoImmediateEditor, edgoTabThrough, edgoVertThrough]
    OptionsCustomize = [edgoBandPanelSizing, edgoBandSizing, edgoColumnMoving, edgoColumnSizing, edgoExtCustomizing, edgoFullSizing]
    OptionsDB = [edgoCanNavigation, edgoLoadAllRecords, edgoUseBookmarks]
    OptionsView = [edgoPreview, edgoRowSelect, edgoUseBitmap]
    PreviewFont.Charset = ANSI_CHARSET
    PreviewFont.Color = clBlue
    PreviewFont.Height = -11
    PreviewFont.Name = 'Tahoma'
    PreviewFont.Style = []
    PreviewLines = 0
    RowFooterColor = clInfoBk
    RowFooterTextColor = clRed
    ShowGrid = False
    ShowHiddenInCustomizeBox = True
    ShowRowFooter = True
    OnReloadGroupList = grEntReloadGroupList
    object grEntCODITE: TdxDBGridMaskColumn
      Caption = 'Código do Item'
      Width = 123
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CODITE'
    end
    object grEntNUMDOC: TdxDBGridColumn
      Caption = 'Doc/Dup'
      Width = 83
      BandIndex = 0
      RowIndex = 0
      FieldName = 'NUMDOC'
    end
    object grEntDTEHIS: TdxDBGridColumn
      Caption = 'Data Op'
      Width = 76
      BandIndex = 0
      RowIndex = 0
      FieldName = 'DTEHIS'
    end
    object grEntCODTIP: TdxDBGridColumn
      Alignment = taCenter
      Caption = 'Tipo'
      HeaderAlignment = taCenter
      Width = 54
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CODTIP'
    end
    object grEntFORNECEDOR: TdxDBGridColumn
      Width = 138
      BandIndex = 0
      RowIndex = 0
      FieldName = 'FORNECEDOR'
    end
    object grEntDSCPRO: TdxDBGridMaskColumn
      Width = 238
      BandIndex = 0
      RowIndex = 0
      FieldName = 'DSCPRO'
    end
    object grEntNUMPRO: TdxDBGridColumn
      Caption = 'Part Number'
      Width = 117
      BandIndex = 0
      RowIndex = 0
      FieldName = 'NUMPRO'
    end
    object grEntCODUND: TdxDBGridMaskColumn
      Alignment = taCenter
      HeaderAlignment = taCenter
      Width = 51
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CODUND'
    end
    object grEntCLSIPI: TdxDBGridColumn
      Caption = 'CF'
      Width = 83
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CLSIPI'
    end
    object grEntVLUEN2: TdxDBGridMaskColumn
      HeaderAlignment = taRightJustify
      Width = 78
      BandIndex = 0
      RowIndex = 0
      FieldName = 'VLUEN2'
    end
    object grEntCSTOUT: TdxDBGridMaskColumn
      HeaderAlignment = taRightJustify
      Width = 76
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CSTOUT'
    end
    object grEntVPFOUT: TdxDBGridColumn
      HeaderAlignment = taRightJustify
      Width = 78
      BandIndex = 0
      RowIndex = 0
      FieldName = 'VPFOUT'
    end
    object grEntVPFITE: TdxDBGridMaskColumn
      Caption = 'Ult. Custo'
      HeaderAlignment = taRightJustify
      Width = 92
      BandIndex = 0
      RowIndex = 0
      FieldName = 'VPFITE'
    end
    object grEntVB1ITE: TdxDBGridMaskColumn
      Caption = 'Tabela 1'
      HeaderAlignment = taRightJustify
      Width = 83
      BandIndex = 0
      RowIndex = 0
      FieldName = 'VB1ITE'
    end
    object grEntDS1ITE: TdxDBGridColumn
      Caption = '% Desc 1'
      HeaderAlignment = taRightJustify
      Width = 77
      BandIndex = 0
      RowIndex = 0
      FieldName = 'DS1ITE'
    end
    object grEntMK1ITE: TdxDBGridMaskColumn
      Caption = '% Mk 1'
      HeaderAlignment = taRightJustify
      Width = 68
      BandIndex = 0
      RowIndex = 0
      FieldName = 'MK1ITE'
    end
    object grEntVB3ITE: TdxDBGridColumn
      Caption = 'Tabela 3'
      HeaderAlignment = taRightJustify
      Width = 80
      BandIndex = 0
      RowIndex = 0
      FieldName = 'VB3ITE'
    end
    object grEntDS3ITE: TdxDBGridColumn
      Alignment = taRightJustify
      Caption = '% Desc 3'
      HeaderAlignment = taRightJustify
      Width = 80
      BandIndex = 0
      RowIndex = 0
      FieldName = 'DS3ITE'
    end
    object grEntMK3ITE: TdxDBGridColumn
      Alignment = taRightJustify
      Caption = '% Mk 3'
      HeaderAlignment = taRightJustify
      Width = 76
      BandIndex = 0
      RowIndex = 0
      FieldName = 'MK3ITE'
    end
    object grEntPERCOM: TdxDBGridColumn
      HeaderAlignment = taRightJustify
      Width = 85
      BandIndex = 0
      RowIndex = 0
      FieldName = 'PERCOM'
    end
    object grEntCODSIT: TdxDBGridMaskColumn
      Width = 50
      BandIndex = 0
      RowIndex = 0
      FieldName = 'CODSIT'
    end
    object grEntQTANTE: TdxDBGridColumn
      Caption = 'Est Inicial'
      HeaderAlignment = taRightJustify
      Width = 84
      BandIndex = 0
      RowIndex = 0
      FieldName = 'QTANTE'
    end
    object grEntQTPEN2: TdxDBGridColumn
      Caption = 'Qtd Entrada'
      HeaderAlignment = taRightJustify
      Width = 97
      BandIndex = 0
      RowIndex = 0
      FieldName = 'QTPEN2'
      SummaryFooterType = cstSum
      SummaryFooterFormat = '###,###,##0'
    end
    object grEntQTATUA: TdxDBGridColumn
      Caption = 'Est Final'
      HeaderAlignment = taRightJustify
      Width = 76
      BandIndex = 0
      RowIndex = 0
      FieldName = 'QTATUA'
    end
    object grEntQTDQTE: TdxDBGridColumn
      Caption = 'Est Atual'
      HeaderAlignment = taRightJustify
      Width = 85
      BandIndex = 0
      RowIndex = 0
      FieldName = 'QTDQTE'
    end
    object grEntQTAQTE: TdxDBGridColumn
      Caption = 'Adquirido'
      HeaderAlignment = taRightJustify
      Width = 77
      BandIndex = 0
      RowIndex = 0
      FieldName = 'QTAQTE'
    end
    object grEntLOGUSU: TdxDBGridColumn
      Caption = 'Usuario'
      Width = 110
      BandIndex = 0
      RowIndex = 0
      FieldName = 'LOGUSU'
    end
  end
  object bImprimir: TBitBtn
    Left = 589
    Top = 266
    Width = 195
    Height = 44
    Caption = '&Imprimir'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 25
    OnClick = bImprimirClick
    Glyph.Data = {
      
      D000}
  end
  object EdModPfa: TdxColorPickEdit
    Left = 143
    Top = 12
    Width = 183
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 0
    HideSelection = False
    Text = 'Compras'
    DropDownListStyle = True
    Items.Strings = (
      ''
      'RMA'
      'Compras'
      'Servicos'
      'Retornos'
      'Transferencias'
      'Industrializacoes'
      'Outros')
    CorDeFoco = clInfoBk
  end
  object EdCodTmo: TdxColorEdit
    Left = 143
    Top = 81
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 5
    OnExit = EdCodTmoExit
    OnKeyDown = EdCodTmoKeyDown
    OnKeyPress = EdPsqNumResKeyPress
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdNomTmo: TdxColorEdit
    Left = 254
    Top = 81
    Width = 383
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 6
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdCodCat: TdxColorEdit
    Left = 143
    Top = 104
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 7
    OnExit = EdCodCatExit
    OnKeyDown = EdCodCatKeyDown
    OnKeyPress = EdPsqNumResKeyPress
    CharCase = ecUpperCase
    MaxLength = 0
    CorDeFoco = clInfoBk
    StoredValues = 2
  end
  object EdCodTip: TdxColorEdit
    Left = 143
    Top = 127
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 9
    OnExit = EdCodTipExit
    OnKeyDown = EdCodTipKeyDown
    OnKeyPress = EdPsqNumResKeyPress
    CharCase = ecUpperCase
    MaxLength = 0
    CorDeFoco = clInfoBk
    StoredValues = 2
  end
  object EdCodMrc: TdxColorEdit
    Left = 143
    Top = 150
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 11
    OnExit = EdCodMrcExit
    OnKeyDown = EdCodMrcKeyDown
    OnKeyPress = EdPsqNumResKeyPress
    CharCase = ecUpperCase
    MaxLength = 0
    CorDeFoco = clInfoBk
    StoredValues = 2
  end
  object EdCodClp: TdxColorEdit
    Left = 143
    Top = 173
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 13
    OnExit = EdCodClpExit
    OnKeyDown = EdCodClpKeyDown
    OnKeyPress = EdPsqNumResKeyPress
    CharCase = ecUpperCase
    MaxLength = 1
    CorDeFoco = clInfoBk
    StoredValues = 2
  end
  object EdCodGru: TdxColorEdit
    Left = 143
    Top = 196
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 15
    OnExit = EdCodGruExit
    OnKeyDown = EdCodGruKeyDown
    OnKeyPress = EdPsqNumResKeyPress
    CharCase = ecUpperCase
    MaxLength = 3
    CorDeFoco = clInfoBk
    StoredValues = 2
  end
  object EdCodSub: TdxColorEdit
    Left = 143
    Top = 219
    Width = 87
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 17
    OnExit = EdCodSubExit
    OnKeyDown = EdCodSubKeyDown
    OnKeyPress = EdPsqNumResKeyPress
    CharCase = ecUpperCase
    MaxLength = 4
    CorDeFoco = clInfoBk
    StoredValues = 2
  end
  object EdCodPro: TdxColorEdit
    Left = 143
    Top = 242
    Width = 87
    Hint = 'Item'
    Color = 16577773
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 19
    OnExit = EdCodProExit
    OnKeyDown = EdCodProKeyDown
    CharCase = ecUpperCase
    MaxLength = 5
    CorDeFoco = clInfoBk
    StoredValues = 2
  end
  object EdNomCat: TdxColorEdit
    Left = 254
    Top = 104
    Width = 383
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 8
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdNomTip: TdxColorEdit
    Left = 254
    Top = 127
    Width = 383
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 10
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdNomMrc: TdxColorEdit
    Left = 254
    Top = 150
    Width = 531
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 12
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdNomClp: TdxColorEdit
    Left = 254
    Top = 173
    Width = 531
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 14
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdNomGru: TdxColorEdit
    Left = 254
    Top = 196
    Width = 531
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 16
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdNomSub: TdxColorEdit
    Left = 254
    Top = 219
    Width = 531
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 18
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object EdNomPro: TdxColorEdit
    Left = 254
    Top = 242
    Width = 531
    Color = 16577773
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Style.BorderColor = 14789952
    Style.BorderStyle = xbsSingle
    Style.ButtonStyle = btsSimple
    Style.ButtonTransparence = ebtInactive
    Style.HotTrack = True
    Style.Shadow = True
    TabOrder = 20
    CharCase = ecUpperCase
    CorDeFoco = clInfoBk
  end
  object rgPeriodo: TRadioGroup
    Left = 637
    Top = 31
    Width = 147
    Height = 117
    Caption = ' Por Periodo de '
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ItemIndex = 1
    Items.Strings = (
      'Entrada'
      'Documento'
      'Operação')
    ParentFont = False
    TabOrder = 23
  end
  object CmpR07: TwwQuery
    CachedUpdates = True
    DatabaseName = 'ISade'
    SQL.Strings = (
      'Select CmpR07.*,'
      '           EstPro.DscPro,'
      '           EstPro.CodAnt,'
      '           EstPro.CodMrc,'
      '           EstPro.NumPro,'
      '           EstMrc.NomMrc,'
      '           FinCom.PerCom,'
      '           EstPro.CodUne as CodUnd,'
      '           EstPro.ClfEnt as ClsIpi,'
      
        '           TextoOcor(EstPro.CodSt1,'#39#39',EstPro.CodSt2,'#39#39','#39#39','#39#39','#39#39')' +
        ' as CodSit,'
      
        '           TextoOcor(IntStrZeros(EstPro.CodMrc,5),'#39'-'#39',EstMrc.Nom' +
        'Mrc,'#39#39','#39#39','#39#39','#39#39') as Marca,'
      
        '           TextoOcor(IntStrZeros(CmpR07.CodFor,5),'#39'-'#39',CmpR07.Nom' +
        'For,'#39#39','#39#39','#39#39','#39#39') as Fornecedor,'
      
        '           CmpR07.CodClp || _UNICODE_FSS '#39'-'#39' || CmpR07.CodGru ||' +
        ' _UNICODE_FSS '#39'.'#39' || CmpR07.CodSub || _UNICODE_FSS '#39'.'#39' || CmpR07' +
        '.CodPro as CodIte'
      'From CmpR07 LEFT JOIN EstPro ON (CmpR07.CodClp = EstPro.CodClp)'
      
        '                                                   AND (CmpR07.C' +
        'odGru = EstPro.CodGru)'
      
        '                                                   AND (CmpR07.C' +
        'odSub = EstPro.CodSub)'
      
        '                                                   AND (CmpR07.C' +
        'odPro = EstPro.CodPro)'
      
        '                       LEFT JOIN EstMrc ON (EstPro.CodMrc = EstM' +
        'rc.CodMrc)'
      
        '                      LEFT JOIN FinCom ON (EstPro.CodCom = FinCo' +
        'm.CodCom)'
      'Where CmpR07.SeqArq = :SeqArq')
    ValidateWithMask = True
    Left = 384
    Top = 21
    ParamData = <
      item
        DataType = ftString
        Name = 'SeqArq'
        ParamType = ptInput
      end>
    object CmpR07SEQARQ: TStringField
      FieldName = 'SEQARQ'
      FixedChar = True
      Size = 22
    end
    object CmpR07CODEMP: TIntegerField
      FieldName = 'CODEMP'
    end
    object CmpR07CODFIL: TIntegerField
      FieldName = 'CODFIL'
    end
    object CmpR07LANEST: TStringField
      FieldName = 'LANEST'
      FixedChar = True
      Size = 3
    end
    object CmpR07DTEENT: TDateTimeField
      DisplayWidth = 10
      FieldName = 'DTEENT'
      EditMask = '!99/99/9999;1;_'
    end
    object CmpR07NUMENT: TIntegerField
      Alignment = taLeftJustify
      DisplayLabel = 'Seq'
      FieldName = 'NUMENT'
    end
    object CmpR07DTEDOC: TDateTimeField
      DisplayLabel = 'Data Doc'
      DisplayWidth = 10
      FieldName = 'DTEDOC'
      EditMask = '!99/99/9999;1;_'
    end
    object CmpR07CODFOR: TIntegerField
      FieldName = 'CODFOR'
    end
    object CmpR07NOMFOR: TStringField
      FieldName = 'NOMFOR'
      FixedChar = True
      Size = 100
    end
    object CmpR07CODCLP: TStringField
      FieldName = 'CODCLP'
      FixedChar = True
      Size = 1
    end
    object CmpR07CODGRU: TStringField
      FieldName = 'CODGRU'
      FixedChar = True
      Size = 3
    end
    object CmpR07CODSUB: TStringField
      FieldName = 'CODSUB'
      FixedChar = True
      Size = 4
    end
    object CmpR07CODPRO: TStringField
      FieldName = 'CODPRO'
      FixedChar = True
      Size = 5
    end
    object CmpR07CODTAM: TStringField
      FieldName = 'CODTAM'
      FixedChar = True
      Size = 10
    end
    object CmpR07CODCOR: TStringField
      FieldName = 'CODCOR'
      FixedChar = True
      Size = 10
    end
    object CmpR07CODMRC: TIntegerField
      FieldName = 'CODMRC'
    end
    object CmpR07NOMMRC: TStringField
      FieldName = 'NOMMRC'
      FixedChar = True
      Size = 100
    end
    object CmpR07VLUEN2: TFloatField
      DisplayLabel = 'Custo'
      FieldName = 'VLUEN2'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object CmpR07VALVEN: TFloatField
      DisplayLabel = 'Preço de Tabela'
      FieldName = 'VALVEN'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object CmpR07CODUND: TStringField
      DisplayLabel = 'UN'
      FieldName = 'CODUND'
      FixedChar = True
      Size = 3
    end
    object CmpR07ICMEN2: TFloatField
      DisplayLabel = '% ICMS'
      FieldName = 'ICMEN2'
    end
    object CmpR07IPIEN2: TFloatField
      DisplayLabel = '% IPI'
      FieldName = 'IPIEN2'
    end
    object CmpR07DSCEN2: TFloatField
      DisplayLabel = '% Desc'
      FieldName = 'DSCEN2'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object CmpR07VDSEN2: TFloatField
      DisplayLabel = 'Valor Desc'
      FieldName = 'VDSEN2'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object CmpR07PACEN2: TFloatField
      DisplayLabel = '% C.F.'
      FieldName = 'PACEN2'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object CmpR07VACEN2: TFloatField
      DisplayLabel = 'Valor C.F.'
      FieldName = 'VACEN2'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object CmpR07TOTEN2: TFloatField
      DisplayLabel = 'Total da Entrada'
      FieldName = 'TOTEN2'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object CmpR07CSTOUT: TFloatField
      DisplayLabel = 'FOB'
      FieldName = 'CSTOUT'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object CmpR07VPFITE: TFloatField
      DisplayLabel = 'Custo Entrada'
      FieldName = 'VPFITE'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object CmpR07QTDQTE: TFloatField
      DisplayLabel = 'Estoque Atual'
      FieldName = 'QTDQTE'
      DisplayFormat = '###,###,##0'
      Precision = 4
    end
    object CmpR07FORNECEDOR: TStringField
      DisplayLabel = 'Fornecedor'
      FieldName = 'FORNECEDOR'
      FixedChar = True
      Size = 100
    end
    object CmpR07MARKUP: TFloatField
      DisplayLabel = '% Markup'
      FieldName = 'MARKUP'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object CmpR07MARCA: TStringField
      DisplayLabel = 'Marca'
      FieldName = 'MARCA'
      FixedChar = True
      Size = 100
    end
    object CmpR07CODTMO: TIntegerField
      DisplayLabel = 'Moeda'
      FieldName = 'CODTMO'
    end
    object CmpR07VALTMO: TFloatField
      DisplayLabel = 'Cotação'
      FieldName = 'VALTMO'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object CmpR07CODSIT: TStringField
      DisplayLabel = 'S.T.'
      FieldName = 'CODSIT'
      FixedChar = True
      Size = 100
    end
    object CmpR07DSCPRO: TStringField
      DisplayLabel = 'Descrição do Item'
      FieldName = 'DSCPRO'
      FixedChar = True
      Size = 70
    end
    object CmpR07CODANT: TStringField
      FieldName = 'CODANT'
      FixedChar = True
      Size = 30
    end
    object CmpR07CLSIPI: TStringField
      FieldName = 'CLSIPI'
      FixedChar = True
      Size = 30
    end
    object CmpR07QTANTE: TFloatField
      FieldName = 'QTANTE'
      DisplayFormat = '###,###,##0'
    end
    object CmpR07QTPEN2: TFloatField
      FieldName = 'QTPEN2'
      DisplayFormat = '###,###,##0'
    end
    object CmpR07QTATUA: TFloatField
      FieldName = 'QTATUA'
      DisplayFormat = '###,###,##0'
    end
    object CmpR07VPFOUT: TFloatField
      DisplayLabel = 'CIF'
      FieldName = 'VPFOUT'
      DisplayFormat = '###,###,##0.0000'
      Precision = 4
    end
    object CmpR07CODTIP: TIntegerField
      FieldName = 'CODTIP'
    end
    object CmpR07PERCOM: TFloatField
      DisplayLabel = 'Comissão'
      FieldName = 'PERCOM'
      DisplayFormat = '###,###,##0.00'
      Precision = 2
    end
    object CmpR07DSCITE: TFloatField
      FieldName = 'DSCITE'
    end
    object CmpR07DTEHIS: TDateTimeField
      FieldName = 'DTEHIS'
    end
    object CmpR07LOGUSU: TStringField
      FieldName = 'LOGUSU'
      FixedChar = True
    end
    object CmpR07NUMPRO: TStringField
      FieldName = 'NUMPRO'
      FixedChar = True
      Size = 30
    end
    object CmpR07QTAQTE: TFloatField
      FieldName = 'QTAQTE'
      DisplayFormat = '###,###,##0'
    end
    object CmpR07NUMDOC: TFloatField
      FieldName = 'NUMDOC'
    end
    object CmpR07VB1ITE: TFloatField
      FieldName = 'VB1ITE'
      DisplayFormat = '###,###,##0.0000'
    end
    object CmpR07DS1ITE: TFloatField
      FieldName = 'DS1ITE'
      DisplayFormat = '###,###,##0.00'
    end
    object CmpR07VB3ITE: TFloatField
      FieldName = 'VB3ITE'
      DisplayFormat = '###,###,##0.0000'
    end
    object CmpR07DS3ITE: TFloatField
      FieldName = 'DS3ITE'
      DisplayFormat = '###,###,##0.00'
    end
    object CmpR07MK1ITE: TFloatField
      FieldName = 'MK1ITE'
      DisplayFormat = '###,###,##0.00'
    end
    object CmpR07MK3ITE: TFloatField
      FieldName = 'MK3ITE'
      DisplayFormat = '###,###,##0.00'
    end
    object CmpR07CODITE: TStringField
      FieldName = 'CODITE'
      Size = 16
    end
  end
  object DsEnt: TwwDataSource
    DataSet = CmpR07
    Left = 412
    Top = 21
  end
  object quSql: TwwQuery
    DatabaseName = 'ISade'
    UniDirectional = True
    ValidateWithMask = True
    Left = 384
    Top = 49
  end
  object SaveDialog: TSaveDialog
    FileName = 'ExpGrid'
    Options = [ofOverwritePrompt, ofHideReadOnly]
    Left = 412
    Top = 49
  end
  object dxComponentPrinter: TdxComponentPrinter
    CurrentLink = dxComponentPrinterLink1
    DateFormat = 1
    PreviewOptions.Caption = 'Visualizando Relatorio'
    PreviewOptions.RegistryPath = 'Software\Developer Express\PrintingSystem\DesignTime'
    TimeFormat = 1
    Left = 412
    Top = 77
    object dxComponentPrinterLink1: TdxDBGridReportLink
      Active = True
      Component = grEnt
      DateTime = 39891.9174550579
      DesignerHelpContext = 0
      PrinterPage.DMPaper = 9
      PrinterPage.Footer = 6350
      PrinterPage.Header = 6350
      PrinterPage.Margins.Bottom = 12700
      PrinterPage.Margins.Left = 4800
      PrinterPage.Margins.Right = 4000
      PrinterPage.Margins.Top = 57900
      PrinterPage.Orientation = poLandscape
      PrinterPage.PageFooter.Font.Charset = DEFAULT_CHARSET
      PrinterPage.PageFooter.Font.Color = clWindowText
      PrinterPage.PageFooter.Font.Height = -11
      PrinterPage.PageFooter.Font.Name = 'Tahoma'
      PrinterPage.PageFooter.Font.Style = []
      PrinterPage.PageFooter.LeftTitle.Strings = (
        'Emerion Consultoria e Sistemas de Informatica Ltda')
      PrinterPage.PageFooter.RightTitle.Strings = (
        'Pagina No. [Page #]')
      PrinterPage.PageHeader.Background.Brush.Color = clBlack
      PrinterPage.PageHeader.Background.PictureMode = ppmTile
      PrinterPage.PageHeader.CenterTextAlignY = taTop
      PrinterPage.PageHeader.Font.Charset = DEFAULT_CHARSET
      PrinterPage.PageHeader.Font.Color = clWindowText
      PrinterPage.PageHeader.Font.Height = -11
      PrinterPage.PageHeader.Font.Name = 'Tahoma'
      PrinterPage.PageHeader.Font.Style = []
      PrinterPage.PageHeader.LeftTextAlignY = taTop
      PrinterPage.PageHeader.LeftTitle.Strings = (
        'VMP PAPEIS'
        'VMP PAPEIS PARA EMBALAGENS LTDA'
        
          'RUA TEIXEIRA SOARES - JD DAS ACACIAS - GUARULHOS - SP CEP 03701-' +
          '010'
        ''
        'Número do Pedido :'
        'No. Documento Fiscal :'
        'Empresa :'
        'Cliente :'
        'Vendedor :'
        'Padrão de Faturamento :'
        'Emitido Em :'
        'Faturado Em :'
        'Valor do Pedido :'
        'Estado de :'
        '')
      PrinterPage.PageHeader.RightTextAlignY = taTop
      PrinterPage.PageHeader.RightTitle.Strings = (
        '[Date Printed][Time Printed]')
      PrinterPage.PageSize.X = 210000
      PrinterPage.PageSize.Y = 297000
      PrinterPage.PaperSource = 15
      PrinterPage.ScaleFactor = 45
      PrinterPage._dxMeasurementUnits_ = 0
      PrinterPage._dxLastMU_ = 2
      ReportTitle.Font.Charset = ANSI_CHARSET
      ReportTitle.Font.Color = clWindowText
      ReportTitle.Font.Height = -15
      ReportTitle.Font.Name = 'Tahoma'
      ReportTitle.Font.Style = [fsBold]
      ReportTitle.Text = #13#10'Relação de Movimentações de Entradas'#13#10#13#10
      BandColor = clWhite
      BandFont.Charset = DEFAULT_CHARSET
      BandFont.Color = clWindowText
      BandFont.Height = -11
      BandFont.Name = 'Tahoma'
      BandFont.Style = [fsBold]
      EvenFont.Charset = DEFAULT_CHARSET
      EvenFont.Color = clWindowText
      EvenFont.Height = -11
      EvenFont.Name = 'Times New Roman'
      EvenFont.Style = []
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Times New Roman'
      Font.Style = []
      FooterFont.Charset = DEFAULT_CHARSET
      FooterFont.Color = clNavy
      FooterFont.Height = -13
      FooterFont.Name = 'Times New Roman'
      FooterFont.Style = [fsBold]
      GridLineColor = clWhite
      GroupNodeFont.Charset = DEFAULT_CHARSET
      GroupNodeFont.Color = clNavy
      GroupNodeFont.Height = -13
      GroupNodeFont.Name = 'Times New Roman'
      GroupNodeFont.Style = [fsBold]
      GroupNodeColor = clWhite
      HeaderFont.Charset = ANSI_CHARSET
      HeaderFont.Color = clNavy
      HeaderFont.Height = -13
      HeaderFont.Name = 'Times New Roman'
      HeaderFont.Style = [fsBold]
      OddFont.Charset = DEFAULT_CHARSET
      OddFont.Color = clWindowText
      OddFont.Height = -11
      OddFont.Name = 'Times New Roman'
      OddFont.Style = []
      Options = [tlpoBands, tlpoHeaders, tlpoFooters, tlpoRowFooters, tlpoFlatCheckMarks]
      PreviewFont.Charset = DEFAULT_CHARSET
      PreviewFont.Color = clWindowText
      PreviewFont.Height = -11
      PreviewFont.Name = 'Times New Roman'
      PreviewFont.Style = []
      RowFooterColor = clWhite
      RowFooterFont.Charset = DEFAULT_CHARSET
      RowFooterFont.Color = clRed
      RowFooterFont.Height = -13
      RowFooterFont.Name = 'Times New Roman'
      RowFooterFont.Style = [fsBold]
    end
  end
  object PopUpMenu: TPopupMenu
    OwnerDraw = True
    Left = 384
    Top = 77
    object Exportarpara1: TMenuItem
      Caption = 'Exportar para...           '
      Hint = 'Exportar para...'
      object HTML: TMenuItem
        Caption = 'HTML'
        Hint = 'Exportar Relatorio no Formato HTML'
        OnClick = HTMLClick
      end
      object Excel: TMenuItem
        Caption = 'EXCEL'
        Hint = 'Exportar Relatorio no Formato EXCEL'
        OnClick = ExcelClick
      end
      object XML: TMenuItem
        Caption = 'XML && XSL'
        Hint = 'Exportar Relatorio no Formato XML '
        OnClick = XMLClick
      end
    end
    object Colunas1: TMenuItem
      Caption = 'Colunas...'
      Hint = 'Colunas...'
      OnClick = Colunas1Click
    end
  end
end
