...
type
  TfmManSin = class(TfmShowPadrao)
    ...
    Label8: TLabel;
    Label18: TLabel;
    EdTotPic: TdxDBColorCurrencyEdit;
    dxDBColorEdit1: TdxDBColorEdit;
    pnFlgOco12: TPanel;
    Label40: TLabel;
    Label41: TLabel;
    EdTotCoc: TdxDBColorCurrencyEdit;
    ...
    procedure EdTotPicExit(Sender: TObject);
    procedure EdTotCocExit(Sender: TObject);
  private
...
begin
  ...
  with fmManEpe.quSQL,SQL do begin
       ...
       Close;
       Text := ' Select NomFor From FinFor Where FinFor.CodFor = '+ QuotedStr(IntToStr(fmManEpe.CmpEntCodFor.Value));
       Open;
  ...
  with fmManEpe.quSQL,SQL do begin
       ...
       with Params do begin
            ...
            Params[1].AsString := fmManEpe.CmpEntCodPfa.Value;
...
begin
  ...
  if fmManEpe.CmpEntCodFor.Value > 0 then begin
     ...
     with fmManEpe.quSql,SQL do begin
          ...
          Close;
          Text := ' Select * From FinFor Where FinFor.CodFor = '+ QuotedStr(IntToStr(fmManEpe.CmpEntCodFor.Value));
          Open;

     end;

     if fmManEpe.quSql.FieldbyName('CodFor').AsInteger = fmManEpe.CmpEntCodFor.Value then begin
  ...
  if Trim( fmManEpe.CmpEntCodPfa.Value ) <> '' then begin
     ...
     with fmManEpe.quSql,SQL do begin
          ...
          with Params do begin
               ...
               Params[0].AsString := fmManEpe.CmpEntTipPfa.Value;
               Params[1].AsString := fmManEpe.CmpEntCodPfa.Value;
  ...
  if key = #20 then begin
     ...
     if (GUsu_Id = 1) or (GUsu_Id = 999) then begin
        ...
        try
           ...
           fmManEn7.CmpEnt.Params[0].AsInteger  := fmManEpe.CmpEntCodEmp.Value;
           fmManEn7.CmpEnt.Params[1].AsDateTime := fmManEpe.CmpEntDteEnt.Value;
           fmManEn7.CmpEnt.Params[2].AsInteger  := fmManEpe.CmpEntNumEnt.Value;
...
procedure TfmManSin.EdTotPicExit(Sender: TObject);
begin
  inherited;
  if fmManEpe.CmpEntTotPic.Value <> fmManEpe.CmpEntTotPis.Value then begin

     if fmManEpe.CmpEntTotPic.Value > fmManEpe.CmpEntTotPis.Value then
        TotDif := fRound(fmManEpe.CmpEntTotPic.Value - fmManEpe.CmpEntTotPis.Value,2)
     else
        TotDif := fRound(fmManEpe.CmpEntTotPis.Value - fmManEpe.CmpEntTotPic.Value,2);

     if TotDif > DifEnt then
        pnFlgOco12.Color := clRed
     else
        pnFlgOco12.Color := clLime;
...
procedure TfmManSin.EdTotCocExit(Sender: TObject);
begin
  inherited;
  if fmManEpe.CmpEntTotCoc.Value <> fmManEpe.CmpEntTotCof.Value then begin

     if fmManEpe.CmpEntTotCoc.Value > fmManEpe.CmpEntTotCof.Value then
        TotDif := fRound(fmManEpe.CmpEntTotCoc.Value - fmManEpe.CmpEntTotCof.Value,2)
     else
        TotDif := fRound(fmManEpe.CmpEntTotCof.Value - fmManEpe.CmpEntTotCoc.Value,2);

     if TotDif > DifEnt then
        pnFlgOco13.Color := clRed
     else
        pnFlgOco13.Color := clLime;
