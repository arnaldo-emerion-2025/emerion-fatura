...
begin
  ...
  if Trim( EdPsqModPfa.Text ) <> '' then begin
     ...
     if EdPsqModPfa.Text <> 'Compras' then
        ...
        sFiltro := sFiltro + ' and (CmpEnt.ModPfa = '+ QuotedStr('Compras') +' or  CmpEnt.ModPfa = '+ QuotedStr('Importacao') +')';
  ...
  if rgPeriodo.ItemIndex = 1 then begin
     ...
     if Trim(fLimpaStr(EdPsqDteOpe1.Text)) <> '' then sFiltro := sFiltro + ' and CmpEnt.DteDoc >= '+ QuotedStr(fDateToSQL(EdPsqDteOpe1.Date));
     if Trim(fLimpaStr(EdPsqDteOpe2.Text)) <> '' then sFiltro := sFiltro + ' and CmpEnt.DteDoc <= '+ QuotedStr(fDateToSQL(EdPsqDteOpe2.Date));
  ...
  with quSQL,SQL do begin
       ...
       Text := ' Insert Into CmpR01(SeqArq,CodFor,TotGec,QtdIte,QtdGec)'+
               ...
               '        Count(*) as QtdGec'+
               ' From CmpEnt'+ sFiltro + ' Group by CmpEnt.CodFor';
       ExecSQL;
...
begin
  ...
  dxComponentPrinterLink1.PrinterPage.PageHeader.LeftTitle.Add('Grupo de fornecedores: '+ EdPsqCodGFo.Text + ' - ' + EdPsqNomGFo.Text);
...
begin
  inherited;
  Save('htm', 'HTML File (*.htm; *.html)|*.htm', 'Curva ABC de fornecedores - Entradas de itens.htm', grEnt.SaveToHTML);
end;

procedure TfmCmpR08.ExcelClick(Sender: TObject);
begin
  inherited;
  Save('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', 'Curva ABC de fornecedores - Entradas de itens.xls', grEnt.SaveToXLS);
end;

procedure TfmCmpR08.XMLClick(Sender: TObject);
begin
  inherited;
  Save('xml', 'XML File (*.xml)|*.xml', 'Curva ABC de fornecedores - Entradas de itens.xml', grEnt.SaveToXML);
end;
