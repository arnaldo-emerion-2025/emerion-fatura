...
begin
  inherited;
  Save('htm', 'HTML File (*.htm; *.html)|*.htm', 'Relação de romaneios pendentes.htm', grPe2.SaveToHTML);
end;

procedure TfmCmpR12.ExcelClick(Sender: TObject);
begin
  inherited;
  Save('xls', 'Microsoft Excel 4.0 Worksheet (*.xls)|*.xls', 'Relação de romaneios pendentes.xls', grPe2.SaveToXLS);
end;

procedure TfmCmpR12.XMLClick(Sender: TObject);
begin
  inherited;
  Save('xml', 'XML File (*.xml)|*.xml', 'Relação de romaneios pendentes.xml', grPe2.SaveToXML);
end;
