...
type
  TfmManPra = class(TfmShowPadrao)
    ...
    procedure EdCodFor1Enter(Sender: TObject);
    procedure EdPrePedEnter(Sender: TObject);
    procedure grPe5Enter(Sender: TObject);
  private
...
begin
  ...
  if pSaida = 'Nao' then begin
     ...
     if not EdCodFor.Focused then begin
        ...
        if CmpPe5CodFor.Value = 0 then fmsgErro('Fornecedor informado não localizado.',EdCodFor);
...
begin
  ...
  if key = 40 then begin {Tecla - Seta para Baixo}
     ...
     if CmpPe5NroPe5.Value = CmpPedQtpPed.Value then begin

        if CmpPedSldGer.Value > 0 then CmpPe5.Append;
  ...
  if key = 46 then begin {Tecla - DEL}
     ...
     if CmpPe5Id_CmpPe5.Value > 0 then begin
        ...
        if NroPe5 < CmpPedQtpPed.Value then
...
begin
  if pSaida = 'Nao' then begin
     ...
     if CmpPe5.State <> dsBrowse then begin
        ...
        if CmpPe5DtvPe5.Value > CmpPedDpePed.Value then
           CmpPe5PraPe5.Value := StrToInt(FloatToStr(CmpPe5DtvPe5.Value - CmpPedDpePed.Value))
        else
           CmpPe5PraPe5.Value := 0;

        if CmpPe5CodFor.Value = 0 then fmsgErro('Campo de preenchimento obrigatório não informado.',EdCodFor);
        ...
        if CmpPe5.State = dsInsert then begin
           ...
           CmpPe5.Locate('Id_CmpPed;NroPe5',VarArrayOf([CmpPedId_CmpPed.Value,NroPe5]),[LoPartialKey]);
...
begin
  ...
  CmpPe5Id_CmpPed.Value := CmpPedId_CmpPed.Value;

  CmpPe5PraPe5.Value := 0;
  CmpPe5FlgAtu.Value := ' ';
  CmpPe5FlgPag.Value := 'Nao';
  CmpPe5ValPe5.Value := CmpPedSldGer.Value;
  CmpPe5DtvPe5.Value := CmpPedDpePed.Value;
  CmpPe5CodFor.Value := CmpPedCodFor.Value;
...
begin
  ...
  if CmpPedCodFor.Value > 0 then begin
     ...
     with quSql,SQL do begin
          ...
          Close;
          Text := ' Select * From FinFor Where FinFor.CodFor = '+ QuotedStr(IntToStr(CmpPedCodFor.Value));
          Open;
...
procedure TfmManPra.FormCreate(Sender: TObject);
begin
  inherited;
  pSaida := 'Nao';
end;

procedure TfmManPra.EdPraPe5Exit(Sender: TObject);
var
Saida : Boolean;
begin
  if pSaida = 'Nao' then begin
     ...
     if (Tecla <> 'ESC') and (Tecla <> 'UP') then begin
        ...
        if saida then begin
           ...
           if CmpPe5.State <> dsBrowse then
              CmpPe5DtvPe5.Value := CmpPedDpePed.Value + CmpPe5PraPe5.Value;
...
procedure TfmManPra.EdCodFor1Enter(Sender: TObject);
begin
  inherited;
  LbText.Caption := 'F1-Iniciais F2-Inteligente';
end;

procedure TfmManPra.EdPrePedEnter(Sender: TObject);
begin
  inherited;
  LbText.Caption := '';
end;

procedure TfmManPra.grPe5Enter(Sender: TObject);
begin
  inherited;
  LbText.Caption := 'ENTER-Alterar informações DEL-Excluir Seta para Baixo-Avançar/Incluir';
end;
