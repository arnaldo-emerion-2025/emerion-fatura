...
type
  TfmManPed = class(TfmPadrao)
    ...
    CmpPe2TIPDSC: TStringField;
    CmpPe2VDSPE2: TFloatField;
    CmpPe2TIPACR: TStringField;
    ...
    CmpPedOB1FPE: TStringField;
    CmpPedOB2FPE: TStringField;
    CmpPedOB3FPE: TStringField;
    CmpPe2DESPE2: TStringField;
    CmpPe2OBSPE2: TStringField;
    CmpPe2QTDREQ: TFloatField;
    CmpPedQTIPRE: TIntegerField;
    CmpPe2ID_CMPPR2: TIntegerField;
    procedure SaiGrupo;
...
begin
  inherited;
  if fmManPfo.Id_CmpPed > 0 then begin

     sEntr := 'S';

     CmpPed.Close;
     CmpPed.Params[0].AsInteger := fmManPfo.Id_CmpPed;
     CmpPed.Open;
     ...
     with quSQL,SQL do begin
          ...
          Close;
          Text := ' Select GerEmp.ApeEmp From GerEmp Where GerEmp.CodEmp = '+ QuotedStr(IntToStr(CmpPedCodEmp.Value));
          Open;
     ...
     with quSQL,SQL do begin
          ...
          Close;
          Text := ' Select FinFor.NomFor,FinFor.SigUfe From FinFor Where FinFor.CodFor = '+ QuotedStr(IntToStr(CmpPedCodFor.Value));
          Open;
     ...
     with quSQL,SQL do begin
          ...
          Text := ' Select EstPfa.DscPfa'+
                  ...
                  '   and EstPfa.CodPfa = '+ QuotedStr(CmpPedCodPfa.Value) ;
          Open;
     ...
     with quSQL,SQL do begin
          ...
          Close;
          Text := ' Select EstClp.NomClp From EstClp Where EstClp.CodClp = '+ QuotedStr(CmpPedCodClp.Value);
          Open;
     ...
     with quSQL,SQL do begin
          ...
          Close;
          Text := ' Select FinTmo.NomTmo From FinTmo Where FinTmo.CodTmo = '+ QuotedStr(IntToStr(CmpPedCodTmo.Value));
          Open;
     ...
     if Trim(CmpPedSitPed.Value) = 'Recebido' then
        sEnc := 'S'
     else
        begin
...
begin
  ...
  CmpPedId_CmpPed.Value := 0;

  CmpPedCodTmo.Value := 1;
  CmpPedQtiPed.Value := 0;
  CmpPedQtiPre.Value := 0;
...
begin
  ...
  CmpPe2Id_CmpPe2.Value := 0;
  CmpPe2Id_CmpPed.Value := CmpPedId_CmpPed.Value;
  ...
  CmpPe2FlgDup.Value := 'Nao';
  CmpPe2TipDsc.Value := 'Percentual';
  CmpPe2TipAcr.Value := 'Percentual';
...
begin
  ...
  if CmpPedFlgOco.Value = 'Sim' then
  ...
  pnId_CmpPed.Caption := IntToStr(CmpPedId_CmpPed.Value);
...
begin
  ...
  if key = 122 then begin // F11-Ocorrencias do Pedido //
     ...
     if CmpPedFlgOco.Value = 'Sim' then begin
        ...
        try
           ...
           fmPsqOcp.CmpPe2.Params[0].AsInteger := CmpPedId_CmpPed.Value;
...
begin
  ...
  if (Tecla <> 'ESC') and (Tecla <> 'UP') then begin
     ...
     if (not EdDscPe2.Focused) and
        (not EdIcmPe2.Focused) and
        (not EdIpiPe2.Focused) and
        (not EdVluPe2.Focused) and
        (not EdQtpPe2.Focused) and
        (not EdCodPro.Focused) and
        (not EdCodSub.Focused) and
        (not EdCodGru.Focused) and
        (not EdCodClp.Focused) then
  ...
  if key = 46 then begin {Tecla - DEL}
     ...
     if Trim( CmpPe2CodClp.Value ) <> '' then begin
        ...
        if NroPe2 < CmpPedQtiPed.Value then
           CmpPe2.Locate('Id_CmpPed;NroPe2',VarArrayOf([CmpPe2Id_CmpPed.Value,NroPe2]),[LoPartialKey])
        else
           begin
...
begin
  ...
  if Finalizar = 'N' then begin
     ...
     if fMsg('Deseja Realmente Abandonar o Pedido?','O') then begin
        ...
        if CmpPedId_CmpPed.Value = Null then CmpPed.Insert;
...
begin
  ...
  if sEnc = 'S' then begin
     ...
     if Trim(CmpPedSitPed.Value) = 'Recebido' then
        fMsg('Operação não Pode ser Realizada. Pedido com Recebimento já Realizado.','E')
     else
        begin
        
        if Trim(CmpPedSitPed.Value) = 'Cancelado' then
...
begin
  ...
  if CmpPed.State = dsInsert then begin
     ...
     CmpPedCodPfa.Value := Trim(CmpPedCodPfa.Value);
     CmpPedTipPfa.Value := Trim(CmpPedTipPfa.Value);
     CmpPedNomCon.Value := Trim(CmpPedNomCon.Value);
  ...
  with quSQL,SQL do begin
       ...
       Close;
       Text := ' Select CmpPed.Id_CmpPed From CmpPed Where CmpPed.SeqPed = '+ QuotedStr(CmpPedSeqPed.Value);
       Open;

  end;

  CmpPed.Close;
  CmpPed.Params[0].AsInteger := quSQL.FieldbyName('Id_CmpPed').AsInteger;
  CmpPed.Open;
  ...
  if Status = 'dsInsert' then begin
     ...
     with fmManPfo.CmpPed,SQL do begin
          ...
          Close;
          Text := fmManPfo.sBase +
                  ' and CmpPed.Id_CmpPed = '+ QuotedStr(IntToStr(CmpPedId_CmpPed.Value));
          Open;

     end;

     if CmpPedQtiPre.Value > 0  then begin

        try

           fmManPe4 := TfmManPe4.Create(Self);

           fmManPe4.CmpPre.Close;
           fmManPe4.CmpPre.Params[0].AsInteger := CmpPedId_CmpPed.Value;
           fmManPe4.CmpPre.Open;
...
begin
  ...
  if (Tecla <> 'ESC') and (Tecla <> 'UP') then begin
     ...
     if psaida = 'Sim' then begin
        ...
        CmpPe2CodTam.Value := Trim(CmpPe2CodTam.Value);
        CmpPe2CodCor.Value := Trim(CmpPe2CodCor.Value);
        CmpPe2DesPe2.Value := Trim(CmpPe2DesPe2.Value);
        CmpPe2ObsPe2.Value := Trim(CmpPe2ObsPe2.Value);
                      ...
                      try
                         ...
                         if EdCodClp.Enabled then
                            ...
                            if EdCodGru.Enabled then
                               ...
                               if EdDesPe2.Enabled then EdDesPe2.SetFocus;
  ...
  if Trim( CmpPe2CodPro.Value ) <> '' then begin
     ...
     if CmpPe2.State <> dsBrowse then begin
        ...
        with quSQL,SQL do begin
             ...
             Text := ' Select Count(*) as QtdReg'+
                     ' From CmpPe2'+
                     ' Where CmpPe2.Id_CmpPed = '+ QuotedStr(IntToStr(CmpPe2Id_CmpPed.Value)) +
                     '   and CmpPe2.CodClp = '+ QuotedStr(CmpPe2CodClp.Value) +
                     '   and CmpPe2.CodGru = '+ QuotedStr(CmpPe2CodGru.Value) +
                     '   and CmpPe2.CodSub = '+ QuotedStr(CmpPe2CodSub.Value) +
                     '   and CmpPe2.CodPro = '+ QuotedStr(CmpPe2CodPro.Value) +
                     '   and CmpPe2.CodTam = '+ QuotedStr(CmpPe2CodTam.Value) +
                     '   and CmpPe2.CodCor = '+ QuotedStr(CmpPe2CodCor.Value) ;
             Open;
        ...
        if CmpPe2FlgDup.Value = 'Sim' then begin
           ...
           with quSQL,SQL do begin
                ...
                Text := ' Select EstPro.CodPro,'+
                        ...
                        '        EstPro.DscPro '+
                        ' From EstPro '+
                        ' Where EstPro.CodClp = '+ QuotedStr(CmpPe2CodClp.Value)+
                        '   and EstPro.CodGru = '+ QuotedStr(CmpPe2CodGru.Value)+
                        '   and EstPro.CodSub = '+ QuotedStr(CmpPe2CodSub.Value)+
                        '   and EstPro.CodPro = '+ QuotedStr(CmpPe2CodPro.Value);
                Open;
              ...
              if quSql.FieldbyName('CodPro').AsString = CmpPe2CodPro.Value then begin
                 ...
                 if quSql.FieldbyName('CodPro').AsString = CmpPe2CodPro.Value then begin
                    ...
                    if CmpPedCodTmo.Value = 1 then begin
                       ...
                       CmpPe2VluPe2.Value := quSql.FieldbyName('VpfIte').AsFloat;
                       CmpPe2VlqPe2.Value := quSql.FieldbyName('VpfIte').AsFloat;

                       end
                    else
                       begin

                       CmpPe2VluPe2.Value := quSql.FieldbyName('VpfOut').AsFloat;
                       CmpPe2VlqPe2.Value := quSql.FieldbyName('VpfOut').AsFloat;

                    end;

                    if Trim( CmpParFlgTab.Value ) <> '' then begin

                       if CmpParFlgTab.Value = 'Custo Referencial' then CmpPe2ValVen.Value := quSql.FieldbyName('VcrIte').AsFloat;
...
begin
  ...
  if CmpPedFlgOco.Value = 'Nao' then
...
begin
  ...
  if CmpPedFlgOco.Value = 'Nao' then
     LbText.Caption := 'D-Descr F1-Ini F2-Int F3-Descr F4-Est F5-Ref F6-Int/Descr F7-Int/Ref F8-PN Ctrl+G-Requisições'
  else
     LbText.Caption := 'D-Descr F1-Ini F2-Int F3-Descr F4-Est F5-Ref F6-Int/Descr F7-Int/Ref F8-PN F11-Ocor Ctrl+G-Requisições';
  ...
  if grPe2.Focused then begin
     ...
     if key = #7 then begin
        ...
        if CmpPedQtiPre.Value > 0  then begin
           ...
           if Id_CmpPe2 > 0 then CmpPe2.Locate('Id_CmpPe2',Id_CmpPe2,[LoPartialKey]);
